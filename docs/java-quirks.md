...
| 14 | `LineSegment.startPointApprox` / `endPointApprox` (84-103) | Result depends on whether `startPoint()` was called earlier (memo present → exact-to-float; else float intersection), differing by ULPs. Call-order-dependent. | `line_segment.rs` — under review (Task 15) | Always compute from the exact point. |
