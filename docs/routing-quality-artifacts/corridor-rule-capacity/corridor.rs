...
impl Corridor {
    ...
    pub fn for_net(board: &copper_board::Board, net: i32) -> Option<Self> {
        ...
        let capacity = (0..board.get_layer_count())
            ...
            .fold(0.0, f64::max);
        let trace_capacity = (0..board.get_layer_count()).map(|layer| {
            let classes: Vec<_> = signals.iter().map(|(n,_,_)| board.rules.net_classes.get(board.rules.nets.get(*n).unwrap().net_class).get_trace_clearance_class()).collect();
            let clearance = classes.iter().flat_map(|a| classes.iter().map(move |b| (*a,*b))).map(|(a,b)| board.rules.clearance_matrix.get_value(a,b,layer,false)).max().unwrap_or(0);
            signals.iter().map(|(n,_,_)| 2.0*f64::from(board.rules.get_trace_half_width(*n,layer).max(0)) + f64::from(clearance)).sum::<f64>()
        }).fold(0.0,f64::max);
        println!("net={} signals={} global_capacity={} trace_capacity={} original_terminal_span={}", net,signals.len(),capacity,trace_capacity,across[1]-across[0]);
