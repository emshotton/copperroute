...
## PR #34 and deadline control

Opened PR #34 with the full generated benchmark report, origin splits and accuracy caveats. Completed drc-mask-one-pass-no-opt: all six main/candidate pairs finish without deadlines, produce byte-identical SES output, and receive identical KiCad findings. No referee failures. The first-pass control supports routing compatibility without claiming that every later timed-out pass is identical. Original full-run gate failure remains disclosed. All artifacts, including the intentionally aborted optimizer-enabled control, are retained on workbench.
