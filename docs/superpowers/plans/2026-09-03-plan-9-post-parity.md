...
- **No static mutable state.** The one recorded exception, `copper_geometry::Line`'s identity counter (ruling AE), is **scheduled for deletion** by this plan: #218 (Task 14) and #74 (Task 18) are its only two readers. If both land, Task 24 deletes the counter and the port has zero exceptions. If #74 is abandoned at Task 18, the counter survives with #74 named as the reason, and that is recorded in Task 25's report.
- **No threads below `crates/copperroute`, and no background waiters anywhere.** The two MCP spawn sites stay two. No task starts a process it then polls; every long run (a corpus sweep, a stem A/B) is run to completion in the foreground with its **full output redirected to a file** under `scripts/differential/out/` or `benchmark/baselines/`, and the file is read from disk. **Never `| tail`, never `| head` on a gate's output** — a truncated gate is an unread gate.
...
The roadmap's `Compat::{Java, Fixed}` design and its whole mode axis; §7.1/§7.2's release split; "byte parity is the acceptance test"; "do not fix these in the port until parity is proven" (`docs/java-quirks.md`'s first rule — Task 0 rewrites it); the Plan 8 scan's R3/R15 **additive-only** constraint on `copper-router` (a Plan 8 constraint that does not survive into Plan 9 — see #251, Task 20); the mandatory `-XX:hashCode=2` and `-Duser.language=en -Duser.country=US` JVM flags (survey §7.3 — both are jar-side workarounds for #144/#145, which the port does not have, and they disappear with the drivers that needed them).
...
cargo test -p copper-router --test batch_parity
...
| normalized score | `copper_router::score::normalized_score(&ScoringSettings)` — an `f32` throughout, deliberately (survey §9.1) | must not fall by more than the `f32` noise floor |
...
`COPPERROUTE_ROUTER_BUDGET=disabled` (ruling AI) and so cannot trip survey §3.1's wall-clock
...
cd benchmark
scripts/remote-run.sh --remote-dir copperroute/benchmark --jobs 12 --poll-interval 300 -- \
...
| **M3** | **Task 18** (the last behavioural task) | the whole branch. Read into Task 25's completion report. Any group-18 row that shows a hard loss here is **reverted at M3** — each of the five remaining rows is individually abandonable by construction. |
...
|---|---|---|---|
| **G** | geometry + DSN round trip | `tests/reference/<board>/*` for the 6 rows of `fixtures.txt`, plus `crates/copper-dsn`'s round-trip goldens | **regenerated from the port**; the 2.3.0 Specctra spelling is kept as a *decision* (#92 is a real HEAD bug), not as a jar comparison |
...
Two further synthetic fixtures are owned by Task 6 (#185's resampled polyline whose two ends meet; #181's two parallel lines with a null intersection).
...
```rust
// crates/copper-router/src/pipeline/airline.rs — Task 2 (R1). The roster carries FIVE `// not ported:`
...
// crates/copper-router/src/pipeline/batch_autorouter.rs — Task 2 (R1) then Task 9 (#213).
...
// crates/copper-router/src/autoroute/path/inserter.rs — Task 2 (R2).
...
fn insert_fanout_micro_neckdown(&mut self, /* … */) -> InsertResult;     // uses copper_board::BoardRules::get_min_trace_half_width

// crates/copper-router/src/pipeline/stop.rs — Task 1 (#234). `RouterBudget` is DECLARED at
// stop.rs:493 and re-exported as `copper_router::pipeline::RouterBudget`; `crates/copper-core/src/ctx.rs:42`
// merely HOLDS one (`pub budget: copper_router::pipeline::RouterBudget`, built at :56). The edit is in
...
// crates/copper-dsn/src/lib.rs — Task 4 (#91). A third variant, not a hard failure.
pub enum BoardReadResult { Ok(Board), Partial { board: Board, diagnostic: ReadDiagnostic }, Err(DsnError) }

// crates/copper-dsn/src/rules_reader.rs — Task 4 (#112). The -1 sentinel becomes unrepresentable.
pub enum RuleLayerScope { AllLayers, One(usize) }
fn apply_rules(&mut self, /* … */, scope: RuleLayerScope) -> Result<(), RulesError>;

// crates/copper-router/src/pipeline/stop.rs + optimizer.rs — Task 9 (#214, #227, #202).
...
// crates/copper-geometry — Tasks 12, 13, 24.
...
// crates/copper-drc + crates/copper-router/src/score — Tasks 19, 20.
...
// crates/copper-settings — Task 21.
...
// crates/copperroute — Tasks 3, 19, 22.
...
5. **`--from-port` on the five generators.** Each generator keeps its jar path behind `--jar` (for the frozen baseline and for triage) and grows a `--from-port` mode that drives `target/release/copperroute` with the same argv. `gen-router-reference.sh --from-port` must run **the port's own `p6t1` twin**, not a re-implementation, so survey §7.2's property — the reference and the differential describe the same run — survives. Every generator writes `meta.txt` with **the port's git sha, the Plan 9 task at that sha, and the `RouterBudget` in force**; a golden cut before a later fix must be **loudly** invalid, which is what that line is for.
...
- `crates/copper-core/tests/register.rs::every_java_bug_marker_has_a_register_row` — new: walks `grep -rn '// Java bug:' crates/*/src`, parses the `#nnn` out of each, and asserts the id exists in `docs/java-quirks.md`. Baseline count recorded (**≥ 165**).
- `crates/copper-core/tests/register.rs::a_fixed_row_has_a_fixed_marker` — new: for every register row whose status is `fixed: T<n>`, assert a `// fixed: T<n>` marker exists at some site. **Vacuously true today; it is the gate every later task arms.**
...
- [x] The two new register tests, in `crates/copper-core/tests/register.rs`.
...
**Files:** `tests/reference-frozen/java-head-2026-09/**` (new, ~34 stem directories copied — a **sibling** of `tests/reference/`, ruling BP1); `tests/reference-frozen/README.md` (new); `tests/reference/README.md`; `benchmark/baselines/quality-baseline-java-head.tsv` (new — the jar column, written **outside** the frozen tree); `benchmark/baselines/stem-times.tsv` (the port timing baseline, seeded at Task 0, re-stamped here); `benchmark/candidates.toml`; `crates/copper-router/src/pipeline/stop.rs` (**`RouterBudget` is declared here at `:493`; `impl Default` at `:522`** — ruling BP5); `crates/copper-core/src/ctx.rs` (the holder at `:42`/`:56`); `crates/copper-settings/src/router_settings.rs` (the new `router.opt_changed_area_ms` field); `crates/copperroute/src/commands/route.rs`; `crates/copper-router/src/tightener/mod.rs` (the pull-tight budget's reader); `crates/copper-router/src/pipeline/fanout.rs` (`parse_timespan_seconds`); `crates/copper-router/src/autoroute/connection_router.rs` (`retry_connection_necked`); `crates/copper-router/tests/{batch_autorouter.rs,fanout.rs}`; `crates/copperroute/tests/cli_e2e.rs`; `scripts/gen-cli-reference.sh` (the two-run identity mode).

**Interfaces consumed:** `copper_core::ctx::RouterBudget` (Plan 8 Task 0), `copper_router::pipeline::RouterStop` (Plan 7), `scripts/quality-ab.sh` and the five `--from-port` generators (Task 0).
...
**Tests (named, binding).**
- `crates/copper-core/tests/ctx.rs::the_default_router_budget_disables_the_opt_changed_area_clock` — fails before (`default()` is 1000 ms), passes after.
- `crates/copperroute/tests/cli_e2e.rs::two_runs_of_every_ci_stem_are_byte_identical` — the two-run identity check, four CI stems.
- `crates/copper-settings/tests/router_settings.rs::opt_changed_area_ms_is_a_settable_field` — `--router.opt_changed_area_ms=250` reaches `RouterBudget`.
- `crates/copper-router/tests/batch_autorouter.rs::the_necked_retry_gets_a_fresh_time_limit` — asserts the retry's budget is the full per-connection limit, not the remainder; fails before with the measured 81/96 ms shape.
- `crates/copper-router/tests/fanout.rs::the_documented_timeout_spellings_parse` — `"5m"` → 300 s, `"300s"` → 300 s, `"1h30m"` → 5400 s; and `parse_timespan_seconds("banana")` is an **error**, not `None`-then-unbounded.
- `crates/copper-router/tests/fanout.rs::an_unparseable_fanout_timeout_is_refused` — the run fails at the setting with the string named.
...
**Files:** `crates/copper-router/src/pipeline/airline.rs` (**three of the roster's five** `// not ported:` lines — `:23` `calculateItemDistance`, `:24` `calculateMinDistance`, `:25` `getItemReferencePoint` — become ported methods; `:21` `nearestPointOnTrace` and `:22` `findClosestPointsBetweenTraces` stay unported); `crates/copper-router/src/pipeline/batch_autorouter.rs` (**both** `autoroute_items` `:702` and `autoroute_items_with_handled` `:714` — the sort lands in the latter, which is the p7t1 differential seam, and the wrapper is untouched; ruling BP5); `crates/copper-router/src/pipeline/pass_runner.rs` (`:168`, the consumer); `crates/copper-router/src/autoroute/path/inserter.rs` (`insert_fanout_micro_neckdown`, `:488-…`, the candidate list at `:509-…`, called from `:394`); `crates/copper-board/src/rules/board_rules.rs` (`get_min_trace_half_width`, `:138` — **the reader**, not `crates/copper-board/src/board/mod.rs:1787`); `crates/copper-router/tests/{batch_autorouter.rs,inserter.rs,fanout.rs}`; `crates/copper-router/README.md` (the roster's re-stated greps); `docs/java-quirks.md`; `benchmark/reports/java-regressions-2026-09.md` (a "status: fixed in the port at Plan 9 Task 2" header line).

**Interfaces consumed:** `copper_board::BoardRules::get_min_trace_half_width` (Plan 2), `copper_board::Board::get_items` (Plan 2), `copper_router::pipeline::pass_runner` (Plan 7).
**Interfaces produced:** `copper_router::pipeline::calculate_item_distance`; `autoroute_items_with_handled` (and therefore its `autoroute_items` wrapper) returning a **sorted** list — both keep their existing signatures.
...
**Tests (named, binding).**
- `crates/copper-router/tests/batch_autorouter.rs::the_work_list_is_sorted_by_airline_distance` — a three-item board whose descending-id order and whose airline order disagree; asserts the routed order is ascending by distance. **Fails before** (the list comes back in descending id).
- `crates/copper-router/tests/batch_autorouter.rs::equal_airline_distances_keep_the_descending_id_tie_order` — the stability claim, asserted explicitly so a later sort change cannot silently reorder ties.
- `crates/copper-router/tests/airline.rs::calculate_item_distance_matches_the_java_formula` — the three ported methods against hand-computed values on a pin/via/trace triple (the Java code is `:162-213`; the expectations are hand-computed and reviewed, not read from the jar — recommendation 9).
- `crates/copper-router/tests/inserter.rs::the_micro_neckdown_fallback_never_goes_below_the_rules_minimum` — a board whose class width **equals** the rules minimum: every candidate is rejected and the connection fails. **Fails before** (a sub-minimum trace is inserted).
- `crates/copper-router/tests/inserter.rs::the_fallback_still_necks_down_when_the_class_is_above_the_minimum` — the large-board benefit is preserved; the 3/4 candidate is still taken when it clears the minimum.
- `crates/copper-router/tests/inserter.rs::the_guard_reads_the_rules_minimum_not_the_running_board_minimum` — inserts a narrow trace first, then asserts the guard does **not** ratchet down. This is the test that pins the survey's one-sentence correction.
...
**Files:** `crates/copperroute/src/commands/route.rs` (`delete_existing_output`, `_accepted`, `set_job_output`, `write_cli_output_if_available`, step 12b's pre-routing snapshot, `resolved_output_format`); `crates/copper-core/src/{job.rs,file_details.rs}` (`try_to_set_output_file`'s return, `set_data`'s re-sniff); `crates/copperroute/tests/cli_e2e.rs`; `tests/reference/cli-kicad-ecc83-json/*`, `tests/reference/cli-kicad-complex-hierarchy-json/*`.

**Interfaces consumed:** `copper_core::{RoutingJob, BoardFileDetails, FileFormat}` (Plan 8 Task 1), `copper_dsn::kicad::write` (Plan 8 Task 10).
...
**Tests (named, binding).**
- `crates/copperroute/tests/cli_e2e.rs::do_out_json_writes_the_routed_board` — the inverse of Plan 8's `do_out_json_writes_the_pre_routing_board`, which is **deleted in this commit** and whose deletion is named in the message. Asserts the JSON's wire count matches the SES's.
- `crates/copperroute/tests/cli_e2e.rs::an_unsupported_output_extension_is_refused_at_the_argument` — `-do out.dsn` exits 1 **before** the board loads, with the accepted formats in the message, and **no file is created**.
- `crates/copperroute/tests/cli_e2e.rs::a_failed_run_leaves_the_previous_result_on_disk` — write a sentinel file at the `-do` path, run with a malformed input, assert the sentinel survives byte-for-byte.
- `crates/copperroute/tests/cli_e2e.rs::an_empty_output_directory_is_not_unlinked` — the `File.delete()` directory case.
- `crates/copper-core/tests/file_details.rs::set_data_keeps_the_format_it_was_given` — the re-sniff is gone.
...
**Files:** `crates/copper-dsn/src/parser/structure.rs`, `.../autoroute_settings.rs`, `.../dsn_file.rs` (`read_integer_scope`, `skip_scope`, `read_scope`), `.../network.rs` (`insert_component`), `.../geometry.rs` (the circle bounds); `crates/copper-dsn/src/rules_reader.rs`; `crates/copper-dsn/src/lib.rs` (`BoardReadResult`); `crates/copper-dsn/src/lexer/scanner.rs` (`DsnScanner::new` — **#86's input ceiling only. `scanner.rs` is edited again at Task 22 (#84/#85: the skip/stop sets and the number grammar); the two edits are in different functions, Task 4 lands first, and each regenerates the G family for its own reason — ruling BP6, named at both ends**); `crates/copper-dsn/tests/{structure_scope.rs,scopes.rs,geometry_scopes.rs,lexer.rs,rules.rs,network.rs}`; `tests/reference/<6 G stems>/*`; `tests/reference/cli-large-outline/*`.

**Interfaces consumed:** `copper_board::{Board, CoordinateTransform}` (Plan 2), `copper_settings::RouterSettings` (Plan 4).
...
**Files:** `crates/copper-board/src/board/trace_normalize.rs`; `crates/copper-board/src/searchtree/{mod.rs,shape_search_tree.rs}` (`overlapping_tree_entries`); `crates/copper-board/src/items/mod.rs` (`get_connection_items`); `crates/copper-board/src/board/mod.rs` (`reduce_nets_of_route_items`, `assign_net_no`, `remove_from_net`); `crates/copper-dsn/src/parser/wiring.rs`; `crates/copper-board/tests/{trace_normalize.rs,board.rs}`; `crates/copper-dsn/tests/wiring.rs`; fixtures `p8t13-via-net-numbers.dsn` + its control (already committed, Plan 8 Task 13).

**Interfaces consumed:** `copper_board::searchtree::ShapeSearchTree` (Plan 2), the Plan 8 Task 13 fixtures.
...
**Tests (named, binding).**
- `crates/copper-board/tests/trace_normalize.rs::a_two_rail_four_rung_ladder_normalizes_and_terminates` — the `#[ignore]` is removed and the test asserts a **literal** final trace count, hand-computed and reviewed.
- `crates/copper-board/tests/trace_normalize.rs::overlapping_tree_entries_returns_a_fresh_collection` — the caller's collection is untouched across two calls.
- `crates/copper-board/tests/board.rs::a_cyclic_contact_graph_terminates_with_a_visited_set` — #106's half, measured **after** #71 lands so the re-measure is recorded.
- `crates/copper-board/tests/board.rs::one_visit_reduces_only_the_visited_net` — replaces `one_visit_reduces_two_nets_…`.
- `crates/copper-board/tests/board.rs::assign_net_no_replaces_the_whole_array` and `…::remove_from_net_removes_the_first_duplicate` — the three existing `assign_net_no_*` tests are re-pointed.
- `crates/copper-dsn/tests/wiring.rs::a_multi_subnet_via_carries_every_net_number` — against `p8t13-via-net-numbers.dsn`, and the control asserts the single-subnet case is unchanged.
- `crates/copperroute/tests/cli_e2e.rs::the_via_net_number_fixture_routes_instead_of_hanging` — the 1 995-byte SES, asserted by size and by re-read.
...
**Files:** `crates/copper-router/src/board_ext/routing_board_ext.rs` (`insert_forced_trace_polyline`); `crates/copper-router/src/autoroute/path/locator_any_angle.rs` (`calculate_next_trace_corners`); `crates/copper-router/src/autoroute/engine.rs` (`remove_incomplete_expansion_room`, `ExpansionDrill::calculate_expansion_rooms`); `crates/copper-router/src/autoroute/drill_page.rs` (`get_drills`); `crates/copper-router/src/autoroute/control.rs` (`init_net`); **the ten-site guard cluster, enumerated (ruling BP11 — no bare crate directories, so explicit-path staging and pairwise conflict detection both work):** `crates/copper-board/src/board/communication.rs` (#67 `host_is_old_kicad`), `crates/copper-settings/src/router_settings.rs` (#123 `get_horizontal_trace_costs` / `get_vertical_trace_costs`), `crates/copper-board/src/structure/component.rs` (#47 `Component::change_side`, #49 `Components::get`/`get_mut`), `crates/copper-board/src/library/package.rs` and `crates/copper-board/src/library/logical_part.rs` (#42 `Packages::get` / `LogicalParts::get`), `crates/copper-board/src/library/board_library.rs` (#43 `remove_via_padstack`), `crates/copper-board/src/items/drill.rs` (#52 `Pin::get_trace_exit_restrictions`), `crates/copper-geometry/src/polyline.rs` (#22 `remove_overlaps`/`from_lines`, #25 `corner_count`), `crates/copper-geometry/src/tile_shape.rs` (#24 `rotate_approx`) — **sixteen source paths in this task's Files line in total**; `crates/copper-router/tests/{drill.rs,control.rs}`; `crates/copper-board/tests/*`; `crates/copper-geometry/tests/polyline.rs`; two **new** synthetic fixtures under `crates/copper-router/tests/data/`.

**Interfaces consumed:** `copper_router::autoroute::{AutorouteEngine, ExpansionDrill, DrillPage, AutorouteControl}` (Plan 6).
...
**Tests (named, binding).** Every one of the sixteen `#[should_panic]`/`expect` reproductions in `copper-router`, `copper-board`, `copper-settings` and `copper-geometry` is **inverted in place** — the `#[should_panic]` attribute is removed and the test asserts the guarded answer:
- `crates/copper-router/tests/insert_forced.rs::a_degenerate_resample_skips_one_segment_not_the_connection` (new fixture `t6-resampled-polyline.dsn`, hand-computed expectation reviewed here).
- `crates/copper-router/tests/locator_any_angle.rs::a_null_intersection_skips_the_correction_loop` (new fixture `t6-parallel-lines.dsn`).
- `crates/copper-router/tests/drill.rs::a_virgin_engine_yields_thirteen_drills` — replaces `a_virgin_engine_yields_no_drills_at_all`; the literal **13** is the JVM-verified count.
- `crates/copper-router/tests/drill.rs::a_stopped_split_does_not_memoise_an_empty_page` — replaces `split_to_convex_stops_when_the_stop_check_trips`.
- `crates/copper-router/tests/control.rs::a_positive_unknown_net_gets_the_half_width_fallback` — replaces `a_positive_net_the_board_does_not_have_throws_like_java`.
- Ten named inversions, one per site, each keeping its existing test name with `_throws_like_java` → `_is_guarded`.
- `crates/copper-geometry/tests/polyline.rs::remove_overlaps_on_a_degenerate_array_normalises_to_a_literal` — #22's blast radius, with the hand-computed answer.
...
**Files:** `crates/copper-dsn/src/kicad/{reader.rs,dto.rs,writer.rs}`; `crates/copper-dsn/src/kicad/java_hash.rs` (`JavaStringSet`, `JavaStringMap`, `java_hash_iteration_order`, `java_string_hash` — **made DEAD here, DELETED at Task 24. Ruling BP16 settles the plan's earlier "deleted here or made dead here": this task removes the last call site and leaves the modules in place with a `// dead: killed by #280, deleted at T24` header, and Task 24's ~150-occurrence count includes them**); `crates/copper-dsn/src/kicad/npe.rs` (`JavaNpe` — **same: dead here, deleted at Task 24**); `crates/copper-dsn/tests/kicad_*.rs`; `crates/copper-core/tests/data/p8t8-kicad-read-{a,b}.txt` and `p8t10-kicad-writer.txt` (probe transcripts re-cut or retired); `tests/reference/cli-kicad-ecc83-json/*`, `tests/reference/cli-kicad-complex-hierarchy-json/*`.

**Interfaces consumed:** `copper_dsn::{BoardReadResult, ReadDiagnostic}` (Task 4's third variant — this task's diagnostics use it), `copper_board::Padstacks` (Plan 2).
...
**Tests (named, binding).**
- `crates/copper-dsn/tests/kicad_dto.rs::a_null_layer_name_is_rejected_with_the_section_named`, `…::a_null_net_name_is_rejected…`, `…::a_null_component_reference_is_rejected…`, `…::a_null_array_element_is_rejected…` — four, each asserting the diagnostic **text** contains the file, the section and the object.
- `crates/copper-dsn/tests/kicad_padstacks.rs::two_pads_of_the_same_name_and_different_layers_get_different_padstacks`, `…::a_drilled_and_an_undrilled_pad_do_not_share_attach_allowed`, `…::a_round_pad_name_carries_size_y`.
- `crates/copper-dsn/tests/kicad_padstacks.rs::an_all_null_shape_array_is_refused_naming_the_pad` — #286, and the `java_drill_item_tile_shape_count` emulation is deleted with it.
- `crates/copper-dsn/tests/kicad_packages.rs::three_components_with_unnamed_pads_yield_one_package` — replaces the three-`NONAME` reproduction.
- `crates/copper-dsn/tests/kicad_nets.rs::net_numbers_follow_declaration_order` — over `Issue649`'s thirteen pad nets, asserting **1..13 in declaration order**; the `java_hash_iteration_order` reproduction test is deleted by name.
- `crates/copper-dsn/tests/kicad_round_trip.rs::the_outline_clearance_round_trips` — #281.
...
**Files:** `crates/copper-router/src/autoroute/{tree_ext.rs,sorted_neighbours.rs,engine.rs,maze_list_element.rs,maze_search.rs,expansion_room.rs,incomplete_room.rs,drill_page.rs}`; `crates/copper-board/src/searchtree/shape_search_tree_90.rs`; `crates/copper-router/tests/{tree_ext.rs,sorted_neighbours.rs,sorted_neighbours_regimes.rs,engine_rooms.rs,maze_list_element.rs,maze_search.rs,expansion_rooms.rs,drill.rs,incomplete_room.rs}`; a **new 90-degree fixture** under `crates/copper-router/tests/data/`; `scripts/differential/run.sh` (`p6t2` retires here).

**Interfaces consumed:** `copper_board::searchtree::{ShapeSearchTree, ShapeSearchTree90Degree, ShapeSearchTree45Degree}` (Plan 2), `copper_router::autoroute::AutorouteEngine` (Plan 6).
...
**Files:** `crates/copper-router/src/pipeline/{batch_loop.rs,optimizer.rs,run.rs,batch_autorouter.rs,pass_runner.rs,stop.rs}`; `crates/copper-core/src/manifest.rs`; `crates/copperroute/src/commands/route.rs`; `crates/copper-router/tests/{optimizer.rs,batch_loop.rs,pass_runner.rs,stop_and_progress.rs}`.

**Interfaces consumed:** `copper_router::pipeline::{RouterStop, StopRequestState, BatchLoopResult, OptimizerResult, PipelineResult}` (Plans 7–8).
...
**Tests (named, binding).**
- `crates/copper-router/tests/batch_loop.rs::a_normal_finish_reports_finished` — replaces `a_normal_finish_reports_cancelled_not_finished`; asserts each of `BatchLoopExit`'s five values on a constructed exit.
- `crates/copper-router/tests/optimizer.rs::an_auto_router_only_stop_still_runs_the_optimizer` — replaces `an_auto_router_only_stop_leaves_every_item_rejected`; asserts ≥ 1 `improved=true` on a board the router leaves improvable, and that the board shape **changes**.
- `crates/copper-router/tests/optimizer.rs::the_stage_scoped_stop_does_not_leak_into_the_router` — the three-state distinction preserved.
- `crates/copper-router/tests/stop_and_progress.rs::max_items_optimises_like_max_passes` — #202, observable only after #227.
- `crates/copper-router/tests/pass_runner.rs::a_two_net_item_is_routed_once_per_qualifying_net` — replaces `a_two_item_is_routed_four_times`; the literal is **2**, not 4.
- `crates/copper-router/tests/batch_loop.rs::the_stagnation_counter_resets_only_from_pass_eight` — asserts the reset fires at pass 8+, not 1-7.
- `crates/copper-core/tests/manifest.rs::the_two_stages_report_their_own_pass_counts` — a three-pass router stage and a one-pass optimizer stage read back as 3 and 1.
- `crates/copper-router/tests/optimizer.rs::the_improvement_flag_is_a_bool` — #228.
...
port at `crates/copper-router/src/autoroute/maze/search.rs::reduce_trace_shapes_at_tie_pins`). This
...
**Files:** `crates/copper-board/src/board/{shape_trace_entries.rs,trace_normalize.rs,clearance_override.rs,mod.rs}`; `crates/copper-router/src/{shove/trace_shover.rs,autoroute/maze_expand.rs,pipeline/run.rs}`; `crates/copper-board/src/{layer_structure.rs,items/conduction_area.rs}`; `crates/copper-router/src/board_ext/drill_item_mover.rs`; `crates/copper-router/tests/{fixtures.rs,maze_expand.rs,board_ext.rs}`; `crates/copper-board/tests/*`; a **new fixture with a signal-layer pour and a foreign-net trace**.

**Interfaces consumed:** `copper_board::Board::{change_conduction_is_obstacle, apply_copper_to_edge_clearance_override}` (Plans 2, 7), `copper_router::pipeline::prepare_board` (Plan 7).
...
**Tests (named, binding).**
- `crates/copper-router/tests/fixtures.rs::the_copper_to_edge_override_is_continuous` — `=500`, `=500.000001` and `=0` all behave the same way; the explicit-class case keys on **provenance**, asserted by a source that supplies the value and one that does not.
- `crates/copper-board/tests/clearance_override.rs::an_explicitly_supplied_default_is_applied` — the guard's inversion, with the **15 254 / 14 644** byte figures re-measured and pasted.
- `crates/copper-board/tests/shape_trace_entries.rs::a_component_keepout_blocks_a_via` — #65.
- `crates/copper-board/tests/shape_trace_entries.rs::a_contact_in_another_clearance_class_blocks_the_shove` — #69, two contacting traces in different classes.
- `crates/copper-board/tests/trace_normalize.rs::a_trace_is_not_cut_inside_a_pin_pad` — #72.
- `crates/copper-router/tests/shove.rs::a_failed_via_shove_names_its_own_obstacle` and `…::the_failing_obstacle_is_cleared_on_entry` — #174.
- `crates/copper-router/tests/maze_expand.rs::a_start_pin_neckdown_does_not_shrink_a_pass_through_trace` — #179.
- `crates/copper-board/tests/conduction.rs::a_signal_layer_pour_obstructs_a_foreign_net` — #50, on the new fixture, with the hand-computed expectation reviewed here.
- `crates/copper-board/tests/layer_structure.rs::an_out_of_range_signal_layer_is_none`, `…::a_zero_net_conduction_area_copies`, `crates/copper-router/tests/board_ext.rs::drill_item_mover_check_does_not_mutate_its_caller` — #35/#46/#175.
...
**Files:** `crates/copper-geometry/src/{rational_point.rs,int_box.rs,int_octagon.rs,polyline.rs,polygon_path.rs,simplex.rs,circle.rs,line_segment.rs,tile_shape.rs}`; `crates/copper-board/src/{board/outline.rs,items/{obstacle_area.rs,component_outline.rs},component.rs}`; `crates/copper-router/src/{shove/trace_shover.rs,autoroute/path/inserter.rs,tightener/any_angle.rs}`; `crates/copper-geometry/tests/*`; `crates/copper-board/tests/areas_and_outlines.rs`; `crates/copper-router/tests/{forced_via.rs,inserter.rs,tightener.rs}`; a **new fixture with per-layer trace widths** (also #128's, Task 21).

**Interfaces consumed:** `copper_geometry::{FloatLine, FloatPoint, Polyline, Simplex, IntBox, IntOctagon}` (Plan 1), `copper_board::BoardOutline` (Plan 2).
...
**Tests (named, binding).**
- `crates/copper-geometry/tests/rational_point.rs::perpendicular_projection_agrees_with_int_point` — #5, over a generated line set not through the origin.
- `crates/copper-geometry/tests/int_box.rs::border_line_index_is_geometric` and `crates/copper-geometry/tests/int_octagon.rs::border_line_index_is_geometric`; `crates/copper-router/tests/shove.rs::a_cut_that_removed_nothing_does_not_set_cut_off` — #7 + #68.
- `crates/copper-router/tests/forced_via.rs::an_unmarked_changed_area_still_normalises` — replaces the JVM-pinned "three traces" literal with **one**.
- `crates/copper-router/tests/inserter.rs::connect_to_trace_looks_the_trace_up_by_id` — #186; the existing test already fails today on the id lookup and the post-loop snapshot, **which is the fixed behaviour**.
- `crates/copper-router/tests/inserter.rs::the_stub_takes_the_width_of_the_layer_it_lands_on` — #187, on the new per-layer-width fixture.
- `crates/copper-board/tests/areas_and_outlines.rs::the_outline_moves_turns_rotates_and_mirrors` — four assertions, replacing the four pinning tests.
- `crates/copper-router/tests/tightener.rs::the_any_angle_bend_branch_is_reachable` — #183.
- `crates/copper-board/tests/component.rs::a_back_side_rotation_keeps_the_outline_and_the_pads_together` — #48 + #57, all three sites in one assertion.
- `crates/copper-geometry/tests/{shape.rs,line_segment.rs}::nearest_corner_at_distance_zero_is_nearest`, `…::the_insertion_shift_copies_the_right_element`, `…::stair_approximation_45_uses_the_y_function`.
...
**Files:** `crates/copper-geometry/src/{polygon_shape.rs,circle.rs}`; `crates/copper-geometry/tests/{polygon_shape.rs,circle.rs,stubs.rs}` (`stubs_match_the_java_stubs` is **deleted** here and its deletion named).

**Interfaces consumed:** `copper_geometry::{ConvexShape, TileShape, FloatPoint}` (Plan 1), `PolygonShape::area` (Task 11's #26 half).
...
**Tests (named, binding).** A new `crates/copper-geometry/tests/polygon_geometry.rs`, every expectation hand-computed and reviewed **in this task's report**:
...
- `crates/copper-geometry/tests/stubs.rs` is **deleted** — the stubs are gone, and a test asserting they match Java's stubs is a test asserting the port is broken.
...
**Files:** `crates/copper-board/src/datastructures/delaunay.rs` (the in-circle predicate and the bounding triangle, and its **private `JavaRandom` copy** at `:81` — **Task 19 edits the same file's `validate()` (#81) six tasks later, and Task 24 deletes the PRNG; Task 13 lands first and touches neither, ruling BP6**); `crates/copper-geometry/src/float_point.rs` (`circle_center` — #9's horizontal case, deferred here from Task 11); `crates/copper-drc/src/net_incompletes.rs`; `crates/copper-drc/src/airline.rs`; `crates/copper-router/src/pipeline/board_history.rs`; `crates/copper-router/src/score/statistics.rs` (**the fanout block only** — **Task 19's #195/#196 rewrite the length-breakdown and bounding-box blocks of this same file, and Task 20's #291 the drill count; dispatch order is 13 → 19 → 20, so each later task re-reads the file rather than assuming Task 13's line numbers, ruling BP6**); `crates/copper-router/tests/{board_history.rs,score.rs}`; `crates/copper-drc/tests/net_incompletes.rs`; `tests/reference/drc-*/AIRLINE_BUDGETS`; a **new multi-net SMD pin fixture**; `scripts/differential/run.sh` (`p2t13`'s Delaunay-order modes retire here).

**Interfaces consumed:** `copper_geometry::Point`'s `BigInt` machinery (the exact `sideOf` predicate already in the tree), `copper_board::datastructures::PlanarDelaunayTriangulation` (Plan 2).
...
**Tests (named, binding).**
- `crates/copper-board/tests/delaunay.rs::a_square_pin_grid_keeps_every_edge` — replaces `square_pins_javas_four_edges`; asserts **5/5, 56/56, 85/85** at 2×2, 5×5 and 6×6.
- `crates/copper-board/tests/delaunay.rs::a_seven_by_seven_grid_straddling_the_origin_leaves_no_witness_pad_edgeless` — the 0.5 % case, over 2 000 dense draws, **0 failures**.
- `crates/copper-board/tests/delaunay.rs::the_edge_set_is_independent_of_insertion_order` — the property that retires the private PRNG; asserts two shuffles give the same edge set.
- `crates/copper-geometry/tests/float_point.rs::circle_center_of_a_horizontal_input_is_finite` — #9.
- `crates/copper-drc/tests/net_incompletes.rs::two_airlines_between_different_items_at_one_location_are_both_kept` and `…::a_nan_length_does_not_compare_equal` — #147; the three existing unit tests are re-pointed.
- `crates/copper-router/tests/board_history.rs::an_empty_history_can_trigger_a_restore`, `…::a_negatively_scored_history_restores`, `…::get_rank_is_score_order_before_the_first_restore` — the three existing tests, inverted.
- `crates/copper-router/tests/score.rs::a_pin_unconnected_on_its_second_net_needs_an_escape` — #194, on the new fixture.
- `crates/copper-drc/tests/net_incompletes.rs::airline_is_not_comparable` — #148.
...
**Files:** `crates/copper-board/src/items/{drill_item.rs,polyline_trace.rs,component_outline.rs}`; `crates/copper-board/src/board/mod.rs` (`set_flip_style_rotate_first`); `crates/copper-board/src/searchtree/shape_tree.rs` (`insert`); `crates/copper-board/src/rules/{via_rule.rs,via_info.rs}`; `crates/copper-router/src/board_ext/routing_board_ext.rs` (`combined_fallback_via_rule`); `crates/copper-board/tests/*`; `crates/copper-router/tests/fanout_order.rs`.

**Interfaces consumed:** `copper_board::{DrillItem, PolylineTrace, ShapeTree, ViaRule, ViaInfo}` (Plan 2).
**Interfaces produced:** `ViaInfo` with **value equality** on (name, padstack, clearance class, attach flag) — which **deletes one of the two readers of `copper_geometry::Line`'s identity token** (the other is #74, Task 18).
...
**Tests (named, binding).**
- `crates/copper-board/tests/drill_item.rs::a_via_that_changes_side_recomputes_its_minimum_width` — #51, with the neckdown consequence asserted (the width the router would neck down to changes).
- `crates/copper-board/tests/polyline_trace.rs::a_rotated_trace_clears_its_tree_shapes` — #60.
- `crates/copper-board/tests/component_outline.rs::clear_derived_data_chains_to_super` — #56.
- `crates/copper-board/tests/board.rs::setting_flip_style_clears_the_caches` — #58.
- `crates/copper-board/tests/shape_tree.rs::a_zero_shape_insert_still_clears_the_entry_array` — #41.
- `crates/copper-router/tests/fanout_order.rs::a_redeclared_via_rule_is_deduplicated_by_value` — #218; asserts `viaInfos.len()` and the `viaRadii` maximum both unchanged by the redeclaration.
- `crates/copper-board/tests/via_info.rs::via_info_equality_is_by_value` — the four fields, and the identity serial's removal.
...
**Files:** `crates/copper-router/src/board_ext/routing_board_ext.rs` (`fanout`); `crates/copper-router/src/pipeline/fanout.rs` (the loop state, the gates, the comparator); `crates/copper-settings/src/router_settings.rs` (`pin_sorting_order` validation); `crates/copper-router/tests/{fanout.rs,fanout_order.rs}`.

**Interfaces consumed:** `copper_board::Board::structural_hash` (Plan 7, widened), `copper_router::pipeline::FanoutLoopState` (Plan 7).
...
**Tests (named, binding).**
- `crates/copper-router/tests/fanout_order.rs::a_successful_retry_does_not_destroy_the_first_attempts_rips` — #221; a constructed two-attempt case where the first attempt rips a connection the second does not need.
- `crates/copper-router/tests/fanout.rs::two_passes_escaping_different_pins_are_not_oscillation` — #222, three pins (the existing test's shape), asserting the loop continues.
- `crates/copper-router/tests/fanout.rs::the_oscillation_break_fires_on_the_named_repeat_count` — the constant's rename and the message.
- `crates/copper-router/tests/fanout_order.rs::a_pin_alone_on_its_net_sorts_explicitly` — #219; asserts the ordering does not depend on `f64::MAX`.
- `crates/copper-settings/tests/router_settings.rs::an_unrecognised_pin_sorting_order_is_refused` — #220.
- `crates/copper-router/tests/fanout.rs::a_pin_whose_net_does_not_resolve_cannot_use_vias` — #223.
...
**Files:** `crates/copper-router/src/optimize/via_optimizer.rs`; `crates/copper-router/src/optimize/connection_to_pin.rs`; `crates/copper-router/src/autoroute/drill_page_array.rs`; `crates/copper-router/tests/{via_optimizer.rs,connection_to_pin.rs,maze_drills.rs}`.

**Interfaces consumed:** `copper_router::optimize::ViaOptimizer` (Plan 7), `copper_board::Board::get_normal_contacts` (Plan 2).
...
**Tests (named, binding).**
- `crates/copper-router/tests/via_optimizer.rs::a_via_on_the_last_corner_is_matched_to_the_last_corner` — #206, with the 37 821-unit window case asserted to be **rejected** now.
- `crates/copper-router/tests/via_optimizer.rs::reposition_via_respects_the_boards_angle_restriction` — #207, both overloads.
- `crates/copper-router/tests/connection_to_pin.rs::a_zero_pin_edge_to_turn_distance_is_refused` — #205, all three guards.
- `crates/copper-router/tests/maze_drills.rs::a_shape_ending_on_a_page_boundary_reaches_the_next_page` — #192, with the decision named in the test's doc comment.
...
**Files:** `crates/copper-router/src/autoroute/engine.rs` (the three HEAD-only guards, instrumented); `crates/copper-router/src/autoroute/instrument.rs` (**new**, `#[cfg(feature = …)]`-free — a plain module gated by a `RouterCounters` field so it costs nothing when off); `docs/plan-9-prep/stale-index-report.md` (**new**, the deliverable); `crates/copper-router/tests/stale_index.rs`.

**Interfaces consumed:** `copper_router::pipeline::RouterCounters` (Plan 7).
...
**Tests (named, binding).**
- `crates/copper-router/tests/stale_index.rs::the_three_guards_are_counted_on_every_router_stem` — asserts the counters are non-zero on at least one stem (the survey's claim that the corpus reaches all three is **verified, not assumed**; if a guard never fires, that is the finding and the report says so).
- `crates/copper-router/tests/stale_index.rs::instrumentation_changes_no_board_byte` — the Plan 7 ruling 11 shape, re-run: with the counters on and off, `batch.ses` is byte-identical on all eight stems. **This is the gate that makes the task safe to land.**
...
**Last on purpose.** Every row here is a deliberate change of policy or of an arbitrary order, and each needs the rest of the catalogue's improvements underneath it before its A/B means anything. **Every row is individually abandonable**, and abandoning one is a recorded outcome, not a failure. **Recommendation 11 is adopted for #172 and #235:** implement them as **settings with the CURRENT behaviour as the default**, then decide each default from its own A/B. The user explicitly declined #182; its disabled stub is deleted and no setting or fixture is added.

**Files:** `crates/copper-router/src/autoroute/control.rs` (`rebuild_via_info`); `crates/copper-router/src/pipeline/failure_log.rs`; `crates/copper-dsn/src/parser/network.rs` (`create_via_rule`); `crates/copper-router/src/tightener/tightener_45.rs`; `crates/copper-board/src/{ids.rs,items/mod.rs,board/mod.rs}` (`items_in_board_order`, the `.rev()`s); `crates/copper-geometry/src/line.rs` (the identity token); `crates/copper-settings/src/router_settings.rs` (two new settings).

**Interfaces consumed:** everything above.
**Interfaces produced:** `router.smd_via_relaxation` (default **on**, i.e. today's behaviour), `router.failure_give_up_threshold` (default **disabled**, i.e. today's behaviour).
...
**The abandonment rule, stated once.** Each of the five remaining rows is committed **separately**, measured at G2 **separately**, and carried into M3. **A row whose G2 shows a hard loss is reverted in the same task**, its register row is set back to `pinned` with the measurement recorded, and Task 25's report names it as *measured and declined*. **#44+#63+#74 is the most likely decline** and the plan says so in advance: if the ordering flip does not pay at M3, it is reverted and `Line`'s identity token survives with #74 named as its sole remaining reader (the Global Constraint on static mutable state records that outcome).

**Tests (named, binding).**
- `crates/copper-router/tests/control.rs::the_smd_relaxation_is_a_setting` — #172; asserts `minNormalViaCost` is **4000** with the relaxation off and **400** with it on, and that `attachSmdAllowed` and the `ViaMask` **agree** either way.
- `crates/copper-router/tests/failure_log.rs::an_item_that_fails_fifty_times_is_skipped_when_enabled` — #235; and `…::the_give_up_policy_is_disabled_by_default`.
- `crates/copper-dsn/tests/network.rs::via_at_smd_reaches_the_net_class_use_via_rule` — #104.
- `crates/copper-router/tests/tightener.rs::the_start_corner_contact_is_chosen_by_geometry` — #210; the `{496, 495, 494}` case, asserting **496** (the nearest) rather than 494 (the id-order last), with the geometric rule named in the test.
- `crates/copper-board/tests/board.rs::the_item_walk_is_ascending` and `crates/copper-geometry/tests/line.rs::polyline_change_compares_lines_by_value` — #44/#63/#74, **only if the flip is kept**; if reverted, these tests are reverted with it.

**Evidence / acceptance.** G1 green after each commit. **#172, #235 and #104 move B/C on the stems they touch**; **#210 moves R/B on 45° stems**; **#44+#63+#74 moves EVERYTHING** (survey's Δrefs is literally "everything") and is regenerated in its own commit so the diff is attributable. G2 per row, separately.

**MILESTONE M3** — the G3 block **exactly as written**, with `--run-id plan9-m3`, compared `--runs java-278fe14,plan9-m3` into `benchmark/reports/plan9-m3-vs-java-278fe14.{json,md}` (`git add -f`), `remote-run.sh`'s output to `benchmark/baselines/plan9-m3.log`, and the workbench binary's sha pasted against `meta.json`'s. It is the **whole branch against the frozen `java-278fe14` run** — **`java-current` is not re-run here or anywhere in this plan** (ruling BN; an earlier draft's "against `java-current`" read as a live jar re-run and was wrong). **Acceptance:** `overall.verdict` **`better`**, zero hard losses, the D3-small numbers at or above M1's, and the **corpus-median `cpu_s` ratio against M2** inside BO's 20 % band or escalated with a keep/rework ruling. M3 is also where each of the five remaining rows is confirmed or reverted. Read into Task 25.

**Steps:**
- [x] Decision before Task 18: #182's disabled stub deleted; no setting or fixture.
...
- [ ] Commit 7: **M3** — the workbench build + sha check, `remote-run.sh … --run-id plan9-m3`, the frozen-view compare; `git add -f benchmark/reports/plan9-m3-vs-java-278fe14.{json,md}`; the M2→M3 cpu ratio; each of the five remaining rows confirmed or reverted.
- [ ] Register: each row → `fixed: T18` or back to `pinned` with "measured and declined at M3" and the number.

**Commit message:** `feat(router): the measured policy switches — #172, #235, #104, #210 and the #44+#63+#74 ordering flip; M3 bench, each row confirmed or declined`
...
**Files:** `crates/copper-drc/src/{report/mod.rs,report/json.rs,unconnected.rs,violations.rs}`; `crates/copper-board/src/items/mod.rs` (`smallest_clearance` — the field is **deleted**); `crates/copper-board/src/board/mod.rs` (`get_all_clearance_violations`); `crates/copper-router/src/score/statistics.rs` (the length breakdown, the bounding box — **Task 13 edited this file's fanout block for #194; re-read it, do not rely on the line numbers this plan quotes, ruling BP6**); `crates/copper-dsn/src/writer/{ses_writer.rs,rules_writer.rs}`; `crates/copper-board/src/datastructures/delaunay.rs` (`validate` — **Task 13 rewrote this file's in-circle predicate and bounding triangle; `validate()` is untouched by it and Task 24 deletes the private PRNG afterwards, ruling BP6**); `crates/copper-router/src/optimize/item_route_result.rs`; `crates/copperroute/src/commands/drc.rs`; `crates/copper-drc/tests/{report.rs,unconnected.rs}`; `crates/copper-board/tests/{clearance_violations.rs,snapshot.rs}`; `tests/reference/drc-*/**` (all 8).

**Interfaces consumed:** `copper_drc::{DesignRulesChecker, DrcReportOptions}` (Plan 5), `copper_core::PARITY_VERSION` (Plan 8).
...
**Tests (named, binding).**
- `crates/copper-drc/tests/report.rs::an_smd_pad_is_not_a_hole` — replaces `smd_pins_are_classified_as_holes`; asserts the BBD Mars-64 **64/12 split** moves to the corrected split and that **the total is unchanged**.
- `crates/copper-drc/tests/unconnected.rs::a_dangling_trace_is_reported_once` (four existing tests re-pointed) and `…::the_via_phase_dedups_too`; `…::natural_tone_preamp_reports_a_stable_count` — the XDIFF's retirement.
- `crates/copper-board/tests/clearance_violations.rs::smallest_clearance_is_a_fold_over_the_current_list` — #153; asserts a second call after an improvement reports the **improved** number.
- `crates/copperroute/tests/cli_e2e.rs::fail_on_violations_exits_one` and `…::drc_without_the_flag_still_exits_zero` — #271.
- `crates/copperroute/tests/cli_e2e.rs::the_quality_score_uses_the_rules_file_weights` — #272; the `obligation:` marker at `resolve.rs` is struck in the same commit.
- `crates/copperroute/tests/cli_e2e.rs::the_drc_unit_flag_reaches_the_report` — #151, all four previously-dead arms.
- `crates/copper-drc/tests/report.rs::only_the_kicad_flavour_is_written` — #154; the `FreeroutingHead` **reader** test stays.
- `crates/copper-router/tests/score.rs::the_three_lengths_sum_to_the_total` and `…::the_bounding_box_holds_extents_not_a_corner` — #195/#196, with the **121 606.75 / 130 610.65** pair as the fail-before evidence.
- `crates/copper-dsn/tests/parity_ses.rs::a_conduction_area_hole_is_written_as_integers` — #110, on `p8t13-conduction-area.dsn`.
- `crates/copper-dsn/tests/rules_round_trip.rs::a_design_name_with_a_space_round_trips` — #111.
- `crates/copper-board/tests/delaunay.rs::validate_fails_on_an_inconsistent_inner_node` — replaces `validate_is_vacuous_on_an_inner_node`; `crates/copper-router/tests/optimizer.rs::improvement_percentage_does_not_truncate_the_via_term` — replaces its pinning twin; `crates/copper-board/tests/snapshot.rs`'s hash comment corrected.
...
**Files:** `crates/copper-core/src/{manifest.rs,stats_from_bytes.rs,stats_json.rs}`; `crates/copper-router/src/score/statistics.rs` (**#291's drill count — Tasks 13 and 19 edited other blocks of this same file first; the "`batch.ses` byte-identical across this whole task" assertion covers this edit too, ruling BP6**); `crates/copperroute/src/commands/route.rs`; `crates/copper-core/tests/{manifest.rs,stats.rs}`; `tests/reference/cli-*/manifest.json` (13); `scripts/differential/run.sh` (`p8t2probe` retires here).

**Interfaces consumed:** `copper_router::score::BoardStatistics` (Plan 7/8), `copper_core::RoutingResultManifest` (Plan 8 Task 4).
...
| **#251** | Port divergence: `BoardStatistics.host` is a `String`, so **the port cannot tell Java's `null` from `""`.** One input reaches it, and it is an XDIFF row on both sides. Ruling **BD** already measured the blast radius: **1 declaration, 3 writes, 6 reads**, ~4 lines of the `p8t2` transcript. | `Option<String>`. **The scan-ruling R3/R15 additive-only constraint on `copper-router` was a Plan 8 constraint and does not survive into Plan 9** — this is the row that proves it. **Land with #248.** |
...
**Tests (named, binding).**
- `crates/copper-core/tests/stats.rs::layer_rule_is_not_counted_as_a_layer` (and three siblings for `(net`, `(via`, `(class`) — #247; the eleven-clause DSN's **2, 3, 2, 2** becomes **1, 1, 1, 1**.
- `crates/copper-core/tests/stats.rs::a_ses_layer_whose_path_ends_the_file_is_counted` — #247's SES branch.
- `crates/copper-core/tests/stats.rs::a_real_specctra_file_yields_a_host` — #248; asserts a **non-null** host on all fourteen corpus DSN rows, and `…::snake_case_host_keywords_are_accepted`.
- `crates/copper-core/tests/stats.rs::the_host_fallback_is_reachable` — #249.
- `crates/copper-core/tests/manifest.rs::all_three_phases_are_written` and `…::the_autorouter_duration_is_not_the_whole_job` — #254.
- `crates/copper-core/tests/manifest.rs::an_unreadable_input_is_distinguishable_from_a_missing_one` — #255; `…::the_resource_usage_object_is_gone` — #256, plus `normalize_manifest`'s special case deleted.
- `crates/copper-core/tests/stats.rs::host_is_optional` — #251.
- `crates/copper-core/tests/manifest.rs::drill_item_count_counts_pins_and_vias` — #291; the literal for a 400-pin/90-via board is **490**.

**Evidence / acceptance.** G1 green. **All 13 C-stem manifests regenerate**, once. `goldens moved:` carries one line per field: *"#247 — layer/net/via/class counts fall to their true values; #248 — `host` becomes non-null on 14/14 DSN rows; #254 — `phases.fanout`/`phases.optimizer` are populated and `autorouter.duration_seconds` is renamed; #256 — `resource_usage` is removed; #291 — `drill_item_count` becomes non-zero."* **`p8t2probe` RETIRES** (BL7): final MATCH count recorded (289 lines today), the pair deleted, the Rust half's table kept as `crates/copper-core/tests/stats.rs` literals **re-derived for the fixed behaviour, not copied from the jar**. `p8t2 e2e` re-golden. G2 unaffected (no routed board changes) — and that is asserted: **`batch.ses` must be byte-identical across this whole task**.
...
**Files:** `crates/copper-settings/src/{merge.rs,field_path.rs,router_settings.rs,dsn_file_settings.rs,resolve.rs,scoring.rs}`; `crates/copper-drc/src/lib.rs` (#155's `DrcSettings`, re-introduced or deleted); `crates/copper-core/src/manifest.rs` (`from_job`'s guard); `crates/copper-settings/tests/{copy_fields.rs,field_path.rs,router_settings.rs}`; the **per-layer-trace-width fixture** from Task 11; `scripts/differential/run.sh` (`p4t1` retires here).

**Interfaces consumed:** the per-layer-width fixture (Task 11), `copper_settings::SettingsMerger` (Plan 4).
...
**Tests (named, binding).**
- `crates/copper-settings/tests/copy_fields.rs::a_rules_file_per_layer_trace_cost_survives_the_merge` — #128, on the per-layer-width fixture; the same test at priorities 40, 55 and 60.
- `crates/copper-settings/tests/copy_fields.rs::a_merged_settings_keeps_its_costs_and_its_flag` and `…::an_unchanged_layer_count_does_not_wipe_the_costs` — #126 + #127.
- `crates/copper-settings/tests/field_path.rs::a_layer_array_of_the_wrong_arity_is_refused` — #119.
- `crates/copper-settings/tests/copy_fields.rs::an_explicit_false_merges` and `…::a_set_field_merges` — #115 + #116.
- `crates/copper-drc/tests/report.rs::include_warnings_false_filters_the_warnings` — #155.
- `crates/copper-settings/tests/field_path.rs::a_private_field_is_not_reachable_from_a_property_path`, `…::a_static_final_field_is_not_reachable`, `…::a_hyphen_is_not_a_separator` — #121 + #118.
- `crates/copper-settings/tests/router_settings.rs::validate_is_idempotent` and `…::max_passes_zero_means_unlimited_after_any_number_of_merges` — #140.
- `crates/copper-settings/tests/rules.rs::a_rules_file_is_parsed_once_against_the_board` — #142; asserts the **13 disagreeing rows of 84** collapse to 0.
- `crates/copper-settings/tests/router_settings.rs::{max_threads_zero_is_refused, validate_does_not_unbox_a_null, clone_carries_the_result_json_path, the_two_fanout_accessors_agree}` — #124/#125/#114/#139.
- `crates/copper-core/tests/manifest.rs::from_job_needs_no_weight_guard` — #258; the `expect` with Java's message is deleted by name.

**Evidence / acceptance.** G1 green. Most rows are **U-only**; **#128 and #142 move B/C on boards with a rules file**, and #128's per-layer trace costs **steer direction preference**, so its per-stem effect is named. `goldens moved:` names #128 and #142. **`p4t1`'s 64-case settings matrix RETIRES** (BL7): final MATCH count recorded (5 728 lines today), the pair deleted, and **the 64 cases are re-derived as `copper-settings` unit tests against the FIXED semantics** — not copied from the jar, which is now wrong about ten of them. G2: incompletes not up; the rules-carrying stems' scores reported.
...
**Files:** `crates/copperroute/src/{cli.rs,legacy.rs,logging.rs,main.rs}`; `crates/copper-settings/src/sources/cli.rs` (`convert_value`, the empty-value rule); `crates/copper-core/src/{file_details.rs,job.rs}` (`set_filename`, `change_file_extension`); `crates/copper-core/src/load.rs` (`load_board_if_needed`); `crates/copper-settings/src/resolve.rs` (`resolve_scheduler_rules_path`); `crates/copper-dsn/src/lexer/{scanner.rs,number.rs}` (**#84/#85: the skip/stop sets and the number grammar. `scanner.rs` was already edited at Task 4 (#86 deleted the 16 MiB input ceiling) — different function, eighteen tasks earlier, already regenerated; re-read the file before editing and do not re-introduce the ceiling — ruling BP6, named at both ends**); `crates/copperroute/tests/cli_e2e.rs`; `docs/cli-legacy-flags.md`; `scripts/differential/run.sh`, `scripts/differential/sweep-p8t5.sh` (**both retire here**), `scripts/differential/matrix/p8t5-argv.tsv`.

**Interfaces consumed:** `copper_settings::CliSettings` (Plan 4), `copper_core::BoardFileDetails` (Plan 8 Task 1).
...
**Tests (named, binding).**
- `crates/copperroute/tests/cli_e2e.rs::an_unknown_native_flag_fails` and `…::a_missing_value_fails` — #259.
- `crates/copperroute/tests/cli_e2e.rs::a_prefix_matched_legacy_flag_warns_and_names_the_spelling` — the ramp; `-decoy a.dsn` still works **and** warns.
- `crates/copperroute/tests/cli_e2e.rs::argv_is_parsed_once_and_the_log_level_agrees` — #262.
- `crates/copper-settings/tests/cli.rs::{yes_is_not_false, on_is_not_false, ture_is_an_error, a_trailing_equals_is_refused}` — #120 + #136.
- `crates/copperroute/tests/cli_e2e.rs::the_five_legacy_optimizer_flags_reach_the_live_path` — #131-#135, one assertion per flag.
- `crates/copperroute/tests/cli_e2e.rs::{a_bare_drc_checks_with_the_default_report_path, a_session_file_at_de_is_refused_at_the_argument, a_missing_dr_path_falls_through_to_the_adjacent_rules}` — #263/#274/#269; and `…::both_modes_find_the_same_adjacent_rules` — #273's asymmetry.
- `crates/copper-core/tests/file_details.rs::{a_file_at_the_root_reports_an_absolute_path, a_posix_directory_with_a_backslash_survives, a_bare_filename_changes_extension}` — #246 + #242; plus `…::the_derived_default_output_rule_is_explicit`.
- `crates/copper-dsn/tests/lexer.rs::{a_tab_is_whitespace_and_a_backspace_is_not, one_number_grammar, a_dsn_freerouting_wrote_is_recognised}` — #84/#85/#87; and every consumer of `dsn_file_generated_by_host` re-checked by a named grep pasted into the commit.
- `crates/copperroute/tests/cli_e2e.rs::{ll_garbage_is_refused, brd_at_de_fails_at_the_argument, the_help_documents_every_form, dash_v_exists}` — #260/#130/#137.

**Evidence / acceptance.** G1 green. **`p8t5` and `sweep-p8t5.sh` RETIRE** (BL7) — they exist to pin the bug-for-bug behaviour this task deletes. Final counts recorded (`p8t5` MATCH, 2 124 lines; the sweep, 87 rows / 87 MATCH), the pair and the matrix deleted, and **the argv matrix is re-derived as `cli_e2e.rs` cases against the FIXED behaviour** — 87 rows in, a smaller, honest set out, and the mapping is written into `docs/cli-legacy-flags.md`. **`p8t1probe` RETIRES** with #246+#242 (its `setFilename`/`changeFileExtension` tables are exactly what this task changes; final count 162 lines). **G moves via #84/#85** — the lexer pair changes what a DSN token is, so the 6 `fixtures.txt` rows and the `copper-dsn` round-trip goldens re-cut, and `sweep-p3t15.sh` is re-run over the 106-fixture corpus with any newly-disagreeing fixture named. `goldens moved:` names #84/#85. G2 unaffected except where #84/#85 changes a parse — asserted stem by stem.
...
**Files:** `crates/copper-geometry/src/java_compat.rs` and its ~110 call sites across all eight crates; `crates/copper-board/src/rules/*` (the name-matching sites, done **under a directed test**); `crates/copper-geometry/src/rational_point.rs` (`java_big_integer_hash_code`, after the read audit); every crate README's shim paragraph.
...
| `java_to_upper` / `java_to_lower` (+ `_unit` variants) | 24 / 13 | `to_uppercase` / `to_lowercase`; ASCII sites → `eq_ignore_ascii_case` | Agree with Java on everything the corpus contains (including `ß`→`SS`). **CAUTION: the clearance-class and net-name comparisons in `copper-board/src/rules` are name matching on USER DATA and a non-ASCII net name exists in the tests (`GND_é中`, quirk #290) — convert those with a directed test, not in bulk.** |
...
**Tests (named, binding).**
- `crates/copper-board/tests/rules.rs::a_non_ascii_net_name_still_matches_its_clearance_class` — the `GND_é中` case (quirk #290), the **one** directed test the caution above demands, written **before** the `copper-board/src/rules` conversion.
- `crates/copper-geometry/tests/rational_point.rs::the_big_integer_hash_has_no_observable_reader` — the read audit, expressed as a test that greps the workspace and asserts the only uses are `HashMap`/`HashSet` keys.
...
- [ ] Commit 4: `java_to_upper`/`java_to_lower`, with `copper-board/src/rules` done last and separately verified.
...
**Files:** `crates/copper-router/src/collections/java_tree_set.rs` (deleted); `crates/copper-board/src/datastructures/delaunay.rs` (the private `JavaRandom` copy at `:81` and its `shuffle`); `crates/copper-dsn/src/kicad/{java_hash.rs,npe.rs}` (deleted); `crates/copper-settings/src/java_compat.rs` (`java_clone`, `java_number_format_parse`, `java_parse_bool`, `java_integer_decode`, `java_split*`); the ten one-off reproductions across four crates; `crates/copper-geometry/src/random.rs` (`JavaRandom` → `SeededLcg`); `crates/copper-core/tests/shims.rs` (**new** — the keep-list gate below lives here); every affected README.
...
**Also here: BL3's rename.** `copper-geometry`'s public `JavaRandom` (59 sites, 11 files) becomes **`SeededLcg`** — **implementation unchanged**, only the claim that it matches the JVM dropped. `PolygonShape.splitToConvex` picks its concavity-scan start from it, so shape division — and therefore every routed board — is reproducible **only if the generator is**; the bit-specified LCG satisfies BL3's cross-version and cross-platform requirement already. **Quirk #30's per-call construction stays** (it is already better than Java's `static` field). **No new dependency is added.**

**Also here: `copper_geometry::Line`'s identity token.** #218 (Task 14) removed one of its two readers. If Task 18 kept the #74 ordering flip, the second reader is gone too and **the token and the static counter are deleted**, taking the port's last static-mutable exception with them. If Task 18 declined #74, **the token stays**, this task says so at the site, and the Global Constraint's sentence is amended to record the outcome. **Either way the answer is written down.**

**Tests (named, binding).**
- `crates/copper-router/tests/collections.rs` is **deleted** with `JavaTreeSet`; its element-drop assertions were already replaced by Task 8's total-order tests, and the deletion commit names them.
- `crates/copper-geometry/tests/random.rs::seeded_lcg_is_value_stable` — a fixed seed's first 16 outputs as literals, so a future refactor cannot silently move a golden (BL3's whole point).
- `crates/copper-dsn/tests/kicad_nets.rs` and `crates/copper-board/tests/delaunay.rs` are unchanged and must stay green — **they are the proof that the deleted emulation was dead.**
- `crates/copper-core/tests/shims.rs::no_java_prefixed_shim_survives_outside_the_keep_list` — a workspace grep test whose allow-list **is** survey §8.1's KEEP column (under its new names). This is the gate that keeps the deletion honest.
...
**1. The whole-branch review (Fable).** A single review of `main..plan-9-post-parity` — every commit, not a sample — against five questions, each of which has a mechanical check behind it so the review cannot be a vibe:
   ...
   3. **Is any `// Java bug:` marker deleted?** Check: `grep -rn '// Java bug:' crates` ≥ 165 (A11), and every `fixed:` register row has a `// fixed: T<n> (#id)` marker naming it — which `crates/copper-core/tests/register.rs` runs, so the check is the test rather than a grep.
...
**Tests (named, binding).**
- `crates/copper-core/tests/register.rs::a_fixed_row_has_a_fixed_marker` — armed at Task 0, now **non-vacuous**: it must pass over ~121 rows.
- `crates/copper-core/tests/register.rs::every_java_bug_marker_has_a_register_row` — still green, count ≥ 165.
- `crates/copper-core/tests/shims.rs::no_java_prefixed_shim_survives_outside_the_keep_list` — Task 24's gate, still green.
- The **full** suite, the five converted drivers, and `COPPERROUTE_SLOW_PARITY=1 cargo test --release`, all recorded in the merge message.
...
**Every review from Task 3 on re-runs `cargo test -p copper-router --test batch_parity` and `run.sh p6t1` on the committed tree, not on the implementer's word.** From Task 2 on, every review additionally reads the task's `goldens moved:` lines and spot-checks one moved golden by eye.
...
| A9 | 0 | `gen-reference.sh` "grows a `--from-port` mode that drives `target/release/copperroute` with the same argv" | the **jar** lane of that generator does not drive the jar's CLI either — it compiles and runs `scripts/gen-reference/RefWriter.java`, because `-de/-do` cannot write DSN headlessly. The port reproduces that limitation, so `target/release/copperroute` has no DSN-write path and there is nothing for `--from-port` to drive | **Task 0 wrote the port's `RefWriter` twin**, `scripts/differential/rust/src/bin/refwriter.rs` — the same three arguments, the same two steps (`copper_dsn::read_board` then `dsn_writer::write` / `ses_writer::write`), which is the pair `crates/copper-dsn/tests/parity_dsn.rs` already asserts reproduces the jar's bytes. Proved: both lanes byte-identical to all seven committed `roundtrip.dsn` / `unrouted.ses`. One new harness file outside the task's Files line, recorded here per BP11. |
| A10 | 0 | `quality-ab.sh`'s quality columns "run with `RouterBudget::disabled()`" (ruling AI, G2 spec) | the CLI **hard-codes** `copper_core::RouterBudget::default()` at `crates/copperroute/src/commands/route.rs:340`, deliberately (`gen-cli-reference.sh`'s header argues it: `p8t1` compares two whole programs and the jar cannot switch its own javac-inlined budget off). There is no knob, and `quality-ab.sh` drives the CLI because its 29 stems are references *of* the CLI | **`COPPERROUTE_ROUTER_BUDGET`**, read at exactly one site (`route.rs::harness_budget`), values `default` (the unset case — Java's four literals, byte-for-byte what every user, test and committed golden gets) and `disabled`; an unrecognised value exits 2 rather than falling back silently. **Not** a settings-surface addition: it is not a flag, not mergeable, absent from the manifest snapshot, and `EnvironmentVariablesSource` cannot see it (that source reads `COPPERROUTE__ROUTER__*` only). Measured: with and without it, `router-rpi-splitter`'s SES is identical. One file outside the task's Files line, recorded here per BP11. |
| A11 | 0, 25 | the `// Java bug:` census is "**≥ 165**", gated as `grep -c '// Java bug:' crates/*/src` | `crates/*/src` answers **148**. Survey §9.1's 165 is `grep -rn '// Java bug:' crates` — **every** file: **153** markers at `.rs` code sites (five of them in `crates/*/tests`, where a reproduced defect's only site is the assertion that pins it) plus **12** prose mentions in the seven crate `README.md`s | **`crates/copper-core/tests/register.rs` counts what §9.1 counted** and asserts both halves: total ≥ 165 and code ≥ 153. Task 25's gate should quote `crates`, not `crates/*/src`. The test excludes itself from both counts, because it discusses the marker in prose (`docs/java-quirks.md`'s own "the marker gate is two greps, not one" note, applied to itself). |
