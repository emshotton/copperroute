...
- [ ] Write `scripts/quality-ab.sh` (referee-only quality metrics + the `cpu_s` column, 29 stems, the two references, the `gate-version` header, BO's thresholds).
- [ ] Add `run.sh --against-jar`.
- [ ] Commit `benchmark/candidates.rs.toml` with its "why this file exists" header; create `benchmark/baselines/` (+ `ab/`).
- [ ] Write `benchmark/scripts/make-java-view.sh`; run it; paste its verification output.
- [ ] `tests/reference/README.md`: the freeze policy text, pointing at the sibling `tests/reference-frozen/` (Task 1 fills in the directory).
- [ ] The two new register tests, in `crates/fr-core/tests/register.rs`.
- [ ] The `quality-ab.sh --dry-run T0` smoke run; commit `benchmark/baselines/ab/quality-ab-T0.tsv` and the seeded `benchmark/baselines/stem-times.tsv`.
- [ ] G1; commit.
...
- **Timing (rulings BO/BP4) — Task 1 is one of the four tasks named in advance as likely to escalate.** #234 stops the pull-tight being abandoned on wall-clock, so **every multi-pass stem does strictly more work**, and this is a **quality-winning** change (trace length falls). The `cpu_s` column is therefore expected to rise and is **gated, not merely reported**: `> 2×` on any stem or `> 20 %` on the corpus median **escalates to the controller for an explicit keep/rework ruling before the task closes** — pre-authorised here, so the escalation is a message, not a re-plan. `benchmark/baselines/stem-times.tsv` is re-stamped at task close with this task's medians, and that is the number Task 2 is scored against.
- **No `bench run` in this task** (ruling BP3). M1's pre-fix reference is the existing `v1.0.0-rs` run of record.

**Steps:**
- [ ] Commit 1: freeze into the sibling `tests/reference-frozen/java-head-2026-09/` (rsync, `diff -r` verified), **then** write `benchmark/baselines/quality-baseline-java-head.tsv`, **then** `chmod -R a-w`; the two READMEs and the counts. **No code change in this commit.**
- [ ] Commit 2: `candidates.toml` verified and tidied, with the corrected `ecc0abf` provenance in the message.
- [ ] Commit 3: #234 — `impl Default for RouterBudget` (in `pipeline/stop.rs`) to 0, the new setting, the CLI wiring, the two directed tests.
- [ ] Commit 4: #208 — the fresh `TimeLimit`, its directed test.
- [ ] Commit 5: #224 — the timespan grammar and the loud refusal, its two directed tests.
- [ ] Commit 6: regenerate G/R/B/C/D `--from-port` with `meta.txt`; the `goldens moved:` line; the direction check.
- [ ] Commit 7: the two-run identity check, wired into G1 for every later task.
- [ ] Commit 8: `quality-ab.sh T1` — commit `benchmark/baselines/ab/quality-ab-T1.tsv` and the re-stamped `benchmark/baselines/stem-times.tsv`; escalate the `cpu_s` ratio if it trips BO's thresholds.
