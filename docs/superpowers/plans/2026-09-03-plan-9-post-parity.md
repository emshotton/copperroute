...
- **No threads below `crates/freerouting`, and no background waiters anywhere.** The two MCP spawn sites stay two. No task starts a process it then polls; every long run (a `bench` milestone, a corpus sweep) is run to completion in the foreground with its **full output redirected to a file** under `benchmark/out/` or `scripts/differential/out/`, and the file is read from disk. **Never `| tail`, never `| head` on a gate's output** — a truncated gate is an unread gate.
...
- **Quirk-register status-column discipline (recommendation 13, adopted).** `docs/java-quirks.md` **keeps every row** — it is a description of the *Java* program and that is what makes it useful upstream. Task 0 adds a **`status` column** with the values `pinned` (unchanged, the port still reproduces it), `fixed: T<n>` (the port no longer reproduces it; Plan 9 task `<n>` fixed it), `totalized`, `candidate`, `keep` (survey §9.1). A task that lands a fix **edits the status cell of every register row it closes, in the same commit**, and adds a `// fixed: T<n> — <one line>` comment **beside** the existing `// Java bug:` marker at each site. **The `// Java bug:` marker is never deleted** (survey §9.1: it is the provenance that makes the register navigable, and it is what a future reader compares the jar against). Task 25's gate is that `grep -c '// Java bug:' crates/*/src` is **≥ 165** and every register row whose status is `fixed:` has a matching `// fixed:` marker.
...
- **BL8 — The frozen baseline is written once and never asserted against again.** `tests/reference/java-head-2026-09/` is a historical artefact, not a test input. No test may read it. Its README says so.
...
scripts/differential/run.sh p8t1 ci              # 10 rows, against the port golden
...
```
plus **the fix's own directed test, named in the task, run once before the fix to record its failure and once after**. Rule: **zero diffs against the committed goldens**, or a regeneration in the same commit whose message carries the `goldens moved:` line. Full output of every driver goes to `scripts/differential/out/<driver>.txt` and is read from the file.

**Tier G2 — per task group. The stem quality A/B.**

`scripts/quality-ab.sh <task-id>` (Task 0 writes it) runs the port over **the 8 batch stems + the 13 CLI stems + the 8 DRC stems**, at each stem's committed `-mp` cap, with **`RouterBudget::disabled()` on both sides** (ruling AI survives the switch: time is out of every measurement), and prints one row per stem against the **FROZEN** jar numbers in `tests/reference/java-head-2026-09/quality-baseline.tsv` (Task 1 writes that file, from the frozen references, once).
...
**Tier G3 — the full 600-board bench. Exactly three runs in the whole plan.**

```
cd benchmark
uv run bench run --candidates java-current,rs-main --tier small,medium,large --seeds 3
uv run bench compare --baseline java-current --against rs-main
```
Rule: **`overall.verdict` is `better` or `same`, with zero `hard_losses`.** The referee's hard metrics are `clean_pass_rate`, `unrouted`, `violations`, in that order, and a `better` verdict is impossible with any hard loss. Two properties make this the right instrument and both are already true: the referee scores the `.ses` **independently** with KiCad 10 DRC and each board's own rules, so it cannot be fooled by the manifest rows Task 19/20 are fixing; and its noise band is **2× the baseline's own seed stdev**, so wall-clock jitter cannot manufacture a verdict. Full output to `benchmark/out/M<n>/`.

**The three milestones, named:**

| milestone | runs at the end of | what it proves |
|---|---|---|
| **M1** | **Task 2** | the two ablation-proven regressions are repaired in the port: small tier clean pass ≥ 0.77, connected ≥ 0.79, DRC-clean ≥ 0.95; large-tier clean pass ≥ 0.30. This is also the run that **re-baselines every later A/B**. |
...
```rust
// crates/fr-router/src/pipeline/airline.rs — Task 2 (R1). The three `// not ported:` lines at
// :22-24 become ported methods and the roster's greps are re-stated in the same commit.
...
// After R1 the work list is sorted ascending by `calculate_item_distance`, the descending-id walk
// surviving as the stable tie order. After #213 the element type carries its qualifying net:
fn autoroute_items(&mut self, board: &Board) -> (Vec<(ItemId, NetIndex)>, HandledItems);
...
// crates/fr-core/src/ctx.rs — Task 1 (#234). `default()` stops meaning "Java's live 1000 ms".
impl RouterBudget { pub fn default() -> RouterBudget; /* opt_changed_area_ms = 0 */ }
// and the value becomes a real setting: `router.opt_changed_area_ms`, default 0.
...
6. **`scripts/quality-ab.sh`.** The G2 harness: 29 stems (8 batch + 13 CLI + 8 DRC), `RouterBudget::disabled()`, each stem's committed `-mp` cap, one row per stem per metric of the G2 table, compared against `tests/reference/java-head-2026-09/quality-baseline.tsv` (Task 1 writes it). It reads the **referee's** DRC for incompletes and violations, never the manifest — the script must fail loudly if handed a manifest number, and its header says why (survey §4.1's 45–63 % disagreement). Output to `scripts/differential/out/quality-ab-T<n>.tsv`, full, never piped.
...
**Steps:**
- [ ] Re-read `docs/java-quirks.md`'s last row and confirm #293 is free; if not, allocate from the real next free id and amend BL6 in place.
- [ ] Add the status column to all three tables; seed every row; mark the ten §9.1 rows `keep`.
- [ ] Rewrite the rules header; add `// fixed:` to the marker table.
- [ ] Write rows #293–#296 with their evidence columns and the "Java-side fix owed" flag.
- [ ] Add `--from-port` to the five generators and `meta.txt`'s sha/task/budget line; prove `--jar` mode byte-unchanged.
- [ ] Write `scripts/quality-ab.sh` (referee-only metrics, 29 stems).
- [ ] Add `run.sh --against-jar`.
- [ ] `tests/reference/README.md`: the freeze policy text (Task 1 fills in the directory).
- [ ] The two new register tests.
- [ ] G1; commit.

**Commit message:** `chore(plan9): the gate machinery — register status column, // fixed: vocabulary, ids #293-#296, --from-port generators and quality-ab.sh`
...
**Interfaces produced:** `RouterBudget::default()` with `opt_changed_area_ms = 0`; the `router.opt_changed_area_ms` setting; `tests/reference/java-head-2026-09/quality-baseline.tsv`; the port-side two-run identity check.
...
1. **Freeze the jar copies FIRST, in their own commit, before #234 touches anything.** `cp -R tests/reference/ tests/reference/java-head-2026-09/` minus the new directory itself, then `chmod -R a-w`. `tests/reference/README.md` gains the freeze paragraph: *"This directory is the historical baseline the port was proven byte-identical to at `c467758`, generated by the clone's HEAD jar. It is **never again an assertion** — no test may read it (BL8). It exists so that a future reader can answer 'what did the jar do here?' after the differential drivers are gone."* Record the byte count and file count in the commit message.
2. **`quality-baseline.tsv`.** Derive the G2 baseline numbers from the frozen references — the referee's DRC over each frozen `route.ses`/`batch.ses`, plus the score/length/via/bend columns — and commit it inside the frozen directory. This is the only file in there that later tasks *read*, and it is read as **numbers**, not as an assertion (its own header says so).
3. **`benchmark/candidates.toml`.** `[candidates.rs-main]` is already live (it was uncommented at `c467758`); **delete the stale `# NOTE: uncomment once the fork builds`** and verify `exec = ["../target/release/freerouting"]` resolves. Then record a baseline `bench run --candidates java-current,rs-main --tier small --seeds 3`, which **makes the two §4.1 regressions measurable in the port before they are fixed** — this is the number M1 is compared against.
...
5. **Regenerate G, R, B, C, D from the port** (`--from-port` on all five), once, at the end of the task, with `meta.txt` carrying the sha, `Plan 9 Task 1`, and `opt_changed_area_ms = 0`.
...
**Evidence / acceptance.**
- The freeze commit: file count and byte count recorded; `chmod` verified; **no test reads the directory** (`grep -rn "java-head-2026-09" crates/` returns only `tests/reference/README.md`'s prose and `quality-ab.sh`'s baseline read).
- **G, R, B, C, D all regenerate**, deliberately and once. The `goldens moved:` line reads: *"#234 — the pull-tight is no longer abandoned on wall-clock, so every multi-pass stem is tightened further; batch SES bytes move on 8/8 stems, router JSONL on 6/6 rows, CLI on 13/13 stems."* Direction is checked: total trace length must **fall or hold** on every stem (a fully-run pull-tight cannot lengthen a trace), and any stem where it rises is a **stop-and-report**.
- G2 against the frozen baseline: incompletes must not rise on any of the 29 stems; violations 0 on every stem.
- The `bench` baseline run recorded to `benchmark/out/baseline-T1/` — **not** a milestone, a baseline.

**Steps:**
- [ ] Commit 1: freeze `tests/reference/java-head-2026-09/`, read-only, with the README paragraph and the counts. **No code change in this commit.**
- [ ] Commit 2: `quality-baseline.tsv` derived from the frozen references.
- [ ] Commit 3: `candidates.toml` tidy + the recorded `bench` baseline run.
- [ ] Commit 4: #234 — `RouterBudget::default()` to 0, the new setting, the CLI wiring, the two directed tests.
- [ ] Commit 5: #208 — the fresh `TimeLimit`, its directed test.
- [ ] Commit 6: #224 — the timespan grammar and the loud refusal, its two directed tests.
- [ ] Commit 7: regenerate G/R/B/C/D `--from-port` with `meta.txt`; the `goldens moved:` line; the direction check.
- [ ] Commit 8: the two-run identity check, wired into G1 for every later task.
- [ ] Register: #234, #208, #224 → `fixed: T1`, with `// fixed: T1` markers beside their `// Java bug:` lines.

**Commit message (final commit of the task):** `feat(determinism): the clock leaves the router — #234 opt_changed_area off by default, #208 a fresh retry budget, #224 the documented timeout spellings; goldens re-cut from the port`
...
**Files:** `crates/fr-router/src/pipeline/airline.rs` (three `// not ported:` lines at `:22-24` become ported methods); `crates/fr-router/src/pipeline/batch_autorouter.rs` (`autoroute_items`, `:657-781`); `crates/fr-router/src/pipeline/pass_runner.rs` (`:168`, the consumer); `crates/fr-router/src/autoroute/path/inserter.rs` (`insert_fanout_micro_neckdown`, `:488-…`, the candidate list at `:509-…`, called from `:394`); `crates/fr-board/src/rules/board_rules.rs` (`get_min_trace_half_width`, `:138` — **the reader**, not `crates/fr-board/src/board/mod.rs:1787`); `crates/fr-router/tests/{batch_autorouter.rs,inserter.rs,fanout.rs}`; `crates/fr-router/README.md` (the roster's re-stated greps); `docs/java-quirks.md`; `benchmark/reports/java-regressions-2026-09.md` (a "status: fixed in the port at Plan 9 Task 2" header line).
...
- **MILESTONE M1** — the full bench, both tiers, output to `benchmark/out/M1/`. **Acceptance, from the report's ablation numbers:** small tier **clean pass ≥ 0.77**, **connected ≥ 0.79**, **DRC-clean ≥ 0.95**; large tier **clean pass ≥ 0.30**. `overall.verdict` `better` or `same`, **zero hard losses**. A run that misses these has not reproduced the ablation and the task is not done — the ablation was measured at Java HEAD and the port is HEAD, so the numbers are expected to transfer; a gap is a real difference between the port and the jar and must be root-caused, not tolerated.
- **I2's column recorded**: via count per stem and per tier at M1 vs the Task 1 baseline, written into `benchmark/reports/java-regressions-2026-09.md` as a status appendix.
...
- [ ] Commit 5: **M1** — run the bench, commit `benchmark/out/M1/` and the report appendix.
...
**Evidence / acceptance.** G1 green. **C moves on the 2 KiCad stems** (`cli-kicad-ecc83-json`, `cli-kicad-complex-hierarchy-json`): their `route.json` now holds a routed board. `goldens moved:` names #289 and the direction (wire count rises from 0 to the SES's count). `p8t7` and `cli_e2e` re-golden. Plan 8 hand-off §3's divergence rows **10** and the `-do out.json` half of row **4** are struck and replaced with a "fixed in Plan 9 Task 3" line — Task 25 carries the edit. G2 unaffected (no routed board changes).

**Steps:**
- [ ] Commit 1: #265 — delete `delete_existing_output` and its call; two directed tests.
- [ ] Commit 2: #268 — the return is tested, the refusal moves to the argument; one directed test.
- [ ] Commit 3: #289 — one serialisation after the pipeline, the snapshot and the re-sniff deleted; two directed tests, one Plan 8 test deleted by name.
- [ ] Commit 4: re-cut the two KiCad C-stems; `goldens moved:`.
- [ ] Register: #289, #268, #265 → `fixed: T3`.
...
**Evidence / acceptance.** G1 green. **G regenerates** (all 6 `fixtures.txt` rows + the `fr-dsn` round-trip goldens) and **B/R/C move on any board that gains its file's router settings** — `goldens moved:` names #95 with the sweep's before/after count, and #94+#89+#93 with the outline's restored extents. `cli-large-outline` re-cut. `sweep-p3t15.sh` re-run over the 106-fixture corpus: **its 5 expected XDIFFs must not grow**, and any fixture that changes is named. G2: incompletes must not rise; #95's newly-read settings can legitimately change a board's route, so a stem whose score moves is explained, not tolerated.

**Steps:**
- [ ] Commit 1: #95 + the corpus sweep counter.
- [ ] Commit 2: #90 (token consumption **and** the caller loop).
- [ ] Commit 3: #94+#89+#93 in one commit, with the fixture decision recorded (`cli-large-outline` reused or a new fixture built with its hand-computed expectation).
- [ ] Commit 4: #112 — `RuleLayerScope`.
- [ ] Commit 5: #91 — `BoardReadResult::Partial` and every consumer.
- [ ] Commit 6: #103.
- [ ] Commit 7: #86.
- [ ] Commit 8: regenerate G (+ B/R/C where #95 moved a board); `sweep-p3t15.sh`; `goldens moved:`.
- [ ] Register: seven rows → `fixed: T4`.
...
**Files:** `crates/fr-router/src/board_ext/routing_board_ext.rs` (`insert_forced_trace_polyline`); `crates/fr-router/src/autoroute/path/locator_any_angle.rs` (`calculate_next_trace_corners`); `crates/fr-router/src/autoroute/engine.rs` (`remove_incomplete_expansion_room`, `ExpansionDrill::calculate_expansion_rooms`); `crates/fr-router/src/autoroute/drill_page.rs` (`get_drills`); `crates/fr-router/src/autoroute/control.rs` (`init_net`); ten sites across `crates/fr-board`, `crates/fr-settings`, `crates/fr-geometry`; `crates/fr-router/tests/{drill.rs,control.rs}`; `crates/fr-board/tests/*`; two **new** synthetic fixtures under `crates/fr-router/tests/data/`.
...
**Files:** `crates/fr-dsn/src/kicad/{reader.rs,dto.rs,writer.rs}`; `crates/fr-dsn/src/kicad/java_hash.rs` (`JavaStringSet`, `JavaStringMap`, `java_hash_iteration_order`, `java_string_hash` — **deleted here or made dead here, deleted in Task 24**); `crates/fr-dsn/src/kicad/npe.rs` (`JavaNpe`); `crates/fr-dsn/tests/kicad_*.rs`; `crates/fr-core/tests/data/p8t8-kicad-read-{a,b}.txt` and `p8t10-kicad-writer.txt` (probe transcripts re-cut or retired); `tests/reference/cli-kicad-ecc83-json/*`, `tests/reference/cli-kicad-complex-hierarchy-json/*`.
...
**Evidence / acceptance.** G1 green. **C moves on the 2 KiCad stems** — twice over: #284 re-names every padstack the SES carries, and #280 renumbers every net. `goldens moved:` names both, with the net-number mapping (old → new) for `Issue649` pasted in full. `p8t7` re-golden; the `P8T8Probe`/`P8T10Probe` transcripts are **re-cut as port goldens or retired** — #282's DTO validation turns several of their rows into rejections, and the surviving rows are the ones that still describe the port. **The `debug_assert!` guarding treeification is deleted** and its deletion named. `JavaNpe`, `net_name_is_null`, `package_pin_names`, `java_drill_item_tile_shape_count` become dead here; **Task 24 deletes them** (do not delete a shim before its fix has merged — survey §8). G2: the two KiCad stems' incompletes must not rise.
...
**Steps:**
- [ ] Commit 1: the 90-degree fixture with its reviewed expectation.
...
**Evidence / acceptance.** G1 green. **B, C, R move on every routed stem** — the optimizer now does work, so **every** board's geometry changes and **every** board's wall time rises. `goldens moved:` names #227 first and quantifies: trace length and via count per stem, before and after. **Direction check: normalized score must RISE on every stem** — an optimizer that runs and makes a board worse is a stop-and-report. `p7t8`, `p7t9`, `p8t1`, `p8t2` re-golden; `p7t9`'s RESULT line changes with #214 and the change is named. G2: score up on every stem; incompletes not up on any; **wall time is reported and explicitly not optimised** (BL2's table). #217's decision recorded in the register row.
...
- [ ] Commit 8: regenerate B/R/C; `goldens moved:` with the per-stem score/length/via table.
...
**MILESTONE M2** — the full bench at the end of this task, output to `benchmark/out/M2/`. **Acceptance:** `overall.verdict` `better` or `same`, **zero hard losses**, against **M1's numbers**. Additionally recorded, because M2 is the point where the T2 heart is complete:
- **The I2 (#296) via-inflation column**, per tier, against M1 and against the Task 1 baseline. If the inflation has deflated with R1, the register row is **closed** with the measurement. If it survives, the row stays open and names **#172** (Task 18) as the next hypothesis — the survey's §10.2 reading is that the 10× via discount on pure-SMD nets and the inflation are plausibly the same phenomenon from two ends.
- **The #231 follow-up experiment** promised at Task 10: one extra `bench` comparison arm with `router.copper_to_edge_clearance_um=0`, so the "does the 500 µm default apply at all?" question has a number. **This is the only extra bench arm the plan authorises**, and it rides on M2's run rather than costing a fourth.
- **The joint neckdown table** (R2 / #179 / #51) at corpus scale.

**Steps:**
- [ ] Commit 1: #206 + #207 + #205.
- [ ] Commit 2: #192, with the boundary decision recorded.
- [ ] Commit 3: regenerate R/B; `p7t4` re-golden; `goldens moved:`.
- [ ] Commit 4: **M2** — the bench, the I2 column, the #231 arm, the neckdown table; `benchmark/out/M2/` committed.
...
**MILESTONE M3** — the full bench at the end of this task, output to `benchmark/out/M3/`. It is the **whole branch** against `java-current`. **Acceptance:** `overall.verdict` **`better`**, zero hard losses, and the small-tier numbers at or above M1's. M3 is also where each of the six rows is confirmed or reverted. Read into Task 25.
...
- [ ] Commit 7: **M3** — the bench; each of the six confirmed or reverted; `benchmark/out/M3/` committed.
...
**Files:** `crates/fr-drc/src/{report/mod.rs,report/json.rs,unconnected.rs,violations.rs}`; `crates/fr-board/src/items/mod.rs` (`smallest_clearance` — the field is **deleted**); `crates/fr-board/src/board/mod.rs` (`get_all_clearance_violations`); `crates/fr-router/src/score/statistics.rs` (the length breakdown, the bounding box); `crates/fr-dsn/src/writer/{ses_writer.rs,rules_writer.rs}`; `crates/fr-board/src/datastructures/delaunay.rs` (`validate`); `crates/fr-router/src/optimize/item_route_result.rs`; `crates/freerouting/src/commands/drc.rs`; `crates/fr-drc/tests/{report.rs,unconnected.rs}`; `crates/fr-board/tests/{clearance_violations.rs,snapshot.rs}`; `tests/reference/drc-*/**` (all 8).
...
**Files:** `crates/fr-core/src/{manifest.rs,stats_from_bytes.rs,stats_json.rs}`; `crates/fr-router/src/score/statistics.rs`; `crates/freerouting/src/commands/route.rs`; `crates/fr-core/tests/{manifest.rs,stats.rs}`; `tests/reference/cli-*/manifest.json` (13); `scripts/differential/run.sh` (`p8t2probe` retires here).
...
**Files:** `docs/plan-9-handoff.md` (**new** — the post-parity completion report); `docs/java-quirks.md` (the final status sweep); `docs/plan-8-handoff.md` (§3's struck divergence rows, §7's struck limitations); `benchmark/reports/java-regressions-2026-09.md` (the closing status section); every crate README's Plan 9 paragraph; `docs/superpowers/plans/2026-09-03-plan-9-post-parity.md` (the amendment log, filled).
...
**1. The whole-branch review (Fable).** A single review of `main..plan-9-post-parity` — every commit, not a sample — against five questions, each of which has a mechanical check behind it so the review cannot be a vibe:
   ...
   3. **Is any `// Java bug:` marker deleted?** Check: `grep -c '// Java bug:' crates/*/src` ≥ 165, and every `fixed:` register row has a `// fixed: T<n>` marker.
...
**3. The completion report, `docs/plan-9-handoff.md`.** Eight sections, mirroring Plan 8's shape so a reader who knows one knows the other:
   ...
   2. **What was measured** — M1, M2 and M3's verdicts side by side with the Task-1 baseline and the report's ablation numbers, plus the G2 table's 29 stems at M3.
   ...
   5. **The frozen baseline** — what `tests/reference/java-head-2026-09/` is, why it exists, and the rule that no test reads it.
...
**Milestones:** **M1** at the end of Task 2, **M2** at the end of Task 16, **M3** at the end of Task 18. Three whole-corpus runs in the plan, plus one authorised extra **arm** (not run) inside M2 for #231's follow-up.
...
| 16 | medium | sonnet | opus | M2 is here |
...
**The count, reconciled.** The survey's §12 headline says **117 FIX rows** and its §7.5 table sums to the same number, but neither sub-list sums to its own subtotal (§7.5's "one family: 49" enumerates 27). A strict row-by-row enumeration of §3–§6 — 129 status-bearing table rows, minus 4 `ALREADY-FIXED`, minus 3 pure `NOT-A-FIX`, minus the 2 rows cross-listed twice (#192 in §4.3 and §4.10; #155 in §5 and §6.1) — gives **120 distinct FIX rows**, plus §6.3's `OutlineJson.clearance` sub-item that survey §10.3 places in group 7, for **121**. **All 121 are placed; none is unplaced.** The three-row gap against the headline is the survey's own arithmetic, not a missing row, and it is recorded here so a reader who counts gets the same answer twice.
