...
> **This is the FINAL plan of the port.** Task 14's hand-off is not "obligations for Plan 9" — there is no Plan 9. It is the **project completion report**: what is ported, what is rostered dead and why, every deliberate divergence from the jar, and how any future reader re-runs every parity check in the repository. Nothing may be deferred out of Plan 8 to a later plan; a thing that cannot land is a **recorded, closed decision** with its evidence, not a `// added in Plan 9:` marker.
...
**Spec:** `docs/superpowers/specs/2026-08-27-freerouting-rust-port-design.md` (§1, §2, §4, §10, §12, §13, §14.4, §15). Also binding: `docs/plan-3-handoff.md` §Plan 8, `docs/plan-4-handoff.md` §Plan 8, `docs/plan-5-handoff.md` §Plan 8, `docs/plan-6-handoff.md` §11, `docs/plan-7-handoff.md` **§7 "The obligation register for Plan 8" (`:389-489`), its twelve **§9 "Contracts Plan 8 must not break"** (`:538-575`) and §12's four open items — written, committed and verified at pre-flight**, `docs/java-quirks.md` (every row and the obligation register), `docs/cli-legacy-flags.md`.

**Later plans:** none. See the callout above.

> ### ✅ Plan 7 has landed, and the pre-flight scan has run — this document is AMENDED IN PLACE
>
> The draft was written while `plan-7-router-batch` was at **`b899468`** (Plan 7 Task 1), so every
> line consuming a Plan 7 interface carried (verified at pre-flight — scan §1). **Plan 7 is now on `main`
> (`8d9b45d`: 2 137 tests, 33 audits at 0 `MISSING` / 0 `UNMAPPED`, SES byte parity on 8/8 stems),
> and the pre-flight conflict scan walked all 31 tags against the committed tree**
> (`.superpowers/sdd/2026-09-01-plan-8-core-cli-mcp/preflight-scan.md`). Twenty-two conflicts came
> out of it; the controller ruled on all twenty-two and they are applied **in place** below, each
> marked “(scan ruling R*n*)” at its site — exactly as Plan 7's own scan produced its 17 rulings.
> **An implementer who still finds a signature different from what is written here stops and
> reports**; it is a further amendment, never a silent adaptation.
>
> **The 22 scan rulings, in one list.** *(Severity A = blocked a task's dispatch.)*
>
> | # | what the draft said | what the tree said | ruling |
> |---|---|---|---|
> | **R1** (A) | `p8t1` runs a bare `-de/-do` and both sides run their budgets live | quirk #234: the jar is non-reproducible above each stem's pass ceiling and its 1000 ms constants are `javac`-inlined; `--verify-driver` measured **8/8 identical, 0 trips** at the caps | per-stem `-mp` caps in `argv.txt`; the port runs `RouterBudget::disabled()`; `router-dac2020-bm01` stays, capped at `-mp 2` |
> | **R2** (A) | Task 3 ports the three `HeadlessBoardManager` clearance overrides (≈ 370 Java lines) | all three already landed by **Plan 7 Task 15b** in `fr-board`'s `clearance_override.rs`, driven by `fr_router::pipeline::prepare_board` | **SHRINK**: `fr-core/src/overrides.rs` struck; Task 3 is wiring + audit and consumes `Board::apply_*` / `prepare_board` **by name**; ≈ 170 Java lines |
> | **R3** (A) | the `CancelToken` adapter joins at “the six existing `poll_deadline` sites” and returns a poll closure | **one** `poll_deadline` call site (`batch_loop.rs:303`); `RouterStop` is `Cell`-based/`!Sync` with private fields; `run_pipeline` has no closure hook | ruling AP's own escape — **add polls**; an **additive-and-wrapped, driver-pinned** `fr-router` change (old signatures kept and wrapped; `batch_parity`/`p6t1`/`sweep-p7t9.sh` byte-unchanged) |
> | **R4** | `RoutingResult.incompletes: Vec<(String, Vec<String>)>` | `build_unrouted_report(&mut Board) -> String` | the field becomes `unrouted_report: String`; the count comes from `final_statistics.connections.incomplete_count` |
> | **R5** | “the **twenty-three** `added in Plan 8:` sites”, incl. `fr-router/src/lib.rs:243-244` | **30 markers** in `crates/*/src`; `fr-router/src/lib.rs` has **none**; 9 unlisted in `clearance_override.rs:399-407` and 2 in `pipeline/{batch_autorouter,optimizer}.rs` | the grep is the checklist, not the list; every orphan assigned an owner |
> | **R6** | `crates/fr-settings/src/resolve.rs:184` (×7) | the marker is at **`:193`** | global correction |
> | **R7** | — (zero mentions of `JsonFileSettings`) | plan-7 §7/§12: *“a real hole in the CLI's settings ladder … Plan 8's Task 5 is where it belongs”* | **Task 5 ports it** — priority 10, `--settings <file>`, quirk #141's reader |
> | **R8** | “the **five** obligations this plan closes” | a **sixth** names Plan 8 with a live trigger: `clearance_override.rs:60` (quirk #232, *“before Plan 8 exposes `router.hole_clearance_um` on a real CLI path”*) | **Task 3 owns and closes it**; `P8T3Probe` gains the tree-op/order pin |
> | **R9** | “contiguous through #196 … next free **#197**” | 235 contiguous rows; `java-quirks.md:540` says next free **#236** | ids from **#236** in write order; labels A–AL are a separate namespace from rulings AO–AV |
> | **R10** | *“grep … returns **five** sites, all in `Freerouting.java`”* | **16 sites in 9 files** | the conclusion (three file-format readers) stands; the evidence sentence is replaced and Task 0 pastes a fresh grep |
> | **R11** | `parseTimespanString` *“returns a `Duration` and **throws**”*, caught at the call site | returns **`Long` seconds**, returns **`null`**, catches `DateTimeParseException` itself; `:44` has no try/catch; `GRACE_PERIOD` is not applied at `:44-52` | Java wins; the doc comment is rewritten to the measured behaviour |
> | **R12** | `route_board` returns `data` (Base64) | **no `base64` dependency**, and none may be added | hand-write RFC 4648 with its §10 vectors — the pair to Task 4's SHA-256 |
> | **R13** (A) | “Fourteen tasks, 0–13”; the four zero-coverage Plan 3 paths appear only as a *limitation* | controller answer 5 **authorised a 15th task** for exactly those paths | **Task 13** is that task (coverage-only, sonnet); the hand-off task renumbers to **Task 14** |
> | **R14** | `scratchpad/plan8-{rulings,evidence}`, `scratchpad/post-parity-roadmap.md` | all committed under `docs/plan-8-prep/` | paths corrected (×3) |
> | **R15** | *“If Plan 7's copy landed, re-use it”* (`unescapeUnicode`) | it landed **private** at `score/statistics.rs:596` | make it `pub` behind a `pub seam:` — the second additive-and-wrapped `fr-router` change |
> | **R16** | “the **ten** DTOs” | **twelve** (+ `ClearanceViolations`, `Fanout`) and 14 fields; `Fanout`'s `i32`s emit `0`, not omission | twelve, with a new serialisation test |
> | **R17** | `FileFormat` is “`RoutingJob.java`'s enum”, seven values | it is `io/FileFormat.java` with **nine** — `Frb` and `Scr` were missing, and both are reachable from `-de`/`-do` | port all nine from the right file |
> | **R18** | `p8t6` drives the jar with `--mcp_server.stdio=true`; “29 tools”; a `SKIP` is a delta | job 3 proves that flag alone does not start the bridge, and measured **28** tools plus three further deltas | job 3's launch line imported verbatim; the `SKIP` escape deleted; the delta table is **ten** rows |
> | **R19** (A) | *“Java parses argv **twice**”*, so the port parses once | job 2 measured **five** parsers; P4 (`CliSettings`) matches `-mp`/`-mt` by exact `switch` and is **already ported** | the port keeps **two**: P3 for the legacy slots, P4 for `--router.*=`/`-mp`/`-mt`. `-mpx 5` must not set `max_passes` |
> | **R20** | `from_job(job, path, written, code, now)` | `normalized_score` is a method needing `&ScoringSettings`, and `board_statistics` is the cached `final_statistics` | two parameters added, plus a test for the `maximum_score <= 0.0` arm |
> | **R21** | the roadmap's five uncovered board classes — “the acceptance-ladder task should add stems” | no task owned it | Task 6 searches the corpus and adds what qualifies (slow lane); Task 14 records what does not |
> | **R22** | twelve audit invocations | `settings/sources` (R7) and `io/` (R17) are reached by none of them | no new invocations; Task 14 re-runs Plan 3's and Plan 4's as a sweep and records both as **MAPPED** |
>
> **Nine line-number drifts** were corrected in place without a ruling (scan notes N1–N9): `query.rs:1070`→`:1096`; `BoardLoader`'s guard `:32`→`:31`; `BoardStatistics(byte[])` `:436-552`→`:436-554`; `batch_autorouter.rs:156`→`:158`; the batch references are at repo-root `tests/reference/`; the “no `fr-router` API change” sentence; the root `Cargo.toml` needs no edit (glob members); “five Plan 6 stems” is five boards / **six rows**; and “exactly two threads” means two **spawn sites** (there is no MCP cargo feature).
>
> **Also resolved at pre-flight, so no task has to decide it:** risk-2's conditional (→ SHRINK, R2); the starting quirk id (→ #236, R9); and **controller answer 1's `KiCadJsonWriter` condition (→ PORT IT** — the headless caller is `RoutingJobSchedulerActionThread.java:277`, the live `-do out.json` path).
>
> The tagged surface, in one list, as the scan checked it:
...
> NoopProgressSink, build_unrouted_report, prepare_board}`; `fr_router::score::{BoardStatistics +
> its TWELVE DTOs, calculate_score, maximum_score, normalized_score, is_pin_escaped}`;
> `fr_router::RouterError::NoRoutableLayer`; `Board::structural_hash` (widened, Plan 7 Task 3);
> `Board::{apply_copper_to_edge_clearance_override, apply_hole_clearance_override,
> assign_hole_keepout_clearance_class}` (Plan 7 Task 15b);
> `ViaRule` owning its `ViaInfo`s (Plan 7 Task 0, landed as `bde59ef`);
> `tests/reference/<stem>/batch.{ses,passes.jsonl,meta.txt,verify-driver.txt}` and the eight batch
> stems of Plan 7 Task 16; `scripts/gen-batch-reference.sh`; `crates/fr-router/tests/batch_parity.rs`.
> **All 31 tags resolved: 25 OK/DRIFT, 6 CONFLICT (rulings R1–R4, R5, R16).**
...
- Behavioral port: reproduce Java bugs, with a `// Java bug:` marker at the site and a row in `docs/java-quirks.md`. `// totalized:` for crash→value changes, `// not ported:`, `// not reachable:`, `// renamed:`, `obligation:` per `docs/java-quirks.md` §Process notes. **There is no `// added in Plan 9:`** — the marker vocabulary loses its deferral arm in this plan (Task 14 enforces it). **The marker's Java method name must sit on the same line as the marker** (Plan 2 ruling 13 — `audit-port.sh` is line-based).
...
- **Threads: exactly two, both in `crates/freerouting/src/mcp/`** (ruling 3). `fr-router` remains single-threaded and `max_threads` remains dead everywhere (quirks #143 / Plan 7's `-mt`-is-dead-everywhere row); the MCP's threads carry no routing policy, only the transport. Nothing below `crates/freerouting` may see a thread — and **there is no cargo feature gating the MCP** (scan note N9: the draft implied one; none exists and no task creates one). The property is structural, not conditional: `mcp` is a module of the binary crate and no crate below it depends on the module either way.
...
- Every task ends with `cargo fmt --all`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, and the `scripts/audit-port.sh` invocations from Task 14 (with `scripts/audit-map/{fr-core,freerouting}.map` once Task 0 creates them) run and their `MISSING` count recorded in the commit message — verified against the committed tree, not against a report.
- **From Task 3 on, every task's verification additionally runs `cargo test -p fr-router --test batch_parity` and `scripts/differential/run.sh p6t1` on the Plan 6 corpus — five boards, **six rows** (`router-rpi-splitter`, `router-dac2020-bm01`, `router-dac2020-bm01-pass2`, `router-j2-reference`, `router-tutorial-board`, `router-ecc83-input`; scan note N8).** Plan 8 touches the board *load*; a load change is invisible in Plan 8's own tests and lethal to Plan 7's.
- Commit trailer:
  ```
...
**The controller's pre-plan rulings AO–AV, the risk-2 ruling and the General clause (`docs/plan-8-prep/rulings-AO-AV.md`, 2026-08-30 — moved out of the session scratchpad and committed; scan ruling R14) are binding and are restated here so the implementer needs one document:**
...
- **AP — cancellation is option (a):** an external `Arc<AtomicBool>` `CancelToken` that the routing thread copies into `RouterStop` at the existing poll sites. **[scan ruling R3 — amended at pre-flight.]** The plan assumed **six** `poll_deadline` sites; the committed tree has **exactly one** (`crates/fr-router/src/pipeline/batch_loop.rs:303`, and `pipeline/stop.rs:67` says so in its module doc — plan-7 ruling AI: *"`poll_deadline` requests `ALL` and has exactly one production call site"*). The other five clock reads are per-stage `TimeLimit`s and are explicitly **not** `poll_deadline`. **This ruling takes ruling AP's own escape: add polls, never a lock.** `RouterStop` gains an **additive, wrapped** constructor/setter so the token can be observed at the one job-level site *and* at the per-stage sites ruling AI enumerated (`fanout.rs:1142`, `optimizer.rs:1110`); every existing signature stays and is wrapped, and the gate is that **all existing drivers are byte-unchanged** (`batch_parity`, `p6t1`, `sweep-p7t9.sh`).
...
- **AU — no HTTP API.** `api/**` is rostered in full (8 425 lines; `JobControllerV1`'s 659 are provably dead — not in `FreeroutingApplication.getClasses()`). **`ApiSettings`' priority-70 tier STAYS**: the MCP `route_board { settings? }` argument composes exactly that tier, per the `obligation:` at `crates/fr-settings/src/resolve.rs:193`.
...
5. **`PARITY_VERSION` does not reach the SES or the DSN, and this plan says so rather than repeating ruling AT's list. (This corrects the ruling and the survey.)** `(hostCad …)` / `(hostVersion …)` are written by `io/specctra/parser/Parser.java:108-115` from `ReadScopeParameter.{hostCad, hostVersion}` (`:69`), which `Parser.readScopeParameter:204` fills **from the input file**; `crates/fr-dsn/src/ses_writer.rs:297` already does the same through `board.communication`. **[scan ruling R10 — the evidence sentence was wrong; the conclusion survives.]** `grep -rn "FREEROUTING_VERSION" src/main/java` returns **16 sites across 9 files**, not five in one: `Freerouting.java:69, 1260, 1334, 1370, 1388` (a log line, the runtime-environment string, three analytics/version-check calls); `settings/GlobalSettings.java:3, 208` (`getReleaseSafeVersion`, a version-comparison helper); `api/mcp/McpControllerV1.java:282, 287` and `api/mcp/AgentCardController.java:47` and `api/v1/AnalyticsControllerV1.java:126, 251` (all rostered, ruling AU/AO); and the three below. **Task 0 pastes the fresh grep into its commit message** rather than transcribing this sentence (Plan 7 ruling 6's precedent). `PARITY_VERSION`'s real file-format readers are still exactly three: the DRC report's `freerouting_version` (`drc/DesignRulesChecker.java:213`, `"Freerouting " + …`), the manifest's `app_version` (`RoutingResultManifest.java:102`) and `BoardStatistics.host`'s fallback (`:118-120`, **unreachable** — quirk label AK). **`Constants.java` is build-generated and is not in the clone's source tree**, so the value must be read out of the HEAD jar at pre-flight (`unzip -p <jar> app/freerouting/Constants.class | strings`, or a one-line reflective probe) and pinned as a literal with the jar's `Build-Revision` beside it. *Cost if wrong:* a manifest `app_version` that no jar ever wrote, and a DRC byte-diff against eight committed references.
...
10. **The pre-bootstrap argv parse is collapsed to one parse, and the two behaviours that differ are recorded.** Java parses argv **twice** — once raw before log4j init (`Freerouting.java:901-917`, `:1032-1065`) and once in `applyCommandLineArguments` — and the two disagree: `-dl` is `equals` in `main:1056` but `startsWith` in `GlobalSettings.java:799`; `-ll` takes the **first** occurrence early and the **last** late (quirk label AJ). **[scan ruling R19 — amended: Java has FIVE parsers, and the port keeps TWO.]** `docs/plan-8-prep/evidence/job2-summary.md` §1 measured them: **P0** the stdio pre-scan (`:901-917`), **P1** user-data (`:944-956`, `startsWith`, **first** wins), **P2** logging (`:1032-1065`, `-dl`/`-ll` by exact `equals`, `-ll` **first** wins), **P3** the flag table (`GlobalSettings:521-838`, `startsWith`, **last** wins), **P4** `settings/sources/CliSettings.java:39-79` — `--router.*=`, and **`-mp`/`-mt` by an exact `switch` on `arg.substring(1)`**. P4 is **already ported** (`crates/fr-settings/src/sources/cli.rs`) and pinned by `p4t1`'s 64-case matrix. So: **P3's rule governs the legacy slots and the `LegacyBridge`; P4's rule governs `--router.*=` / `-mp` / `-mt`.** Collapsing them would make `-mpx 5` set `max_passes`, which **no** Java parser does. P0/P1/P2 stay `// not ported:` (logging is not a parity surface — spec §2 drops log files) with the one quirk row this ruling already prescribes. `p8t5`'s shapes must include **`-mpx 5`, `-mtx 2` and `-dlx`** so the two-parser split is asserted, not assumed. *Cost if wrong:* a log level that differs from the jar's on a doubled `-ll`, which no output file can observe.
11. **One audit map per new crate, and `management/`, `core/` and `api/` are audited for the first time in the project.** `scripts/audit-map/{fr-core.map, freerouting.map}` are new; Task 14 runs **twelve** invocations (§The audit, Task 14) over `core/`, `core/scoring/`, `core/results/`, `core/events/`, `io/kicad/`, `management/`, `management/jobs/`, `management/sessions/`, `api/`, `api/mcp/`, `logger/` and `Freerouting.java` itself, to **zero `MISSING` and zero `UNMAPPED`**. §5.7 of the survey exists because nobody ever ran the first of those. *Cost if wrong:* the audit is the only mechanical check that 3 300 lines arrived and 13 500 were consciously dropped; weakening it is forbidden.
...
crates/fr-board/
  src/board/mod.rs                :57 BoardComparator -> `// not ported:` with ruling AS (Task 14)
crates/fr-router/
...
crates/freerouting/
  ...
  README.md
scripts/audit-map/fr-core.map     NEW (Task 0, filled by every task, closed by Task 14)
...
docs/plan-8-handoff.md            the PROJECT completion report (Task 14)
...
/// `Constants.java` is build-generated and absent from the clone's source (verified at pre-flight:
/// `src/main/java/app/freerouting/Constants.java` does not exist; the symbol is
/// `app.freerouting.constants.Constants`). The literal below is read out of the HEAD jar and the
/// jar's `Build-Revision` is recorded beside it. **Starting point, not a licence to guess:**
/// `docs/plan-8-prep/evidence/job3-summary.md` records the HEAD jar answering `serverVersion` =
/// `2.3.1-SNAPSHOT` (build-date 2026-08-29 per job 2) — confirm against the jar, do not copy.
/// The grep in the ruling above is 16 sites in 9 files, not 5 in 1 (scan ruling R10).
...
impl CancelToken {
    ...
    /// Ruling AP's adapter, **as amended by scan ruling R3**. `RouterStop` is `Cell`-based
    /// (`!Sync`) with private fields and `run_pipeline` takes `&RouterStop` and offers no closure
    /// hook, so the drafted `(RouterStop, impl Fn())` pair had no call site at either end. The
    /// adapter is therefore a **constructor**, and the poll happens inside `fr-router` at the sites
    /// Task 0 adds (additive-and-wrapped; see the Global Constraint). A `CancelToken` that is never
    /// cancelled makes every added poll a no-op, which is what keeps `batch_parity` byte-identical.
    pub fn as_router_stop(&self) -> fr_router::pipeline::RouterStop;   // scan ruling R3
}
...
pub struct Ctx<'a> {
    ...
    pub budget: fr_router::pipeline::RouterBudget,        // (verified at pre-flight — scan §1)
}
/// Spec §10's `RoutingResult`.
pub struct RoutingResult {
    pub stats: fr_router::score::BoardStatistics,          // (verified at pre-flight — scan §1)
    /// Scan ruling R4: `build_unrouted_report(board: &mut Board) -> String` returns the jar's own
    /// diagnostic **text** (`pipeline/unrouted_report.rs:23`). Structuring it here would be a
    /// second, port-only implementation of a method Plan 7 already pinned. The *count* comes from
    /// `pipeline.final_statistics.connections.incomplete_count`.
    pub unrouted_report: String,                           // build_unrouted_report — scan ruling R4
    pub drc_violations: Vec<fr_board::ClearanceViolation>,
    pub timed_out: bool,
    pub pipeline: fr_router::pipeline::PipelineResult,     // (verified at pre-flight — scan §1)
}
...
**Files:** `crates/fr-core/{Cargo.toml,README.md}`, `crates/fr-core/src/{lib,cancel,progress,ctx,pipeline,timespan}.rs`, `crates/fr-core/tests/{cancel,timespan,pipeline}.rs`; `Cargo.toml` (workspace members — **no edit needed**: `members = ["crates/*", "tests/parity"]` is a glob and picks `fr-core` up automatically; scan note N7); `crates/fr-settings/src/lib.rs:193` (the `parseTimespanString` marker — **consumed**); `crates/fr-board/src/board/clearance_override.rs:405-407` (`getRoutingBoard` / `replaceRoutingBoard` / `getCurrentRoutingJob` → `Ctx` — **three of the nine `added in Plan 8:` markers there; the other six are Task 3's**, scan ruling R5). **`crates/fr-router/src/lib.rs:243-244` is STRUCK: it holds no `added in Plan 8:` marker** — `:243` is `// not ported: DebugControl`, and `grep -n "added in Plan 8" crates/fr-router/src/lib.rs` is empty (scan ruling R5); `scripts/audit-map/{fr-core.map,freerouting.map}` (created, seeded); `scripts/differential/java/probes/P8T0Probe.java`.
...
**Interfaces consumed:** `fr_router::pipeline::{run_pipeline, PipelineResult, RouterStop, StopRequestState, RouterBudget, ProgressSink, RoutingEvent, NoopProgressSink, TaskState, build_unrouted_report}` (verified at pre-flight — scan §1); `fr_router::score::BoardStatistics` (verified at pre-flight — scan §1); `fr_drc::DesignRulesChecker` (Plan 5); `fr_settings::RouterSettings` (Plan 4); `fr_board::{Board, ClearanceViolation, TimeLimit}` (Plan 2).
...
/// (verified at pre-flight — scan §1); the MCP needs a sink whose writes go through a `Mutex`-guarded stdout
...
/// `TextManager.parseTimespanString` (util/TextManager.java:83-93). **[scan ruling R11 — the
/// draft's Java premise was false; read at pre-flight.]** The method is
/// `public static Long parseTimespanString(String)`: it returns the duration **in seconds as a
/// `Long`**, returns **`null`** on a `null`/blank input (`:84-86`), and **catches
/// `DateTimeParseException` itself** (`:90-92`) — it never throws. `Duration` is only an internal
/// intermediate (`Duration.parse(convertFromTimespanToDurationFormat(s))`, `:88`). The one call
/// site, `RoutingJobSchedulerActionThread.java:44`, has **no try/catch**; it null-checks at `:45`.
/// `Option<Duration>` is therefore the right shape for the wrong reason, and the probe records
/// `null` — not an exception class — for every malformed row.
pub fn parse_timespan(s: &str) -> Option<std::time::Duration>;
/// `threadAction:44-52` — parse, cap at `MAX_TIMEOUT` (`:24`, verified `24 * 60 * 60`), then
/// `job.timeoutAt = job.startedAt.plusSeconds(timeout)` (`:51`). **`GRACE_PERIOD` (`:25`, verified
/// `30`) is NOT applied here** (scan ruling R11): `:44-52` applies only the cap; the grace belongs
/// to the monitor thread and lands on `Deadline::timed_out_at`, never on `stop_at`.
/// Returns `None` when the string is blank/unparseable, which is Java's "no job timeout".
pub fn job_timeout_deadline(timeout_string: Option<&str>) -> Option<Deadline>;
```

**Transcription notes, each with its Java line.**
- **`parseTimespanString`'s grammar is 11 lines and it is not obvious.** `TextManager.java:83-93` — read it; do not infer it from the strings `"01:00:00"` and `"1.5"`. The grammar lives in `convertFromTimespanToDurationFormat` (`:101-118`), which is what turns `HH:mm:ss` / `mm:ss` / `ss` into an ISO-8601 duration string; `parseTimespanString` is only the `Duration.parse` + null-on-failure wrapper (scan ruling R11). `P8T0Probe.java` pins 30 inputs including `"1:30:00"`, `"90"`, `"1.5"`, `"1:2:3:4"`, `"x"`, `""`, `"25:00:00"` (over the cap), `"-1"`, `" 1:00 "`, and whatever the method's own regex/split rejects.
- **`MAX_TIMEOUT` and `GRACE_PERIOD` are literals at `:24` and `:25`** — transcribe the values from the file, do not use "24 h" and "30 s" from this plan. (Pre-flight read them as `24 * 60 * 60` and `30`; re-read anyway.)
- **The monitor thread is NOT ported** (quirk label Y). Its *observable* effect is two instants: `stop_at` (when `requestStop()` fires — `ALL`, quirk #201, which is why `AutorouteBatchLoop:251-253`'s `requestStopAutoRouter()` is always a no-op) and `timed_out_at` (30 s later, when `job.state = TIMED_OUT` is written). `Deadline` carries both; **`poll_deadline`'s one production site (`batch_loop.rs:303`) plus the polls this task adds** (scan ruling R3) are where they bite.
...
- **Scan ruling R5 corrects this bullet.** `crates/fr-dsn/src/lib.rs:12`'s `SessionToEagle` line is real and this task consumes it (re-word to `// not ported:`). `crates/fr-router/src/lib.rs:243-244` **does not exist as a marker** and is struck. What this task actually consumes beside `fr-dsn:12` is `crates/fr-board/src/board/clearance_override.rs:405-407` (`getRoutingBoard`, `replaceRoutingBoard`, `getCurrentRoutingJob` → `// renamed:` to `fr_core::Ctx`'s fields). The two `NamedAlgorithm.job` markers at `crates/fr-router/src/pipeline/batch_autorouter.rs:158` and `pipeline/optimizer.rs:372` name **`core/RoutingJob`** and are therefore **Task 1's**, not this task's. `RoutingPipeline.createForGui` and the six `NamedAlgorithm` listener methods were already accounted for by Plan 7 (hand-off §4, ruling AK: *"`RoutingPipeline` is now wholly accounted for … so Plan 8 has no `RoutingPipeline` surface left to build"*). Do not delete a marker; re-point it with its reason.
...
- **Regression gate:** `cargo test -p fr-router --test batch_parity` (verified at pre-flight — scan §1) unchanged — the adapter must be invisible.
...
/// **Scan ruling R17.** The enum is `app.freerouting.io.FileFormat`
/// (`src/main/java/app/freerouting/io/FileFormat.java`), **not** a nested enum of `RoutingJob`, and
/// it has **nine** values — the draft's seven omitted `Frb` and `Scr`, both of which
/// `getFileFormat(Path):230-247` returns (`.frb`, `.scr`) and `getFileFormat(byte[]):175-177`
/// returns on the `0x00 0x05` magic. `Scr` is what quirk L's `-do out.scr` zero-byte path needs.
#[derive(Debug, Clone, Copy, PartialEq, Eq)] pub enum FileFormat { Unknown, Dsn, Frb, Ses, Rules, Scr, DrcJson, KicadDesignJson, KicadSessionJson }
impl FileFormat {
...
**Files:** `crates/fr-core/src/{stats_from_bytes.rs,stats_json.rs,lib.rs}`, `crates/fr-core/tests/stats.rs`; `crates/fr-router/src/score/mod.rs:51-53` (three `added in Plan 8:` markers — **consumed**) (verified at pre-flight — scan §1); `scripts/differential/java/probes/P8T2Probe.java`.
**Java:** `core/scoring/BoardStatistics.java (648)` — **`BoardStatistics(byte[], FileFormat)` `:436-552` (117)**, `countOccurrences` `:578-586` (9), `toString` `:589-591` (3, `GsonProvider.GSON.toJson(this)` — the JSON surface itself), the field block `:37-79` (for the serialisation order), the `host` fallback `:114-121` and the bounding-box assignment `:126-131`, `:385-386` (quirk label AK); `util/TextManager.unescapeUnicode` (used at `:121`); the **twelve** DTO classes' field order (`BoardStatisticsBoard`, `…Layers`, `…Items`, `…Components`, `…Pads`, `…Nets`, `…Connections`, `…Traces`, `…Bends`, `…Vias`, **`…ClearanceViolations`, `…Fanout`**) — scan ruling R16. **≈ 140 ported Java lines.**

**Interfaces consumed:** `fr_router::score::{BoardStatistics + its TWELVE DTOs}` — **scan ruling R16**: the committed type (`crates/fr-router/src/score/statistics.rs:37-69`) has **14 fields**, and the DTO set is the plan's ten (`Board, Layers, Items, Components, Pads, Nets, Connections, Traces, Bends, Vias`) **plus `BoardStatisticsClearanceViolations` (re-exported from `fr-drc`) and `BoardStatisticsFanout`**, plus the `Rectangle2DFloat` helper. `BoardStatisticsFanout`'s fields are plain `i32`, so Gson emits `0` rather than omitting — the null-omission rule does **not** apply to it; `fr_core::{FileFormat, PARITY_VERSION}` (Tasks 1, 0).
**Interfaces produced:**
```rust
...
impl BoardStatisticsExt for fr_router::score::BoardStatistics {         // an extension trait —
    ...
    /// never builds a board.                                           // (verified at pre-flight — scan §1)
...
- **`unescapeUnicode` runs on `host` at `:121`** — it is a `\uXXXX` decoder. **Scan ruling R15: Plan 7's copy landed but is PRIVATE** — `crates/fr-router/src/score/statistics.rs:596`, `fn unescape_unicode(text: &str) -> String`, with a `// totalized:` note at `:595` about `$`/`\` in `Matcher.appendReplacement`. **Make it `pub` behind a `pub seam:` marker and call it**; do **not** copy it, because a second copy needs the same `// totalized:` row and would drift. This is the second of the plan's two additive-and-wrapped `fr-router` changes (see the Global Constraints and scan ruling R3); `batch_parity` is the gate.

**Tests (`crates/fr-core/tests/stats.rs`):** the `P8T2Probe` table as literals, field for field, for **every corpus `.dsn`** (`tests/reference/fixtures.txt`'s stems), **every committed `.ses`** and every `tests/reference/<stem>/batch.ses` (verified at pre-flight — scan §1); `the_dsn_host_scrape_finds_nothing_on_a_real_dsn` (quirk G); `layer_rule_is_counted_as_a_layer` (quirk F); `bends_keys_start_with_digits`; `component_outline_count_serialises_as_component_count`; `the_json_key_order_is_declaration_order_and_nulls_are_omitted`; **`the_fanout_and_clearance_violation_objects_serialise`** (scan ruling R16 — twelve DTOs, and `fanout`'s non-`Option` `i32`s emit `0` rather than being omitted).
...
### Task 3: the board load/save sequence, `prepare_board` wired in, and the `management/` audit (risk-2, SHRUNK by scan ruling R2)

> **Conditional scope — RESOLVED AT PRE-FLIGHT: SHRINK (scan ruling R2).** The scan ran the plan's own decision procedure, `grep -rn "apply_copper_to_edge\|apply_hole_clearance\|assign_hole_keepout" crates/`, and found **all three overrides already ported** by Plan 7 Task 15b (ruling AW), as `Board` methods in `crates/fr-board/src/board/clearance_override.rs`:
> `:102 clearance_override_board_units(&self, clearance_um: f64) -> i32`, `:130 apply_copper_to_edge_clearance_override(&mut self, clearance_um: f64) -> bool`, `:237 apply_hole_clearance_override(&mut self, clearance_um: f64) -> bool`, `:277 assign_hole_keepout_clearance_class(&mut self, clearance_um: f64) -> bool`; the driver is **`fr_router::pipeline::prepare_board(board: &mut Board, settings: &RouterSettings) -> bool`** (`crates/fr-router/src/pipeline/board_prep.rs:57`).
> **`crates/fr-core/src/overrides.rs` is STRUCK** and this task re-implements nothing. It **consumes `Board::apply_*` and `prepare_board` by name** and its remaining job is wiring + audit. Plan-7 contract 6 is why the wiring is not optional: *"`pipeline::prepare_board` before routing (ruling AW). 15 of the 16 corpus boards are mutated by `applyCopperToEdgeClearanceOverride` at default settings. **Skipping it routes a different board, silently.**"* Quirks **AC** and **AD**'s override halves are Plan 7's (#231, #232) and are **re-cited, not re-landed**; what remains genuinely open is AD's *double-application* (the parser-side call at `Structure.java:1268` has no port counterpart) and quirk #232's boundary (scan ruling R8).

**Files:** `crates/fr-core/src/{load.rs,save.rs,lib.rs}`, `crates/fr-core/tests/{load,overrides}.rs`; `crates/fr-dsn/src/parser/wiring.rs:596` (plan-6 §11.5 — `read_via_scope` stops calling the unchecked `insert_via`); `crates/fr-board/src/board/clearance_override.rs:399-407` (**nine `added in Plan 8:` markers** — seven are this task's `load`/`save` methods; `getRoutingBoard`/`replaceRoutingBoard`/`getCurrentRoutingJob` are Task 0's `Ctx`, scan ruling R5) and `:60` (the sixth Plan-8 `obligation:`, scan ruling R8); `scripts/audit-map/fr-core.map`; `scripts/differential/java/probes/P8T3Probe.java`. **No `overrides.rs`** (scan ruling R2).
**Java:** `management/HeadlessBoardManager.java (1011)` — ctor `:168-170` (3), `getRoutingBoard` `:255-257` (3), `replaceRoutingBoard` `:276-278` (3), **`createBoard` `:310-344` (35 — called *by the DSN parser*, `io/specctra/parser/Structure.java:1268`)**, **`applyHoleClearanceOverride` `:346-396` (51)**, **`assignHoleKeepoutClearanceClass` `:404-464` (61)**, **`applyCopperToEdgeClearanceOverride` `:466-552` (87)**, `getCurrentRoutingJob` `:570-572` (3), `calculateCrc32ForBoard` `:607-618` (12), `loadFromSpecctraDsn` `:673-705` (33), `applyParsedBoardResult` `:711-737` (27), **`applyRouterSettingsForLoadedBoard` `:739-749` (11)**, `applyImmediatePostLoadProcessing` `:751-757` (7), `loadFromKiCadJson` `:794-823` (30), `saveAsSpecctraSessionSes` `:862-877` (16); `management/BoardLoader.java (57)` — `loadBoardIfNeeded` `:19-56` (38, incl. the "only DSN and JSON" guard `:31-37`; the `//` comment is `:31`, verified at pre-flight); `settings/DefaultSettings.java` — `DEFAULT_HOLE_CLEARANCE_UM` `:81`, `DEFAULT_COPPER_TO_EDGE_CLEARANCE_UM` `:78`, **read only to name the constants the pins use**. The three override bodies (`:346-396`, `:404-464`, `:466-552`, 199 lines) are **struck from this task's budget** — Plan 7 Task 15b ported them (scan ruling R2). **≈ 170 ported Java lines.**

**Interfaces consumed:** `fr_dsn::{read_board, read_metadata, BoardReadResult, BoardMetadata, CoordinateTransform, DsnReadOptions}` (Plan 3); `fr_dsn::kicad::read_board` — **stubbed here, discharged by Task 9**, with an explicit `obligation:` marker at the stub (Plan 7 scan ruling 6's precedent: no unannounced forward references); `fr_settings::{resolve_headless, SettingsInputs, RouterSettings}` (Plan 4); `fr_board::{Board, BoardRules::{get_hole_clearance, set_hole_clearance}, ClearanceMatrix}` (Plan 2); `Board::reduce_nets_of_route_items` (`crates/fr-board/src/board/query.rs:**1096**` — the plan said `:1070`; corrected at pre-flight (scan note N1). **Ported in Plan 2 and never called; this task is its first caller**); **`fr_board::Board::{apply_copper_to_edge_clearance_override, apply_hole_clearance_override, assign_hole_keepout_clearance_class, clearance_override_board_units}` and `fr_router::pipeline::prepare_board` (Plan 7 Task 15b — consumed by name, never re-implemented; scan ruling R2)**; `fr_core::{RoutingJob, BoardFileDetails}` (Task 1).
**Interfaces produced:**
```rust
...
// (no overrides.rs — scan ruling R2. `load` calls `fr_router::pipeline::prepare_board`, which is
//  Plan 7 Task 15b's driver over `fr_board::Board::apply_*`.)
...
**Transcription notes.**
- **`applyRouterSettingsForLoadedBoard` has FOUR steps and `fr_settings::resolve_headless` models only the first two.** Verified at pre-flight against `HeadlessBoardManager.java:739-749`: `setLayerCount` when it disagrees → `applyBoardSpecificOptimizations` → `applyCopperToEdgeClearanceOverride` → `applyHoleClearanceOverride`. `:741-744` and `:745` are Plan 4's, and `resolve.rs`'s module docs name exactly those lines. `:746`/`:747` are **board mutations** and are **Plan 7 Task 15b's `fr_router::pipeline::prepare_board`**. The port's `load` calls `resolve_headless` for the first two and **`prepare_board` for the last two**, **in Java's order** (scan ruling R2). Wiring `prepare_board` in is the whole of plan-7 contract 6 and the reason this task still exists.
- **Quirk label AD — the two overrides run TWICE on a DSN load.** Once from `createBoard` (`:342-343`), which the **parser** invokes at `io/specctra/parser/Structure.java:1268`, and again from `applyRouterSettingsForLoadedBoard:746-747`. Each can re-trigger `searchTreeManager.reinsertTreeItems()`. **Reproduce the double application**; a single call is a divergence whenever the first pass changes what the second reads. The port's DSN reader has no manager callback, so `load_from_specctra_dsn` calls **`prepare_board`** explicitly at the point `Structure.java:1268` would have and again after — and the code comment says exactly that. This is the half of quirk AD that Plan 7 Task 15b did **not** close (it ran the pair once, from the router side); it is this task's, and it is the reason `the_override_runs_twice_on_a_dsn_load` stays in the test list (scan ruling R2).
- **Quirk label AC is Plan 7's (quirk #231) — re-cite it, do not re-land it** (scan ruling R2). `:501-507`'s early return makes a *defaulted* 500 and an *explicitly requested* 500 behave differently, and `docs/plan-8-prep/evidence/job1-summary.md` measured the consequence: `usesDefaultEdgeClearanceValue` is always true on the corpus, so the early return turns entirely on `usesFallbackOutlineClass`, and **15 of 16 boards proceed**. `a_defaulted_500_and_an_explicit_500_diverge` moves to a *re-assertion through the CLI*, not a fresh port.
...
- **`assignHoleKeepoutClearanceClass` is `Board::assign_hole_keepout_clearance_class` and is already ported** (scan ruling R2). Job 1 measured it **never reached on the corpus** (gated on `configuredClearanceBoardUnits > 0`, merged default `0`) although six corpus boards carry 4–40 circular keepouts each — which is exactly why this task's pins run at `--router.hole_clearance_um ∈ {100, 500}`. **Quirk #83's J-then-I `ClearanceMatrix` indexing must not be "fixed"** here any more than in the KiCad reader (Plan 3 hand-off §Plan 8); assert the asymmetry survives, do not rewrite the method.
...
**Tests.**
- `crates/fr-core/tests/overrides.rs` (the *wiring* pins; the override bodies are pinned by `crates/fr-router/tests/clearance_override.rs`, Plan 7 Task 15b): the `P8T3Probe` table as literals — the clearance matrix row for the board edge, `holeClearance`, the reclassified keepout item ids and the search-tree leaf count, **before and after each override**, on a KiCad-exported fixture and on `--router.hole_clearance_um` ∈ {0, 100, 500}; `the_override_runs_twice_on_a_dsn_load` (quirk AD, asserted through a call counter); `a_defaulted_500_and_an_explicit_500_diverge` (quirk AC).
...
- **Scan ruling R8 — the sixth Plan-8 obligation, and it is this task's.** `crates/fr-board/src/board/clearance_override.rs:60` reads: *"obligation: Plan 8 (`HeadlessBoardManager.applyHoleClearanceOverride` second invocation) — **before Plan 8 exposes `router.hole_clearance_um` on a real CLI path**, pin the quirk-#232 boundary (non-default hole clearance > 0 with zero circular keepouts → Java's second run does one extra `reinsertTreeItems`, shifting internal tree order — the quirk-#229 divergence class) with a tree-op/order pin, or reproduce the second run."* Task 5's `--set section.field=value` and Task 6's settings chain **are** that exposure, so the trigger is live. `P8T3Probe` therefore also dumps the **tree-op order and the search-tree leaf count after the second invocation with `hole_clearance_um > 0` and zero circular keepouts**, and this task closes the obligation. Plan-7 §10 row 15b: *"quirk #232's boundary is promoted to a Plan 8 obligation rather than pinned here."*

- **Regression gate (mandatory from here on):** `cargo test -p fr-router --test batch_parity`, `scripts/differential/run.sh p6t1` on the five Plan 6 stems, `cargo test -p fr-dsn`, and `scripts/differential/sweep-p3t15.sh` at 525 MATCH + 5 XDIFF — pasted into the commit message.
...
**Steps:** (the conditional is already resolved — SHRINK, scan ruling R2) → `P8T3Probe.java` → the tables → `load.rs` (the four-step sequence, in order, with **`prepare_board`** as steps 3–4 and the double application at `Structure.java:1268`'s point) → `save.rs` → the `wiring.rs:596` fix → the quirk-#232 boundary pin (scan ruling R8) → the nine `clearance_override.rs:399-407` markers consumed (scan ruling R5) → `p3t15` + `p6t1` + `batch_parity` unchanged → the `management/` audit-map rows → fmt/clippy/test/audit → commit `feat(core): the board load sequence and the three HeadlessBoardManager clearance overrides`.
...
**Interfaces consumed:** `fr_core::{RoutingJob, BoardFileDetails, PARITY_VERSION}` (Tasks 0, 1); `fr_core::stats_json::to_gson_json` (Task 2); `fr_settings::json` (Plan 4, quirk #141's strictness split); `fr_router::score::BoardStatistics::normalized_score` — verified: a **method** taking `&ScoringSettings` (scan ruling R20).
**Interfaces produced:**
```rust
...
impl RoutingResultManifest {
    ...
    /// **Scan ruling R20 — two parameters added.** `normalized_score` is a *method* needing a
    /// `&ScoringSettings` (`crates/fr-router/src/score/normalized.rs:130`,
    /// `pub fn normalized_score(&self, scoring: &ScoringSettings) -> f32`), and `board_statistics`
    /// is the cached `PipelineResult::final_statistics`. The drafted signature could reach neither.
    pub fn from_job(job: &RoutingJob, input_path: &std::path::Path, output_written: bool,
                    exit_code: i32, now: &dyn Fn() -> String,
                    stats: &fr_router::score::BoardStatistics,
                    scoring: &fr_settings::ScoringSettings) -> RoutingResultManifest;
...
- **`board_statistics` and `normalized_score` are a FULL recompute** including two `DesignRulesChecker` runs (`:117-120` → `BoardStatistics.java:110-427`). Task 6 supplies the already-computed `PipelineResult::final_statistics` (verified at pre-flight — scan §1) — assert it equals a fresh recompute in a test, then use the cached one, and say so.
...
**Tests (`crates/fr-core/tests/manifest.rs`):** `the_key_order_is_declaration_order`; `nulls_are_omitted`; `fanout_and_optimizer_phases_are_empty_objects` (quirk E); `autorouter_duration_is_the_whole_job` (quirk E); `sha256_matches_the_nist_vectors`; **`a_connectionless_board_scores_zero_not_nan`** (scan ruling R20 — `normalized_score` answers `0.0` when `maximum_score <= 0.0`, while `calculate_score`/`maximum_score` **panic** on `None` weights or a `None` `connections.maximum_count`; `empty_board.dsn` is the stem that finds it); `an_unreadable_input_omits_the_sha256_key` (quirk W); `resolve_git_sha_walks_all_three_sources_then_unknown`; `no_trailing_newline`; `the_parent_directory_is_created`.
...
**Files:** `crates/freerouting/src/{main.rs,cli.rs,legacy.rs,logging.rs}`, `crates/freerouting/Cargo.toml` (+ `fr-core`, `fr-settings`), `crates/freerouting/tests/legacy_cli.rs`; **`crates/fr-settings/src/sources/{json_file.rs,mod.rs}` and `crates/fr-settings/src/lib.rs:116-118` (scan ruling R7 — `JsonFileSettings`, priority 10, `--settings <file>`)**; `docs/cli-legacy-flags.md` (the `.json` paragraph, ruling 14); `crates/fr-settings/src/sources/cli.rs:582` (the `added in Plan 8:` marker — **consumed**); `scripts/differential/java/P8T5.java`, `scripts/differential/rust/src/bin/p8t5.rs`, `scripts/differential/sweep-p8t5.sh`, `scripts/differential/{run.sh,README.md}`.
...
- **Plan ruling 10 at its site, as amended by scan ruling R19:** **two** parsers, not one. `legacy::rewrite` + `apply_command_line_arguments` follow P3 (`startsWith`, last-wins); `CliSettings` keeps P4's exact `switch` for `-mp`/`-mt`/`--router.*=` unchanged. The three early-parse divergences (`-dl` `equals` at `main:1056`; `-ll` first-wins at `main:1060-1065`; `--user_data_path` first-wins at `:944-956`) get one quirk row and a `// not ported:` naming the pre-bootstrap passes. Test `-mpx 5` **must not** set `max_passes` and `-decoy a.dsn` **must** set the design input — the two rules in one table.
- **Scan ruling R7 — `JsonFileSettings` is this task's, and the plan had not claimed it anywhere.** Plan 7's hand-off says it twice (§7 *"a gap the audit surfaced and nobody has claimed … **Plan 8's Task 5 (the CLI surface) is where it belongs**; it is not in the draft's task bodies today"*, and §12 item 3 *"It is the `--settings <file>` source. Plan 8's CLI task should take it"*). Today `crates/fr-settings/src/lib.rs:116-118` rosters its three methods `// not ported:` on spec §2 ("no persistent config file") and `merger.rs:24,62` reserve priority 10. **The controller has ruled it in: port `settings/sources/JsonFileSettings.java` (~40 lines, three public methods — `loadSettings :48-54`, `getSettings :65-68`, `getSourceName :70-73`, `getPriority :75-78`) as a real priority-10 source in `crates/fr-settings/src/sources/json_file.rs`, wired to `--settings <file>` on the native form and to `freerouting.json` in the working directory.** Quirk #141's strictness split governs its reader (`fr_settings::json`, `Strictness.LENIENT` on the textual pass — `JsonFileSettings.java:48-54` is exactly what `json.rs:85` names), and `JsonFileSettings.java:60` swallows a parse failure into an **empty `RouterSettings`**, one level of leniency the port reproduces. Re-word the three `// not ported:` lines to the port's site, add the source to `p8t4`'s matrix, and record in `docs/cli-legacy-flags.md` that **quirk label AJ still holds**: `mcp_server.stdio=true` read from `freerouting.json` is silently ignored by Java, and the port's `mcp` subcommand is the supported spelling.
...
- **The sequence, in Java's order, with nothing skipped:**
  ...
  12. `run_pipeline` (verified at pre-flight — scan §1);
...
- **`p8t1`** runs `run_jar` and `run_port` on **the argv recorded in each stem's `tests/reference/cli-<stem>/argv.txt`** for every stem of `tests/reference/cli-fixtures.txt` and asserts **(a)** the SES files are **byte-identical**, **(b)** the exit codes are equal, **(c)** `normalize_log` of both sides is equal. **No tolerance, ever** — a divergence is an `XDIFF` row in `crates/freerouting/README.md` with the first differing byte, the offending item and a one-line root cause.
- The stem set is **the eight Plan 7 batch stems** — verified present at pre-flight, each with a full `tests/reference/<stem>/batch.{ses,passes.jsonl,meta.txt,verify-driver.txt}` set (`router-rpi-splitter`, `router-j2-reference`, `router-ecc83-input`, `router-empty-board` in CI; `router-dac2020-bm01`, `router-tutorial-board`, `router-fanout-bm11`, `router-strict-drc-cnh` behind `#[cfg_attr(debug_assertions, ignore)]` + `FR_SLOW_PARITY=1`) **plus the two Plan 3/5 round-trip stems** `tutorial_board` and `Issue026-J2_reference`. Plan 7's CI/slow split carries over unchanged.
- **Scan ruling R1: `argv.txt` carries each stem's `-mp` cap, transcribed from plan-7 hand-off §1's table** — `router-rpi-splitter` 8, `router-j2-reference` 99, `router-ecc83-input` 8, `router-empty-board` 1, **`router-dac2020-bm01` 2**, `router-tutorial-board` 8, `router-fanout-bm11` 2, `router-strict-drc-cnh` 2. A bare `-de/-do` would run `router-dac2020-bm01` at unlimited passes, which quirk #234 proves **unmeasurable** (`-mp 3` → `score=841.0115`; `-mp 20` → `835.88336`; the same `-mp 20` under DEBUG logging → `841.0115` again). The cap is the honest bound, not a green result hiding a red one. `router-dac2020-bm01` **stays** in the set — controller answer 2 (§Controller answers) makes it the KiCad-exported override-exercising stem, and at `-mp 2` it is byte-reproducible.
- **`gen-cli-reference.sh`** is the sibling of `gen-batch-reference.sh` (verified at pre-flight — scan §1): it drives the **bare HEAD jar** (unlike Plan 7's generator, which needed a probe to disable the `optChangedArea` budget — **the CLI has no such need, because both sides run the budget live and the comparison is between two whole programs**), and writes `tests/reference/cli-<stem>/{argv.txt,route.ses,route.exit,route.log,manifest.json,meta.txt}` with the jar identity, `java -version`, the hash mode and every machine-specific prefix replaced. **State the budget difference from Plan 7 in the script header** so a reader does not think it was forgotten.
...
**Steps:** the `tests/parity` helpers → `gen-cli-reference.sh` → each stem's `argv.txt` with its `-mp` cap (scan ruling R1) → the references → the roadmap's five board classes searched for in the fixture corpus, those that qualify added to `cli-fixtures.txt` in the **slow** lane and those that do not recorded in the task report (scan ruling R21) → `route.rs`'s 16-step sequence → `p8t1` on the smallest stem → the rest → `p8t2` → the quirk rows → fmt/clippy/test/audit + `batch_parity` + `p6t1` → commit `feat(cli): freerouting route end to end — SES byte parity with the HEAD jar`.
...
**Interfaces consumed:** `fr_drc::{DesignRulesChecker::new, generate_report, report_to_json, DrcJsonFlavor, DrcReportOptions, DrcCoordinates, KiCadDrcReport}` (Plan 5); `fr_core::{load::load_board_if_needed, RoutingJob}` (Tasks 1, 3); `fr_dsn::{rules_reader::read, ses_reader::read}` (Plan 3); `fr_dsn::kicad::import_session` — **stubbed here, discharged by Task 10**, with an explicit `obligation:` marker; `fr_settings::{SettingsMerger, SettingsSource, priority}` (Plan 4); `fr_router::score::BoardStatistics::normalized_score` (verified at pre-flight — scan §1).
...
- **Quirk label C — the quality score uses a DIFFERENT settings merge from the router's** (`:342-352`): merge #1 with **only** `DsnFileSettings(drcJob.input)`, no `.rules`, no board pass, no merge #2. So `-dr x.rules -drc r.json` scores with weights the rules file never influenced while the *violations* were computed against the clearances it installed. **Reproduce it by composing `SettingsMerger` directly**, exactly as ruling AU's sparse-payload path does — **this is a second, independent user of the `resolve.rs:193` obligation** and the task must say so at the site so Task 12 does not think it is the only one.
...
- **The quality score becomes a COMPUTATION** (plan-5 ruling 5): `fr_router::score::BoardStatistics::normalized_score` (verified at pre-flight — scan §1) is an `f32`, widened to the report's `double` at `Freerouting.java:349` — which is why a value serialises as e.g. `999.9000244140625`. **Keep the `f32` computation and widen at the boundary.** The **eight committed `drc-*` references pin the values**, turning the injection into eight free acceptance cases.
...
**Java:** `io/kicad/KiCadJsonReader.java` — **`importSession` `:757-856` (100)**, the `.json` branch of `Freerouting.initializeDrc:301-307` that calls it (**`new FileReader(f)` = PLATFORM DEFAULT CHARSET**, quirk label U); `io/kicad/KiCadJsonWriter.java (227)` — `write(RoutingBoard)` `:27-31`, **`write(RoutingBoard, String designName)` `:32-226` (195)**; `management/jobs/RoutingJobSchedulerActionThread.java:275-278` (**the `-do out.json` call site — verified at pre-flight as the headless reachability that resolves controller answer 1 to PORT**: `:275 if (job.output.format == FileFormat.KICAD_SESSION_JSON)` → `:277 KiCadJsonWriter.write(job.board, job.name)`) and **`setJobOutput` `:259-295`** re-read for quirk label T. **≈ 330 ported Java lines, of which the writer is 195 (`KiCadJsonWriter.java:32-226`) and `importSession` is 100 (`:757-856`)** — the budget controller answer 1 asked for.
...
**Evidence.** New code, so **contract tests plus the documented-delta driver** (ruling General). The five deltas above — **plus the five from Tasks 12 and scan ruling R18: no progress, no cancellation, auth on by default, the blank-line desync, the id-less `-32700`, and the tool set (28 vs 4)** — are written into `crates/freerouting/README.md` **in this task**, each with the Java file:line beside it, so `p8t6` (Task 12) only has to assert they are still the deltas.
...
**Files:** `crates/freerouting/src/mcp/tools/{mod,route_board,check_drc,board_info,list_settings,schema}.rs`, `crates/freerouting/src/commands/info.rs`, `crates/fr-core/src/summary.rs`, `crates/freerouting/tests/mcp_stdio.rs`, `crates/fr-core/tests/summary.rs`; `crates/fr-settings/src/resolve.rs:193` (the `obligation:` — **closed**); `scripts/differential/java/P8T6.java`, `scripts/differential/rust/src/bin/p8t6.rs`, `scripts/differential/java/P8T7.java`, `scripts/differential/rust/src/bin/p8t7.rs`.
...
**Transcription notes.**
- **Ruling AU's sparse tier is the one hard part.** `route_board { settings? }` is **exactly** the priority-70 `ApiSettings` payload that `resolve_headless`' premise does not cover: an API job never runs merge #1, so the payload is *sparse* and merge #2's own `0..60` chain does the work. **Compose it from `SettingsMerger` directly**, per the `obligation:` at `crates/fr-settings/src/resolve.rs:193`, and **close that obligation here**. Note that Task 7's DRC quality score is a *second* direct composer (quirk label C); the two are different compositions and neither is `resolve_headless`.
...
- **File-path variants write to disk and return paths** (spec §13), keeping large SES bodies out of the model context unless `ses_text` is requested. `data` (Base64) is offered only when the caller asks for text, and the field is named as `BoardFilePayload` names it (`api/dto/BoardFilePayload.java:19-28`, `@SerializedName("data") public String dataBase64`).
- **Scan ruling R12 — there is no `base64` dependency and the Global Constraint forbids adding one.** `grep base64` over `Cargo.toml`, `Cargo.lock` and every crate manifest returns nothing; Java uses `java.util.Base64`. **Hand-write it**, exactly as Task 4 hand-writes SHA-256: RFC 4648 standard alphabet **with** padding, ~40 lines in `mcp/tools/mod.rs`, pinned by RFC 4648 §10's test vectors, with a comment saying why it is not a dependency. The two hand-written primitives (SHA-256, Base64) are a visible pair and Task 14's hand-off lists both under "deliberate divergences".
...
- **`p8t6` — the documented-delta driver (ruling AO).** `P8T6.java` drives the **jar's** stdio mode (`--mcp_server.stdio=true`) through `initialize`, `tools/list` and one `tools/call`, capturing every line; `p8t6.rs` drives the port's `mcp` subcommand with the same script. The driver **asserts the deltas are exactly the recorded list** — protocol version, `serverName`/`serverVersion`, `capabilities`, `ping`, `structuredContent`, the `\r`/`\n` stripping, the tool set (**28** OpenAPI-derived vs 4 flat — measured, scan ruling R18), the input shape (`{path, query, body}` vs flat), progress, cancellation, auth-on-by-default, the blank-line desync and the id-less `-32700`. **A NEW delta is a failure**; a delta that disappears is also a failure (the table is the contract). **Scan ruling R18 — the launch is a solved problem; import it verbatim from `docs/plan-8-prep/evidence/job3-summary.md` §1 and delete the `SKIP` escape.** `--mcp_server.stdio=true` **alone does not start the bridge**: `McpServerSettings.isEnabled` defaults to `false`, so `--mcp_server.enabled=true` is mandatory, and `--api_server.enabled=true` is effectively mandatory for every generated tool (`ApiServerSettings.isEnabled` also defaults to `false`, and generated tools are HTTP calls into the API). Job 3's working line is
```
...
```
A `SKIP` is now a **defect**, not a delta. The measured tool count is **28**, not 29, and job 3 adds three deltas the draft does not list — **auth on by default**, the **blank-line desync**, and the **id-less `-32700`** — bringing the recorded set to **ten**.
...
**Steps:** the four schemas + the anti-drift pair → `BoardSummary` → `board_info` + `info` → `list_settings` → `check_drc` → `route_board` (with the sparse composer, closing `resolve.rs:193`) → the end-to-end conversation → `p8t6` → `p8t7` → fmt/clippy/test/audit → commit `feat(mcp): the four tools, the hand-written schemas and freerouting info`.

---

### Task 13: the four zero-coverage Plan 3 paths — directed fixtures, no behaviour change (controller answer 5's authorised task, landed by scan ruling R13)

> **Why this task exists.** Controller answer 5 (§Controller answers, item 5) reads: *"The four zero-coverage Plan 3 paths: **15th task authorised** (small, coverage-only — directed fixtures per path, no behaviour change); the project hand-off must show zero known-uncovered ported paths or name each survivor with its reason."* The draft never wrote it — it dispatched fourteen tasks (0–13) and listed the four paths only under Task 14's §7 "Known limitations", which answers the *second* arm of an authorisation whose first arm was granted. The pre-flight scan landed the task (scan ruling R13). It is **coverage only**: a fixture and a test per path, **no port behaviour changes**, and any behaviour question it surfaces is a reported finding, not a fix.

**Files:** `crates/fr-dsn/tests/*` (the four directed tests, in the suites that already own each path), `crates/fr-dsn/tests/data/` (the four synthetic fixtures), `docs/plan-3-handoff.md` (the four rows' status lines).
**Java:** none ported. Each path's Java counterpart is **re-read to build its fixture** — the fixture must be what the *jar* accepts, verified by running the HEAD jar over it, not what the port's reader happens to tolerate.

**The four paths.** `docs/plan-3-handoff.md`'s zero-coverage list is the authority — **read it and take the four rows verbatim; do not take them from this sentence.** For each: name the path, the Java site, why Plan 3 could not cover it (no corpus fixture reaches it), and the directed fixture that now does.

**Interfaces consumed:** `fr_dsn::{read_board, read_metadata}` (Plan 3) and whatever each path's suite already imports.
**Interfaces produced:** none. **This task declares no type and changes no signature.**

**Transcription notes.**
- **A synthetic fixture must be accepted by the jar first.** Build it, run `java -jar <HEAD jar> -de <fixture> -do <out.ses>` (headless, standard flags), and commit the jar's exit code and output beside it. A fixture only the port accepts proves nothing about the port.
- **No behaviour change.** If a directed fixture makes the port answer differently from the jar, that is an `XDIFF` **reported to the controller**, not a fix in this task — the four paths were uncovered, so a divergence is a discovery and belongs in its own decision.
- **`sweep-p3t15.sh` is the gate: 525 MATCH + 5 XDIFF, unchanged** (the same number Task 3 holds). A fixture added to the corpus that changes the sweep's arithmetic must be added to the **directed** data directory instead, not to the corpus.

**Tests:** one named test per path, each naming its Java site and its fixture in the doc comment, plus `every_plan_3_zero_coverage_path_has_a_directed_test` — a list assertion so a future reader can see the four are closed.

**JVM-pinned evidence (required).** For each fixture: the HEAD jar's exit code, its SES (or its error), and the port's, committed as `crates/fr-dsn/tests/data/p8t13-directed-<path>.txt`. **Acceptance: all four paths reach a live test, the sweep is unchanged at 525 MATCH + 5 XDIFF, and any divergence is an `XDIFF` row with a root cause and a controller question.**

**Steps:** read `docs/plan-3-handoff.md`'s four rows → one fixture per path, jar-accepted → one test per path → the list assertion → `sweep-p3t15.sh` unchanged → the four status lines written into `docs/plan-3-handoff.md` → fmt/clippy/test/audit + `batch_parity` + `p6t1` → commit `test(dsn): directed fixtures for the four zero-coverage Plan 3 paths`.

---

### Task 14: audits to zero, the roster closed, the READMEs, the quirk register closed, and the PROJECT hand-off
...
```
**Scan ruling R22 — two surfaces the twelve invocations do not reach, confirmed by a sweep rather than by new invocations.** `settings/sources/JsonFileSettings.java` is under `settings/sources` (Plan 4's map) and `io/FileFormat.java` is under `io/` (Plan 3's map); neither is one of the twelve. Task 14 re-runs those two earlier invocations as part of the whole-repo sweep and records that **`FileFormat.java` is MAPPED** (scan ruling R17 ports it) and **`JsonFileSettings.java` is MAPPED, no longer `ROSTERED`** (scan ruling R7 ports it). The twelve stay twelve; the sweep is what proves nothing fell between two plans' maps.
...
```
**Scan ruling R5 — the inventory is 30 markers, not 23, and the checklist is the grep, not this list.** Measured on the committed tree (plan-7 hand-off §7): **32 hits / 30 markers proper** in `crates/*/src`, 0 in `crates/*/tests`. Their homes and owners: `crates/fr-board/src/board/clearance_override.rs:399-407` (**9** — six Task 3, three Task 0); `crates/fr-router/src/score/mod.rs:51-58` (**8** — three Task 2, five Task 14/ruling AS); `crates/fr-drc/src/lib.rs:114-119` (**6** — Task 8 takes `:114,116,117,119`, Task 10 takes `:115,118`) and `:144` (**1**, BoardComparator — **Task 14**, the ruling-AS twin of `fr-board/src/board/mod.rs:57`); `crates/fr-router/src/pipeline/{batch_autorouter.rs:158, optimizer.rs:372}` (**2** — `NamedAlgorithm.job` → `core/RoutingJob`, **Task 1**); `crates/fr-board/src/board/mod.rs:57` (**1**, Task 14); `crates/fr-dsn/src/lib.rs:12` (**1**, Task 0); `crates/fr-settings/src/lib.rs:193` (**1**, Task 0); `crates/fr-settings/src/sources/cli.rs:582` (**1**, Task 5). Prose mentions: `crates/fr-dsn/README.md:56` and `:62`, `crates/fr-router/README.md:665, 724, 2980, 3545, 3555, 3567` (the plan's `:2051,2053` was wrong — scan note N7). **`crates/fr-router/src/lib.rs:243-244` is struck; it holds no marker.** Every site the grep returns is **consumed** — ported, re-pointed to `// not ported:` / `// not reachable:` / `// renamed:` **with its reason**, or reworded to the past tense in prose. **Re-run the grep at port time**: Plan 7 will have added more. Also: **`grep -rn "EXIT_NOT_IMPLEMENTED" crates/` returns nothing** (Task 5's ruling AR reservation is discharged by Task 12 landing the last subcommand), and **`grep -rn "obligation:" crates/` is down by the SIX this plan closes** (`fr-settings/src/resolve.rs:193` — **not `:184`, scan ruling R6**; `fr-drc/src/report/json.rs:57`; `fr-dsn/src/parser/wiring.rs:596`; `fr-board/src/board/clearance_override.rs:60` — **the sixth, scan ruling R8**; `mcp/server.rs:9-32`; `mcp/stdio.rs:18-28`) with every survivor listed in the hand-off as a **closed-with-reason** row, not an open one — **there is no later plan to inherit an open obligation.**

**`docs/java-quirks.md`.** **[scan ruling R9 — amended at pre-flight]** The register is contiguous through **#235** (Plan 7 landed #194–#235); `docs/java-quirks.md:540` names **#236** as the next free id, and the pre-flight scan re-read it on the committed tree. **Ids are allocated contiguously from #236 in write order** (ruling Z). The 38 plan **labels** A–AL below live in a namespace of their own (`docs/plan-8-prep/survey.md:520-564`) and must not be confused with the controller's rulings AO–AV; every task that lands a row appends its label→id line to the amendment log. The table below gives **labels, not ids**; ids are allocated **contiguously, in the order rows are written**, and **every task that lands a row appends one line to the amendment log below mapping its plan label to the id it actually took**.
...
**Amendment log (plan label → landed id).** One line per row above, written by the task that landed it. The register's next free id, **verified on the committed tree at pre-flight, is #236** (scan ruling R9). Re-read the last row anyway before allocating — a sibling task may have taken it first.
...
7. **Known limitations** — spec §2's out-of-scope list restated as *what this program will never do*, plus quirk #113 (the `(string_quote .)` regex divergence Plan 3 could not close without a regex engine) and quirk #162 (the non-terminating `calculateNewIncompleteRooms`, still unguarded because a guard Java lacks breaks parity). **The four zero-coverage Plan 3 paths are NOT a limitation any more — Task 13 closed them** (scan ruling R13), so §7 states "zero known-uncovered ported paths" and cites Task 13's four directed tests rather than naming survivors.
...
`-XX:hashCode=2` is mandatory on `p8t3` (quirk #144's `HashMap`-ordered `unconnected_items`) and harmless elsewhere; the locale flags are mandatory on `p8t3` (quirk #145's `%.4f`) and on anything that formats a number. **[scan ruling R1 — amended at pre-flight.] Plan 7's `RouterBudget::disabled()` rule DOES apply.** The draft said both sides run their wall-clock budgets live; that cannot be reconciled with a byte gate. Java's four `TIME_LIMIT_TO_PREVENT_ENDLESS_LOOP = 1000` constants are `static final int` with constant initialisers, `javac`-inlines them, and **no flag or reflection can switch them off** (quirk #234) — so the jar side runs them whatever the port does, and above each stem's measured pass ceiling the *jar itself* is not reproducible. The port therefore runs **`RouterBudget::disabled()`**, Java's own "off" value, exactly as Plan 7 does. The comparison is sound because `gen-batch-reference.sh --verify-driver` measured **8/8 bare-jar identical with 0 budget trips** at each stem's cap: where the budget never trips, live and disabled are the same run. **Every `p8t*` driver header states this, with the trip count beside it**, and a stem that ever records a trip is an `XDIFF` citing quirk #234 — never a tolerance.

| driver | what it pins | invocation | task |
|---|---|---|---|
| **`p8t1`** | **the headline gate** — SES **bytes**, exit code, normalised stderr | each stem's `argv.txt`: `-de <dsn> -do <ses> -mp <cap>` vs `route <dsn> -o <ses> --max-passes <cap>` (scan ruling R1) | 6 |
...
| **`p8t6`** | **a documented-delta driver** (ruling AO) — the ten-row delta table, asserted to be exactly itself | `mcp` vs job 3's full launch line (`--mcp_server.enabled=true --mcp_server.stdio=true --api_server.enabled=true` + both `authentication.enabled=false`; scan ruling R18), driving `initialize` + `tools/list` + `tools/call` | 12 |
...
**The reference generator.** `scripts/gen-cli-reference.sh` (Task 6) is the sibling of `gen-batch-reference.sh` (verified at pre-flight — scan §1), sharing its `portable()` path rendering, its preflight and its `timeout(1)` bound (quirk #162 still does not terminate on ~0.4 % of room completions). Three modes:
...
## Hand-off checklist (Task 14 signs each line off in `docs/plan-8-handoff.md`)
...
- [ ] All **twelve** `audit-port.sh` invocations of Task 14 exit 0 with **zero `MISSING`, zero `UNMAPPED`**, run on the committed tree by the reviewer; the `ROSTERED` count for `api/**` recorded.
...
- [ ] The **six** obligations this plan closes are closed with evidence (`resolve.rs:193`, `fr-drc/src/report/json.rs:57`, `fr-dsn/src/parser/wiring.rs:596`, **`fr-board/src/board/clearance_override.rs:60`** (scan ruling R8), `mcp/server.rs:9-32`, `mcp/stdio.rs:18-28`), and **every surviving `obligation:` row in the register is CLOSED-with-reason, none open**.
- [ ] `#![forbid(unsafe_code)]` present in every crate root touched, **including the new `fr-core`**.
- [ ] **`p8t1` MATCH** on all eight stems **at each stem's recorded `-mp` cap** (scan ruling R1): SES bytes, exit code, normalised logs (ruling AV). The port ran `RouterBudget::disabled()` and every driver header records the jar's budget-trip count. Any `XDIFF` carries a root cause and the first differing byte.
...
- [ ] Plan 7's `cargo test -p fr-router --test batch_parity` and `run.sh p6t1` on the Plan 6 corpus — **five boards but SIX rows**, `router-dac2020-bm01-pass2` included (scan note N8) — still green on the committed tree; `sweep-p3t15.sh` still 525 MATCH + 5 XDIFF after Task 3's `wiring.rs:596` change.
...
- [ ] Ruling 3: exactly two `std::thread` **spawn sites** (scan note N9 — ruling 3's "one tool thread per `tools/call`" means N live threads from two spawn sites, not two threads), both under `crates/freerouting/src/mcp/`; `grep -rn "std::thread\|rayon" crates/fr-*/src` returns nothing.
- [ ] **Scan ruling R3: the `fr-router` poll seam is additive-and-wrapped and driver-pinned** — every pre-existing signature still exists, and `batch_parity`, `run.sh p6t1` (**all six rows**, scan note N8) and `sweep-p7t9.sh` are byte-unchanged. Same gate for scan ruling R15's `pub` on `score::unescape_unicode`.
- [ ] **Scan rulings R7, R12, R13, R17 landed:** `JsonFileSettings` is a live priority-10 source, Base64 is hand-written with its RFC 4648 §10 vectors, Task 13's four directed fixtures close the Plan 3 coverage gap, and `FileFormat` carries all nine `io/FileFormat.java` values.
...
Everything else is a hard dependency: Task 1 needs Task 0's `Error`/`PARITY_VERSION`; Task 2 needs 1; Task 3 needs 1 (and stubs 9); Task 4 needs 1, 2; Task 5 needs 0; **Task 6 needs 0, 1, 2, 3, 4, 5** and builds the `tests/parity` helpers every later driver uses; Task 7 needs 2, 3, 6 (and stubs 10); Task 9 needs 8; Task 10 needs 9 and discharges Tasks 6 and 7's stubs; Task 12 needs 6, 7, 11; **Task 13 needs 3 only** (it is `fr-dsn` coverage and shares no file with 4–12, so it may run beside anything from Task 3 on); Task 14 needs everything.

Sizing. **Task 3** (≈ 170 lines after scan ruling R2's shrink, but it wires `prepare_board` into the load path that Plans 3–7 all depend on, and plan-7 contract 6 says skipping it "routes a different board, silently"), **Task 6** (the plan's headline gate — 410 Java lines, sixteen ordered steps, five quirks and the harness every later driver consumes), **Task 8** (577 Java lines at the ceiling, and quirk #83 sits in the middle of it) and **Task 14** (twelve first-ever audits plus the project completion report) are the four largest: **opus, reviewed twice**. **Tasks 0, 1, 7, 9, 11, 12** are large: **opus** (Task 0 because the concurrency seam and the three-state mapping decide whether cancellation works at all; Task 1 because three of its methods are totalisations; Task 7 because the load order and the separate merge are both silently wrong if transposed; Task 9 because the two Java recovery boundaries are easy to invent instead of reproduce; Tasks 11 and 12 because they are the port's only concurrency and its only new protocol). **Tasks 2, 4, 5, 10, 13** are medium: **sonnet** — Task 2 because it is a transcription pinned field-for-field by a probe, Task 4 because the manifest is a DTO tree plus a hash, Task 5 because it deletes more than it writes, Task 10 because the writer is mechanical (its one judgement call, quirk T, is a *measurement* the steps prescribe), Task 13 because it is directed fixtures with no behaviour change.

Reviewers: **opus for 0, 1, 3, 6, 7, 8, 9, 11, 12, 14; sonnet otherwise** (Task 13, the coverage task scan ruling R13 landed, is sonnet). **From Task 3 on, every review re-runs `cargo test -p fr-router --test batch_parity` and `run.sh p6t1` on the five Plan 6 stems** (verified at pre-flight — scan §1), reading the header line to confirm which jar it used. From Task 6 on, every review additionally runs `p8t1` on the four CI stems **on the committed tree, not on the implementer's word**. Task 14's audits are run by the reviewer.
...
3. **Ruling AR reserves exit codes 2 and 3 for the native subcommand form, and Task 14's checklist then demands `EXIT_NOT_IMPLEMENTED` be unreachable.** That leaves **2** (clap's usage error) as the only port-only code, and **3** as a documented-but-unused reservation. **Default: keep 3 reserved and unused, documented in the hand-off's divergence table.** The alternative is to delete it and say the port has exactly three exit codes (0, 1, 2). Confirm — it is a one-line difference in a table a user reads.
...
## Banked evidence (2026-08-30, `docs/plan-8-prep/evidence/` — scan ruling R14)
