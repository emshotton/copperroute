...
- `Result`/`Option` where Java throws or returns `null`; `catch_unwind`/`Result` only at documented boundaries. **Plan 6's five boundaries (plan-6 ruling 7) each degrade to a specific value and the port must produce that same degraded value, not propagate**; Plan 7 adds exactly two more (ruling 7 below).
...
5. **`java_round`, `java_round_to_int`** wherever Java calls `Math.round` — half-up on ties, not banker's.
...
9. **Plan-6 ruling 7's catch boundaries degrade to a value.** Each of the five is a specific `AutorouteAttemptResult` or an unchanged room list, and the port produces that value. Plan 7's two additions (ruling 7) follow the same shape.
...
- **AM/General — no rayon; all five multithread classes `// not ported:`** with the survey's evidence (`autoroutePassMultiThread:412` has zero callers; `BatchOptimizerMultiThreaded` is reachable only from `BatchOptimizer.createForGui:56-66`). Quirk ids from **#189**. Task size ≤ ~600 Java lines. **Exact parity per pass is the target**; time-limited stopping points are compared with the budget disabled.
...
7. **Two new recovery boundaries, on top of Plan 6's five.** Java's `catch (Exception)` sites in Plan 7's scope that Plan 6 did not cover are `AutoroutePassRunner.java:144` (the per-item catch inside `runSingleThread`'s loop — degrades to "this item failed", the loop continues) and `AutorouteBatchLoop.java`'s `anyRoutable` throw path (`:51-56` — `TaskState.CANCELLED` plus an `IllegalArgumentException` that `RoutingPipeline.run` does **not** catch, so it escapes to the job scheduler). The port's `run_pipeline` answers `Result<PipelineResult, RouterError>` for exactly that second one and `catch_unwind`s the first. **`StackOverflowError` is caught by neither language (quirk #27) and stays fatal.** *Reason:* these are why Java finishes a board on which one item panics. *Cost if wrong:* one bad item aborts a whole fixture.
...
13. **One audit map, extended, and the audit must reach zero for six more directories.** `scripts/audit-map/fr-router.map` gains rows for `autoroute/pipeline/*`, `board/optimize/ViaOptimizer.java`, `core/{RouterCounters,StoppableThread,ProgressThrottler}.java` and `core/scoring/*`, with cross-crate rows for the `fr-board` and `fr-drc` additions in whichever form Task 1 of Plan 6 settled on (`../` glob or a sibling map — **match what is in the tree, do not re-litigate**). Task 17 runs `autoroute`, `autoroute/pipeline`, `board/optimize`, `core/scoring`, `board/facade` (the `RoutingBoard`/`RoutingBoardOperations` slices) and `board/trace` (the `PolylineTrace` slice) to **zero MISSING and zero UNMAPPED**. *Cost if wrong:* the audit is the only mechanical check that 7 500 lines arrived; weakening it is forbidden.
...
| `fr_router::score::{BoardStatistics, ScoringDtos…}`, `BoardStatistics::normalized_score` | Task 1 | 2, 10, 12, 13, 14, 15, 16 |
| `BoardHistory`, `BoardHistoryEntry` | Task 2 | 10 |
| `Board::structural_hash` (widened) + `Board::hash_decision_equal` | Task 3 | 2, 11, 12 |
| `RouterStop`, `StopRequestState`, `RouterBudget`, `RouterCounters`, `TaskState`, `NamedAlgorithmType` | Task 4 | 8–16 |
...
| `RoutingBoardExt::remove_items_and_pull_tight`, `BatchAutorouter::remove_tails` | Task 8 | 9, 10, 13 |
| `route_connection_full` (steps 1–8) | Task 8 | 9 |
...
| `RoutingBoardExt::fanout`, `FanoutComponent`, `FanoutPin`, `EscapeStatistics` | Task 11 | 12 |
| `BatchFanout::fanout_board`, `FanoutRunSummary` | Task 12 | 10 (discharges its stub), 15 |
| `ReadSortedRouteItems`, `BatchOptimizer::opt_route_item`, `BatchAutorouter::autoroute_passes_for_optimizing_item` | Task 13 | 14 |
| `BatchOptimizer::run_batch_loop` | Task 14 | 15 |
| `run_pipeline`, `PipelineResult`, `AutorouteUnroutedReport` | Task 15 | 16, 17 |

```rust
// ── crates/fr-router/src/pipeline/stop.rs (Task 4, ruling AI) ───────────────────────────────────
/// Port of `core.StoppableThread`'s three-state stop flag (core/StoppableThread.java:8, 23-42).
...
pub enum StopRequestState { None, AutoRouterOnly, All }          // :8
...
pub struct RouterStop {
    state: std::cell::Cell<StopRequestState>,                    // :10
...
impl RouterStop {
    ...
    pub fn request_stop(&self);                                  // :23-28  -> ALL
    pub fn request_stop_auto_router(&self);                      // :33-37  NONE -> AUTO_ROUTER_ONLY
    pub fn is_stop_requested(&self) -> bool;                     // :39-41  == ALL
    pub fn is_stop_auto_router_requested(&self) -> bool;         // :42     != NONE
...
pub struct RouterBudget {
    ...
    /// `BatchAutorouter.shouldFireBoardUpdate`'s 250 ms and `ProgressThrottler`'s 1000 ms
    /// (core/ProgressThrottler.java:8) — progress only, but a knob so a driver can pin them.
    pub progress_throttle_ms: i32,
}
impl Default for RouterBudget { /* 1000, 10000, 1000 */ }
...
pub enum RoutingEvent {
    TaskStateChanged { algorithm: NamedAlgorithmType, state: TaskState },   // NamedAlgorithm:60-72
...
/// Ruling 1(a)'s per-pass tuple. Not a Java type — Java scatters these across FRLogger lines —
/// so it carries a `// renamed:` note and the five Java sites each field is read from.
...
**Files:** `crates/fr-board/src/rules/{via.rs,mod.rs}`, `crates/fr-dsn/src/rules_reader.rs`; `crates/fr-board/tests/rules_round_trip.rs` (the existing `re_declared_via_info_re_points_the_existing_via_rule_unlike_java` test **inverts** — rename it and say so), `crates/fr-router/tests/control.rs` (the `rebuild_via_info` obligation block at `control.rs:381-400`).
**Java:** `rules/ViaRule.java (130)` — `list` `:21`, `appendVia` `:29-31`, `removeVia` `:34-36`, `viaCount` `:39-41`, `getVia` `:44-47`, `contains` `:55-62`, `swapVias` / the remaining accessors `:64-106`; `rules/ViaInfo.java (108)`; `rules/ViaInfos.java (88)` — `add`, `remove`, `get(name)`, `count`; `io/specctra/RulesReader.java:340-350` (`applyViaInfo`, 11). **≈ 337 Java lines re-read; ≈ 150 of port changed.**

**Interfaces consumed:** none (this is the first task).
**Interfaces produced:**
```rust
// crates/fr-board/src/rules/via.rs
/// Port of `rules.ViaRule` (ViaRule.java:16-129). **Owns** its `ViaInfo`s.
///
...
impl ViaRule {
    ...
    pub fn get_via(&self, index: usize) -> &ViaInfo;                    // :44-47
...
**Java:** `core/scoring/BoardStatistics.java` — the field block `:37-79`, the three delegating ctors `:81-108`, **the computing ctor `:110-435` (326)**, `isPinEscaped` `:555-576` (22), `calculateScore` `:597-616` (20), `getMaximumScore` `:619-621` (3), `getNormalizedScore` `:624-635` (12), the nested `BoardStatisticsFanout` `:638-647` (10); the DTOs `core/scoring/BoardStatistics{Board (15), Layers (14), Items (32), Components (11), Pads (11), Nets (14), Connections (14), Traces (49), Bends (20), Vias (24)}.java`. **≈ 520 ported lines.**
**Rostered here, not ported:** `BoardStatistics(byte[], FileFormat)` `:436-554` (119 — the SES/DSN text-scraping twin), `countOccurrences` `:578-587`, `toString` `:589-596`, `core/scoring/BoardScoreBreakdown.java` (192) and `ScoringWeightComparison.java` (232) — all `// added in Plan 8:` (ruling 4), each with the reader that makes it Plan 8's (the result manifest, the CLI's `--score` output).
...
- **`float` vs `f64` is load-bearing.** `calculateScore` (`:597-616`) and every DTO total are Java `float`; the intermediates inside the computing ctor are `double` and narrowed by `(float)` casts at specific lines (`:189-192`, `:255-256`, `:339`). Transcribe the type at each site — an `f64` that is never narrowed changes the score in the last bits, and `AutorouteBatchLoop:425`'s `> lastBestScore + 0.5` and `BatchOptimizer:220-230`'s `< improvementThreshold` are both threshold comparisons on it.
- **The unit normalisation is a HEAD-only correction with a 9-line comment explaining itself** (`:196-215`): `boardUnitToMmFactor = Unit.scale(1.0, board.communication.unit, MM) / (resolution > 0 ? resolution : 1)`, and the same for µm. Port it verbatim including the `resolution > 0` guard; `traces.totalLengthMm` is what `calculateScore` multiplies by `defaultPreferredDirectionTraceCost`.
- **`traces.totalWeightedLength`** (`:250-262`): `length * (halfWidth + board.clearanceValue(trace.clearanceClassIndex(), defaultClearanceClass, layer))`, **halved when `fixedState == SHOVE_FIXED`** (`:256-259`). `BatchOptimizer.optRoutePass:288` uses it as `minCumulativeTraceLength`, so this is a routing decision, not a report number.
...
- **The fanout block** (`:407-434`): walks `board.getSmdPins()`, counts `total` (netCount > 0), `alreadyConnected` (`pin.getUnconnectedSet(netNumber).isEmpty()`) and `escaped` (`isPinEscaped`), then sets `pinsToEscape = total - alreadyConnected`. Note it reads **net index 0 only** (`:415`) for a multi-net SMD pin — `// Java bug:` and a quirk row.
- `host` falls back to `"Freerouting," + FREEROUTING_VERSION` when the DSN carried none (`:118-121`) and runs through `TextManager.unescapeUnicode` (`:122`) — a `// not ported:` for the GUI text manager with the unescape transcribed inline (it is a `\\uXXXX` decoder; read it, do not guess).
- `getMaximumScore` divides by zero for a board with no connections; `getNormalizedScore`'s `max(0, x)` then yields `NaN`, not `0`, because `max(0, NaN)` is `NaN` in Java's `Math.max`. Reproduce with `java_max`. Quirk row — `empty_board.dsn` reaches it.
...
**Java:** `autoroute/BoardHistory.java (202)` — `MAX_HISTORY_SIZE` `:29`, fields `:31-34`, both ctors `:36-44`, **`add` `:47-79`**, `clear` `:82-84`, `contains` `:87-100`, `remove` `:103-111`, `getMaxScore` `:114-127`, **`restoreBoard` `:134-155`**, `restoreBestBoard` `:158-160`, `size` `:163-170`, `getRank` `:173-186`, the **private nested** `BoardHistoryEntry` `:188-201`; `autoroute/BoardHistoryEntry.java (33)` — the **top-level, shadowed, dead** twin. **≈ 235 Java lines.**
...
pub struct BoardHistory {
    ...
    max_history_size: usize,          // :31, default MAX_HISTORY_SIZE = 30 (:29)
...
pub struct BoardHistoryEntry {
    /// Java's `byte[] board = board.serialize(false)` (:196). Ruling 8: a `Board` clone.
    pub board: Board,
    pub hash: u64,                    // :197  board.getHash()  -> Board::structural_hash
    pub score: f32,                   // :198  new BoardStatistics(board).getNormalizedScore(...)
    pub restore_count: i32,           // :199
}
impl BoardHistory {
    pub fn new(scoring: &ScoringSettings) -> BoardHistory;                    // :36-38
    pub fn with_capacity(scoring: &ScoringSettings, max: usize) -> BoardHistory; // :41-44
    pub fn add(&mut self, board: &mut Board);                                 // :47-79
    pub fn clear(&mut self);                                                  // :82-84
    pub fn contains(&self, board: &Board) -> bool;                            // :87-100
    pub fn remove(&mut self, board: &Board);                                  // :103-111
    pub fn max_score(&self) -> f32;                                           // :114-127
...
**Transcription notes.**
- **`add` is two full score computations when the cap is reached and one when it is not** (`:47-79`): `contains` (hash) first, then — only at capacity — `new BoardStatistics(board).getNormalizedScore(...)` for the gate (`:56`), a linear scan for the worst entry (`:62-68`), an early return when `newScore <= worstScore` (`:72-75`), and then `new BoardHistoryEntry(...)` which computes the score **again** (`:198`). Under capacity there is **no score gate at all** — any distinct board enters. Reproduce both, including the double computation.
- **`getMaxScore` starts at `0`, not `-inf`** (`:116`), so an empty history answers `0`. `AutorouteBatchLoop:306` compares `bh.getMaxScore() > boardScoreAfter` with a strict `>`, so an empty history never triggers a restore — which is why the loop's `bh.size() >= STOP_AT_PASS_MINIMUM` guard at `:298` is belt-and-braces. Record it; do not "improve" it to `f32::NEG_INFINITY`.
- **`restoreBoard` mutates under a *read* lock** (`:139-141` takes `readLock`, `:143` sorts the list, `:150` increments `restoreCount`). Single-threaded, so unobservable — but it is the reason `getRank`'s answer depends on how many restores have happened, and `AutorouteBatchLoop:315-320` breaks the whole pass loop on `getRank(...) > BOARD_RANK_LIMIT`. Quirk row, and a test that pins the ordering dependence.
...
- `under_capacity_any_distinct_board_enters` and `at_capacity_a_worse_board_is_rejected_without_serialising`.
...
- `the_top_level_board_history_entry_class_is_unreachable` — a doc-only test asserting the roster line exists (`grep` in the test, as Plan 5 did for its roster).
...
**Java:** `board/facade/BoardSnapshotManager.java` — `serialize(boolean)` `:26-42`, `getHash` `:57-70`, `diffTraces` `:84-99`; `board/facade/BasicBoard.java:153-172` (the four delegating methods); the three **decision** sites: `autoroute/pipeline/BatchFanout.java:152-156`, `autoroute/BoardHistory.java:87-100` (`contains`) and `:173-186` (`getRank`). **≈ 120 Java lines.**
...
impl Board {
    ...
    /// BoardSnapshotManager.java:57-70), which is an **MD5 hex string over `serialize(true)`**.
    ///
    ...
    /// `BasicBoard.diffTraces` (BasicBoard.java:169-171 → BoardSnapshotManager.java:84-99) —
...
The `?` cells are the work: the implementer fills each with `yes` / `no` / `partially` from the current implementation (`snapshot.rs:210-243`) and, for every `no`, either widens the hash or records why the field cannot distinguish two boards the pipeline can reach. **The `Line` identity token must stay out** (plan-6 ruling AE: "no output, ordering, hash or serialised form can observe a token"), so widening must go through `Line`'s `PartialEq` fields, never its identity.
...
**Java:** `core/StoppableThread.java (43)` — `StopRequestState` `:8`, the flag `:10`, `requestStop` `:23-28`, `requestStopAutoRouter` `:33-37`, `isStopRequested` `:39-41`, `isStopAutoRouterRequested` `:42`; `core/RouterCounters.java (48)`; `core/ProgressThrottler.java (32)`; `autoroute/pipeline/NamedAlgorithmType.java (7)`; `autoroute/pipeline/TaskState.java (11)`; `autoroute/pipeline/NamedAlgorithm.java (126)` (the five identity accessors ported, the three listener lists rostered); `management/jobs/RoutingJobSchedulerActionThread.java:55-90` (36 — the monitor thread whose **effect** the deadline reproduces). **≈ 300 Java lines, of which ≈ 180 ported and ≈ 120 rostered.**
...
pub struct RouterCounters {
    pub pass_no: i32, pub queue_size: i32, pub incomplete_count: i32,
    pub items_routed: i32, pub items_skipped: i32, pub items_not_routed: i32,
    /* … transcribe the field list verbatim from the 48-line file */
}
```
...
- `ProgressThrottler` (32) is `// not ported:`: its `shouldUpdate()` is a 1 000 ms wall clock gating a progress event, and `ProgressSink` replaces the whole mechanism. `NamedAlgorithm`'s `getId`/`getName`/`getVersion`/`getDescription`/`getType` (`:20-28`) are ported as five `const`s on the two algorithm structs; `:30-96`'s three `List<Listener>`s and their `add`/`remove`/`fire` triples are `// not ported:` naming `ProgressSink` (ruling AK).
...
- `the_budget_defaults_are_javas_literals` (1000 / 10000 / 1000, each with its Java line in the assertion message).
- `router_counters_field_list_matches_java` — a doc-comment test listing the 48-line file's fields.
...
**Files:** `crates/fr-router/src/board_ext/{routing_board_ext,tightener/mod}.rs`, `crates/fr-board/src/{board/mod.rs,items/trace.rs}` (the four `// added in Plan 7:` markers become `// renamed:` pointing at `fr-router`, the `RoutingBoardExt` precedent Plan 6 Task 15b set); `crates/fr-router/tests/{opt_changed_area,connection_to_pin}.rs`; `scripts/differential/java/{P7T3.java,P7T6.java}`, `scripts/differential/rust/src/bin/{p7t3.rs,p7t6.rs}`.
**Java:** `board/facade/RoutingBoardOperations.java` — `optChangedArea` `:52-79` (28); `board/facade/RoutingBoard.java` — `optChangedArea` overload 1 `:151-161` (11), overload 2 `:171-190` (20); `board/optimize/TraceTightener.java` — **`optChangedArea(ExpansionCostFactor[])` `:121-169`** (49, the class's last unported method; marker at `crates/fr-router/src/board_ext/tightener/mod.rs:47`); `board/trace/PolylineTrace.java` — **`checkConnectionToPin` `:1013-1076` (64)**, **`correctConnectionToPin` `:1082-1245` (164)**, **`swapConnectionToPin` `:1252-1313` (62)**, and re-enabling the four call sites at `pullTight:844, 848, 853, 857`. **≈ 400 Java lines.**

**Interfaces consumed:** `RouterStop`, `RouterBudget` (Task 4); `TraceTightener::{get_instance, split_traces_at_keep_point, smoothen_end_corners_at_trace}` and `PolylineTraceExt::{pull_tight, pull_tight_with, pull_tight_with_engine}` (Plan 6 Tasks 15a/15b); `Board::{changed_area, join_graphics_update_box, overlapping_objects}` (Plan 2).
**Interfaces produced:**
```rust
// crates/fr-router/src/board_ext/routing_board_ext.rs — RoutingBoardExt gains three methods
pub trait RoutingBoardExt {
    /* … the five Plan 6 methods, unchanged … */
...
pub trait PolylineTraceExt {
    ...
    /// `PolylineTrace.checkConnectionToPin(boolean)` (PolylineTrace.java:1013-1076).
    fn check_connection_to_pin(board: &Board, trace: ItemId, at_start: bool) -> Option<ItemId>;
...
- **The trio re-enters through `pullTight`** (`PolylineTrace.java:841-861`), only when the polyline came back unchanged, the angle restriction is **not** 90°, and `board.rules.getPinEdgeToTurnDist() > 0` — then `swapConnectionToPin(true)` `:844`, `swapConnectionToPin(false)` `:848`, `correctConnectionToPin(true, …)` `:853`, `correctConnectionToPin(false, …)` `:857`, **each recursing into `pullTight` on success**. Plan 6 pinned `pinEdgeToTurnDist = -1` inside `FoundConnectionInserter.insertTrace:140-141` (restored at `:447`), which is why the branch was unreachable there and is reachable here: `optChangedArea` calls `pullTight` **outside** that window. Delete the marker block at `crates/fr-router/src/board_ext/tightener/mod.rs:775` and the four in `crates/fr-board/src/items/trace.rs:96-99`.
...
- The trio's `correctConnectionToPin` (164 lines) calls `combine()` and `insertTrace`, which route through `PolylineTrace.change` — the site quirk #74 is about. Ruling AE's contract (Conventions §7) applies at every `Polyline` construction inside it.
...
- `crates/fr-router/tests/connection_to_pin.rs`: one test per trio method per angle regime from `p7t6`'s literals; `the_trio_is_skipped_at_ninety_degrees`; `the_trio_is_skipped_when_pin_edge_to_turn_dist_is_not_positive`; `a_successful_swap_recurses_into_pull_tight`.
...
- **`p7t6 <mode>`** — `P7T6.java`, `package app.freerouting.board.trace;`, drives `check/correct/swapConnectionToPin` on hand-built traces at every `pinEdgeToTurnDist` × angle regime × start/end. Acceptance: **0 diffs, all modes**; then `p7t3 3` **0 diffs**.

**Steps:** `P7T3.java`/`P7T6.java` → the two Rust twins → tests from the Java output → implement `optChangedArea` with the `ViaOptimizer` arm stubbed → `p7t3 0-2` 0 diffs → implement the trio → `p7t6` 0 diffs → `p7t3 3` 0 diffs → fmt/clippy/test/audit → commit `feat(router): optChangedArea's tightener sweep and PolylineTrace's ConnectionToPin trio`.

---

### Task 6: `ViaOptimizer` part A — `optViaLocation`, `optPlaneOrFanoutVia`, `isWithinTolerance`

**Files:** `crates/fr-router/src/board_ext/{mod,via_optimizer}.rs`, `scripts/audit-map/fr-router.map` (re-point the `ViaOptimizer` row from `lib.rs`); `crates/fr-router/tests/via_optimizer.rs`; `scripts/differential/java/P7T4.java`, `scripts/differential/rust/src/bin/p7t4.rs`.
**Java:** `board/optimize/ViaOptimizer.java` — `optViaLocation` `:33-158` (126), `optPlaneOrFanoutVia` `:161-296` (136), `isWithinTolerance` `:719-732` (14). **≈ 276 Java lines.**

**Interfaces consumed:** `RoutingBoardExt::opt_changed_area` and `TraceTightener::opt_changed_area` (Task 5, whose `ViaOptimizer` arm this discharges); `PolylineTraceExt::pull_tight` (Plan 6 Task 15a); `Board::{pick_items, item_normal_contacts, move_drill_item}` (Plan 2 / Plan 6 Task 10b).
**Interfaces produced:**
```rust
...
- Both methods mutate via position through the drill-item mover; a failed move must leave the board **exactly** as it was (Java restores by re-inserting), and there is a `structural_hash`-before/after test for the refusal path.
...
**Steps:** `P7T4.java` → `p7t4.rs` → tests → implement → `p7t4 0/2` 0 diffs → fmt/clippy/test/audit → commit `feat(router): ViaOptimizer's optViaLocation and the plane/fanout arm`.
...
**Transcription notes.**
- Java overloads on the **arity and types** of the contact list, not on a mode flag; the port renames the three (`// renamed:` on each) because Rust has no overloading. `optViaLocation` chooses between them by contact count — read `:100-140` and reproduce the dispatch exactly, including which count falls to the general case.
...
pub struct BatchAutorouter<'a> {
    pub board_is_optimizer_owned: bool,          // :263 isOptimizerAutorouter
    pub remove_unconnected_vias: bool,           // :115 = !settings.isFanoutEnabled()
    pub with_preferred_directions: bool,         // :120
    pub start_ripup_costs: i32,                  // :123
    pub trace_pull_tight_accuracy: i32,          // :126
    pub fanout_timed_out: bool,                  // :95, written by AutorouteBatchLoop:173
    pub total_items_routed: i32,                 // :99
...
/// The whole of `AutorouteConnectionRouter.route` (AutorouteConnectionRouter.java:30-160):
/// [`route_connection`]'s steps 1-5 plus **steps 6-8** — `optChangedArea` on `ROUTED` (`:95-121`),
...
**Files:** `crates/fr-router/src/pipeline/{pass_runner,failure_log,item_route_result,airline,batch_autorouter}.rs`, `crates/fr-router/src/lib.rs` (delete the nine `// added in Plan 7: RoutingFailureLog.*` and twelve `ItemRouteResult.*` markers); `crates/fr-router/tests/{pass_runner,item_route_result}.rs`; `scripts/differential/java/{P7T1.java,P7T2.java}`, `scripts/differential/rust/src/bin/{p7t1.rs,p7t2.rs}`.
**Java:**
...
**≈ 456 ported Java lines**; rostered here: `AutoroutePassRunner.runMultiThread` `:40-149` (110), its five `log*` helpers `:338-487` (150), `BatchAutorouter.autoroutePassMultiThread` `:411-413` (3), `RoutingFailureLog.{shouldSkip, getUnroutableItems, hasUnroutableItems, clear}` (~35), `AutorouteAirlineCalculator:42-213` (172) — **≈ 470 rostered lines, every one with its caller evidence.**
...
pub struct ItemRouteResult { /* itemId, viaCountBefore/After, minCumulativeTraceLength,
                                traceLength, incompleteCountBefore/After — read the file */ }
...
/// (AutorouteAirlineCalculator.java:16-40) — the one method of the class with a live caller
...
11. **Ruling 7's sixth boundary**: `AutoroutePassRunner.java:144`'s `catch (Exception)` degrades one item to "not routed" and the loop continues; `catch_unwind` over the per-item body, with a test that injects a panic and asserts the pass still completes with the remaining items routed.
...
- `crates/fr-router/tests/item_route_result.rs`: the ladder's three rungs; `improvement_percentage_truncates_the_via_term` (the Java bug, with the value `BatchOptimizer:340-348` computes beside it); `compare_to_has_no_live_caller` (the roster assertion).
- **`BatchAutorouterDebugTest`** (ruling 14) ports into `crates/fr-router/tests/pass_runner.rs` with its Java file:line in the doc comment.

**JVM-pinned evidence (required).**
- **`p7t1 <dsn> <passNo>`** — `P7T1.java`, `package app.freerouting.autoroute.pipeline;`, HEAD jar, JDK 25, `-XX:hashCode=2`, headless flags, budget disabled both sides. Prints the `(item id, net no)` pairs of `getAutorouteItems` **in order, before any routing**, plus the `handledItems` set after each net index. Acceptance: **0 diffs on all six corpus stems × passes 1-3.**
...
**Interfaces consumed:** `AutoroutePassRunner::run_single_thread` (Task 9); `BoardHistory` (Task 2); `BoardStatistics` (Task 1); `RouterStop`, `TaskState`, `ProgressSink` (Task 4); `BatchAutorouter::remove_tails` (Task 8); `BatchFanout::fanout_board` (**Task 12 — stubbed here**).
**Interfaces produced:**
```rust
...
1. **`anyRoutable`** (`:44-50`): any `layerActive[i] && layers[i].isSignal`. False → `TaskState.CANCELLED` **and throw `IllegalArgumentException`** (`:51-56`) — ruling 7's seventh boundary: `RoutingPipeline.run` does **not** catch it, so the port's `run` answers `Err(RouterError::NoRoutableLayer)` and `run_pipeline` propagates. `empty_board.dsn` reaches it.
...
12. **The best-board restore** (`:298-344`): `if (bh.size() >= 8 || isStopAutoRouterRequested())` and then `if ((currentPass % 4 == 0 && currentPass >= 8) || isStopAutoRouterRequested())`; **`if (bh.getMaxScore() > boardScoreAfter)`** — a **strict** `>` (`:306`); `boardToRestore = bh.restoreBoard(3)` (`:307`); `null` → "was not able to improve" + `requestStopAutoRouter` + `break` (`:308-313`); `bh.getRank(boardToRestore) > 30` → `requestStopAutoRouter` + `break` (`:315-320`); else `router.board = boardToRestore; consecutiveNoImprovementPasses = 0` (`:322-334`).
...
- `the_dead_hash_set_is_not_allocated` (quirk cand. B — the roster assertion).
...
- `autoroute/pipeline/BatchFanout.java` — the ctor `:35-78` (44), `fanoutBoard` ×2 `:81-163` (83), `EscapeStatistics` `:591-606` (16), `FanoutPassStatus` `:609-622` (14), `FanoutRunSummary` `:625-629` (5), `FanoutProgressListener` `:578-583` (6, `// not ported:` → `ProgressSink`), **`Component` `:631-693` (63)** with its `compareTo` `:682-693`, **`Component.Pin` `:695-778` (84)** with its `compareTo` `:742-777`.
**≈ 459 Java lines.**

**Interfaces consumed:** `route_connection_full`, `RoutingBoardExt::opt_changed_area` (Tasks 5/8); `ViaRule` (Task 0 — `fanout` builds a **combined** rule at run time, which is exactly the path ruling H was measured on); `AutorouteControl::{new_for_net, rebuild_via_info}` (Plan 6 Task 8); `Board::structural_hash` (Task 3); `BoardStatistics::is_pin_escaped` (Task 1).
**Interfaces produced:**
```rust
// crates/fr-router/src/board_ext/routing_board_ext.rs
pub trait RoutingBoardExt {
...
pub struct FanoutComponent { pub component: ComponentId, pub smd_pins: BTreeSet<FanoutPin>,
...
/// `pinIndex` — quirk row, and the `double` comparison uses `java_min`/`java_max` semantics.
...
**Transcription notes for the ordering:** the `Component` ctor (`:642-678`) computes the gravity centre and builds the `TreeSet<Pin>` (`:673`); `sortedComponents` is a `TreeSet<Component>` over `board.components` **1-based, `1..=count()`** (`:52-61`). Both comparators are total with `final` keys — **confirm that against Java and record it** (ruling 5); if either turns out non-total, it is a `JavaTreeSet` site and the plan says so rather than the implementer deciding quietly.

**Tests (`crates/fr-router/tests/fanout_order.rs`):** `components_sort_by_pin_count_descending_then_id`; `pins_sort_by_the_selected_double_then_pin_index`; `an_unrecognised_sorting_order_falls_back_to_pin_index` (the quirk); `a_net_with_one_pin_gets_double_max_value`; `the_four_target_strategy_carries_the_ripped_set_into_the_second_attempt` (quirk cand. Q); `the_combined_via_rule_holds_both_sources` (Task 0's payoff); `targets_are_sorted_by_squared_midpoint_distance_stably`.
...
**Java:** `autoroute/pipeline/BatchFanout.java` — **`fanoutPass` `:166-506` (341)**, `maybePublishProgress` `:508-542` (35), `publishProgress` `:544-576` (33), and the `fanoutBoard` pass loop `:110-162` (53, whose skeleton Task 11 delivered). **≈ 409 Java lines.**

**Interfaces consumed:** everything Task 11 produced; `Board::structural_hash` (Task 3); `RouterStop` (Task 4); `ProgressSink` (Task 4).
**Interfaces produced:**
```rust
// crates/fr-router/src/pipeline/fanout.rs
/// Port of `autoroute.pipeline.BatchFanout` (BatchFanout.java:24-779) — the SMD escape pre-pass.
pub struct BatchFanout<'a> { /* sorted_components, settings, counters — the ctor's fields :35-78 */ }
impl<'a> BatchFanout<'a> {
    ...
    /// `&mut dyn ProgressSink` (ruling AK).
    pub fn fanout_board(board: &mut Board, settings: &RouterSettings, stop: &RouterStop,
                        budget: RouterBudget, progress: &mut dyn ProgressSink)
...
**Java:**
- `autoroute/pipeline/BatchOptimizer.java` — `containsOnlyUnfixedTraces` `:85-92` (8), **`optRouteItem` `:395-514` (120)**, `getCurrentPosition` `:520-525` (6), **the protected inner `ReadSortedRouteItems` `:563-659` (97)** with its `next()` `:573-654`;
...
impl ReadSortedRouteItems {
    pub fn new() -> ReadSortedRouteItems;                                    // :566-571
...
/// is `RoutingPipeline.createForGui:41` ← `gui/workspace/progress/GuiRoutingJobWorker.java:212`.
///
...
/// not ported: `BatchOptimizer.createForGui` — the GUI factory (`global-constraints.md`: no GUI).
pub struct BatchOptimizer<'a> { /* the field block :29-38, transcribed */ }
impl<'a> BatchOptimizer<'a> {
    /// `createForHeadless(RoutingJob, ...)` (:51-53).
...
- **`runBatchLoop`** (`:125-272`): `useIncreasedRipupCosts = true` (`:132`); `initialStats`/`initialScore` (`:135-138`); `deadlineMs = start + parseTimespanString(optimizer.timeoutString)` (`:153-160`, `RouterStop`'s deadline, ruling AI); fire `STARTED` (`:162-163`); then
  ...
  **`withPreferredDirections = currentPass % 2 != 0`** (`:200` — the passes *alternate*);
  ...
  `passImprovement = (after - before) / before` (`:208-210`);
...
**Java:** `autoroute/pipeline/RoutingPipeline.java (143)` — `StageListener` `:16-25` (10, → `ProgressSink`), the ctor `:32-37` (6), `createForHeadless` `:45-47` (3), the accessors `:50-78` (29), `run` `:81-85` (5), **`runRoutingStage` `:87-114` (28)**, **`runOptimizationStage` `:116-129` (14)**, `normalizeRouterAlgorithm` `:131-142` (12); `autoroute/pipeline/AutorouteUnroutedReport.java (80)` — `build` `:19-54` (36), `describeItem` `:60-79` (20); `autoroute/pipeline/BatchAutorouter.java:483-485` (`buildUnroutedConnectionsReport`). **≈ 225 Java lines.** Rostered here: `RoutingPipeline.createForGui` `:39-43` and `NamedAlgorithm`'s listener plumbing `:30-96` (67).
...
4. The fanout-only mode's settings mutation is the one place the port must **not** take `&RouterSettings` and be done: it needs an owned, temporarily-modified copy with a `// renamed:` note saying Java mutates and restores.
5. **Ruling 7's seventh boundary**: `AutorouteBatchLoop.run`'s `IllegalArgumentException` (`:51-56`) is **not** caught here, so `run_pipeline` propagates `Err(RouterError::NoRoutableLayer)`. A test on `empty_board.dsn` pins it.
...
- `finish_autoroute_is_called_exactly_once` (a counting `Board` wrapper or an assertion on the engine's state).
- `the_fanout_only_mode_sets_max_passes_to_zero_and_restores_it`.
...
while IFS='|' read -r stem dsn max_passes fanout optimizer; do
  "$JAVA_BIN" -XX:+UnlockExperimentalVMOptions "-XX:hashCode=$HASH_MODE" \
      -Djava.awt.headless=true -Duser.language=en -Duser.country=US \
      -Dfreerouting.opt_changed_area_ms=0 \
      -jar "$JAR" -de "$JAVA_DIR/$dsn" -do "$REF/$stem/batch.ses" \
      -mp "$max_passes" -mt 1 -oit 1 \
...
> **The `-Dfreerouting.opt_changed_area_ms=0` flag does not exist in Java.** Ruling AI requires the budget disabled on both sides, and the jar hard-codes `TIME_LIMIT_TO_PREVENT_ENDLESS_LOOP = 1000`. So `P7T9.java` — **not** the bare jar — is what the generator actually runs for the reference: it re-implements `RoutingJobSchedulerActionThread`'s eight lines of headless wiring (`:92-99`, `:259-266`) around `RoutingPipeline.createForHeadless`, reflects the constant to `0` in `AutorouteConnectionRouter`, `BatchAutorouter` and `RoutingBoard`, and calls `SesWriter.write` itself. **The generator's first job is to prove that is faithful**: with the budget left at 1000, `P7T9.java`'s SES must be byte-identical to the bare jar's `-de/-do` output on all five stems. That check is `gen-batch-reference.sh --verify-driver`, it runs in CI's slow lane, and its output is committed to `batch.meta.txt`. If it ever fails, the driver is wrong and no `p7t9` MATCH means anything.

**`tests/reference/router-fixtures.txt`** gains three columns — `max_passes|fanout|optimizer` — appended to the existing `stem|dsn|max_items[|ripup_pass_no]` rows so `gen-router-reference.sh` keeps reading it unchanged (it stops at field 4). The batch rows:
...
**`--verify-hash-modes`** regenerates each stem under `-XX:hashCode=0..4` and requires **five byte-identical SES files** — the direct check that Plan 6's premise still holds now that fanout and the optimizer are live. Plan 6's survey already verified this end-to-end on two boards; this task extends it to **all eight stems plus the fanout-heavy one** and pins the result in `batch.meta.txt`.
...
### Task 17: audits to zero, the roster, the README, quirks #189+, and the Plan 7 hand-off
...
```
plus **`grep -rn "added in Plan 7" crates/` returning nothing** (every marker is consumed, re-pointed to `// not reachable:` per ruling AJ, or rewritten to `// added in Plan 8:` **with the reason, never deleted**), and `grep -rn "item_tree_shape_ref\|item_tile_shape_ref" crates/fr-router/` still returning nothing (plan-6 ruling 10).
...
- `autoroute/events/**` (6 files, 130) — observers, replaced by `ProgressSink` (ruling AK).
...
- `AutorouteAirlineCalculator:42-213` (172) — reached only from `BatchAutorouterThread` (ruling 6's fresh grep).
...
**`docs/java-quirks.md` rows — numbering.** The register is contiguous through **#188** (Plan 6 Task 17b; re-check `docs/java-quirks.md`'s last row before writing, and if a late Plan 6 fix added more, renumber and say so in the commit message). **#189 is the first free label.** The table below gives **labels, not ids**: as in Plan 6's amendment block, ids are allocated **contiguously, in the order rows are written**, so the first Plan 7 task to write a row takes #189 and each later task re-checks the register's last row before writing. Every task that lands a row **must** append an amendment line here mapping its plan label to the id it actually took.

| plan label | what | Java site | task |
|---|---|---|---|
| **#189** | `getHash()`'s Javadoc says "an MD5 hash of the board trace state", but `serialize(true)` writes `getTraces()`, `getVias()` **and `itemList`** — the whole item graph. The wrong doc is what justified the port's original narrow hash. | `BasicBoard.java:163`; `BoardSnapshotManager.java:29-35, 56` | 3 |
| **#190** | `BoardStatistics`' fanout block reads **net index 0 only** for a multi-net SMD pin, so a pin escaped on its second net counts as unescaped. | `core/scoring/BoardStatistics.java:415` | 1 |
| **#191** | `getMaximumScore` divides by `connections.maximumCount`, so a board with no connections scores **NaN** — `Math.max(0, NaN)` is `NaN`, not 0. `empty_board.dsn` reaches it. | `BoardStatistics.java:619-635` | 1 |
| **#192** | `BoardHistory.getMaxScore` starts at `0`, not `-inf`, so an empty history answers 0 and the strict `>` at the restore gate never fires. | `autoroute/BoardHistory.java:116` | 2 |
| **#193** | `BoardHistory.restoreBoard` **sorts the list and increments `restoreCount` under a *read* lock**, so `getRank`'s answer depends on how many restores have happened — and `getRank` gates the pass loop's `BOARD_RANK_LIMIT` break. | `BoardHistory.java:139-155, 173-186`; `AutorouteBatchLoop.java:315-320` | 2 |
| **#194** | `autoroute/BoardHistoryEntry.java` (33 loc, public, `Comparable`, holding a live board and an `Instant`) is **shadowed** by `BoardHistory`'s own private nested class of the same name and is unreachable. | `BoardHistory.java:188`; `autoroute/BoardHistoryEntry.java` | 2 |
| **#195** | `maxItems` calls `requestStop()` (**`ALL`**) while `maxPasses` calls `requestStopAutoRouter()` (**`AUTO_ROUTER_ONLY`**), and `RoutingPipeline:117` tests `isStopRequested()` — so hitting `--max-items` **silently disables the optimizer stage** while hitting `--max-passes` does not. | `AutoroutePassRunner.java:219` vs `AutorouteBatchLoop.java:271`; `RoutingPipeline.java:117` | 4, 15 |
| **#196** | The job monitor calls `requestStop()` (`ALL`) **30 s before** it writes `job.state = TIMED_OUT`, so `AutorouteBatchLoop:251-253`'s `requestStopAutoRouter()` is always a no-op — the state only changes which `TaskState` is *reported*. | `RoutingJobSchedulerActionThread.java:75, 84`; `AutorouteBatchLoop.java:251-253` | 4 |
| **#197** | `RoutingBoardOperations.optChangedArea` compares `clipShape != IntOctagon.EMPTY` by **reference** on a value object; every router caller passes `null`, so the branch always runs. Same shape as quirk #34. | `RoutingBoardOperations.java:64` | 5 |
| **#198** | `retryConnectionNecked` reuses the **same `TimeLimit` object** the first attempt already consumed, so the necked retry inherits whatever budget is left — often none on a hard connection, making it a no-op precisely where it is wanted. | `AutorouteConnectionRouter.java:171, 204-214` | 8 |
| **#199** | The strict-DRC rollback assigns `router.board`, a field of `BatchAutorouter`, so `AutorouteBatchLoop.run`'s local `RoutingBoard board` (`:38`) goes stale. Latent — nothing reads it again. | `AutorouteConnectionRouter.java:250-252`; `AutorouteBatchLoop.java:38` | 8 |
| **#200** | `getAutorouteItems` appends an item **once per qualifying net index**, and the pass runner then loops over **every** net index of each appearance — a 2-net item that qualifies twice is routed four times per pass, and the inner index is a net index rather than the one that qualified. | `BatchAutorouter.java:363-403`; `AutoroutePassRunner.java:202, 207` | 9 |
| **#201** | `ItemRouteResult.improvementPercentage` divides `viaCountAfter / viaCountBefore` as `int / int`, truncating the via term to 0 or 1 while the trace term is a `double`; `BatchOptimizer:340-348` recomputes the same expression correctly with a `(float)` cast, so the field is wrong and the used value is right. | `ItemRouteResult.java:59-65`; `BatchOptimizer.java:340-348` | 9, 14 |
| **#202** | A **normal** end of routing reports `CANCELLED`, not `FINISHED`: every ordinary exit from the pass loop calls `requestStopAutoRouter()` first, so `:571`'s `if (!isStopAutoRouterRequested())` is false. Only a fully-routed board reaches `FINISHED`. | `AutorouteBatchLoop.java:271, 311, 318, 474, 505, 571-585` | 10 |
| **#203** | The `else if` that resets the stagnation counter for a fully-routed board hangs off `if (currentPass >= 8 …)`, so on passes 1-7 a routed board resets the counter and on pass ≥ 8 it does not — the opposite of the comment three lines above. | `AutorouteBatchLoop.java:422, 509-517` | 10 |
| **#204** | `alreadyRoutedBoardHashes` is a live `HashSet<String>` whose only two readers are commented out; it is still allocated per run and `.clear()`ed twice. | `AutorouteBatchLoop.java:249, 257-267, 334, 446` | 10 |
| **#205** | `RoutingBoard.fanout`'s "≤ 4 targets" strategy calls `autorouteConnection` **twice with the same `rippedItemList`**, so items ripped by the failed first attempt are carried into the second attempt's accounting and reported as this pin's rips. | `RoutingBoard.java:1064-1085` | 11 |
| **#206** | `BatchFanout.Component.Pin.compareTo` leaves `result = 0` for an **unrecognised** `pinSortingOrder` string, so the order silently becomes pure `pinIndex`; and `distanceToClosestOnNet` is `Double.MAX_VALUE` for a net with one pin. | `BatchFanout.java:711, 742-777` | 11 |
| **#207** | The fanout oscillation key is `((long) routedCount << 32) ^ vias.size()`, so two passes with the same `(routedCount, viaCount)` are treated as identical progress even when the geometry differs — and the XOR collides for large via counts. | `BatchFanout.java:133` | 12 |
| **#208** | `canUseVias`' whole gate is skipped when `net == null`, so a null-net SMD pin is fanned out with no via check at all. | `BatchFanout.java:239-249` | 12 |
| **#209** | `autoroutePassesForOptimizingItem` contains an **empty `if` body** whose `updatedRoutingBoard == null` test can never be true, because the same variable is dereferenced unconditionally 15 lines earlier. | `BatchAutorouter.java:256, 271-273` | 13 |
| **#210** | `ReadSortedRouteItems.next()` rescans the whole `itemList` **twice per returned item** — O(n²) per pass — and the "vias beat traces at the same coordinate" tie-break depends on the two loops sharing `currentMinCoor`, which the code does not make obvious. | `BatchOptimizer.java:573-654, 605` | 13 |
| **#211** | The whole multithreaded autoroute path is **unreachable**: `autoroutePassMultiThread` has no caller in `src/main` or `src/test`, so `RouterSettings.maxThreads` has no live reader and `BatchAutorouterThread` is a 621-line dead duplicate. Extends #143 ("`-mt` is dead *headless*") to "dead everywhere". | `AutoroutePassRunner.java:40-149`; `BatchAutorouter.java:411-413`; `BatchAutorouterThread.java` | 17 |
| **#212** | `RoutingFailureLog`'s documented `FAILURE_THRESHOLD = 50` give-up policy is never applied: `shouldSkip`'s only caller is the dead `BatchAutorouterThread:324`, and `getUnroutableItems`/`hasUnroutableItems`/`clear` have no callers at all. | `RoutingFailureLog.java:56-63, 70-87, 103-106` | 9, 17 |
...
| **`p7t6`** | the `ConnectionToPin` trio at every `pinEdgeToTurnDist` × angle regime × start/end | `board.trace` | `<mode>` | 5 |
...
| `--verify-driver` | runs the **bare jar**'s `-de/-do` and `P7T9.java` with the budget left at Java's 1000, and requires **byte-identical SES**; the result is stamped into `batch.meta.txt`. Without this, a `p7t9` MATCH proves only that the port agrees with a driver, not with freerouting |
...
**CI split (ruling AM).** `crates/fr-router/tests/batch_parity.rs` runs `router-rpi-splitter`,
`router-j2-reference`, `router-ecc83-input` and `router-empty-board` in normal CI (`java_dir`-gated,
skipping cleanly when the clone is absent, as Plan 5 established). `router-dac2020-bm01`,
`router-tutorial-board`, `router-fanout-bm11` and `router-strict-drc-cnh` are
`#[cfg_attr(debug_assertions, ignore)]` and additionally gated on `FR_SLOW_PARITY=1`, i.e. release
mode with the env var set. `scripts/differential/sweep-p7t9.sh` runs every stem × every mode and
...
- [ ] `grep -rn "added in Plan 7" crates/` returns **nothing**; every marker is consumed, `// not reachable:` (ruling AJ) or `// added in Plan 8:` with a reason.
...
- [ ] Quirk rows written, ids allocated contiguously from **#189**, and every plan label mapped to its landed id in an amendment block.
...
Three relaxations are available and the controller may take them: **Task 0** (`fr-board`'s
`ViaRule`) shares no file with Tasks 1–7 and can run in parallel with any of them — it only has to
land before **Task 11**; **Task 4** (the stop machine, the budget, `ProgressSink`) is pure and can
run in parallel with Tasks 1–3; and **Tasks 5–7** (the tightener sweep, `ViaOptimizer`, the trio)
share no file with Tasks 1–4. Everything else is a hard dependency: Task 2 needs Task 1's score;
Task 3 audits what Task 2 consumes; Task 8 needs Tasks 4 and 5; Task 9 needs Task 8; Task 10 needs
Tasks 2 and 9; Task 11 needs Tasks 0, 5 and 7; Task 12 needs Tasks 3, 10 and 11; Task 13 needs
Task 9; Task 14 needs Task 13; Task 15 needs Tasks 10, 12 and 14; Tasks 16 and 17 need Task 15.
...
**sonnet** for 2 and 6. **Tasks 4, 17** are small-to-medium: **sonnet**, with Task 17's audit run by
...
`cargo test -p fr-router --test reference_parity` (Plan 6's ladder — it must stay 369/369) and,
...
rayon and quirk #211 is the evidence. The spec's "Strategies as enums: item selection / board
...
listener plumbing (ruling AK); the five multithread classes (quirk #211, ruling AM); the JMX and
...
(quirk #212); `AutorouteAirlineCalculator:42-213` (ruling 6); `BoardStatistics`' file-scraping
constructor, `BoardScoreBreakdown` and `ScoringWeightComparison` (ruling 4 — Plan 8's);
`autoroute/BoardHistoryEntry.java` (quirk #194); the `alreadyRoutedBoardHashes` allocation
(quirk #204); every `FRLogger` call in 7 500 lines.
...
2. **Task 1 size.** A 19th task is authorised: split `fr_router::score` into 1a (`BoardStatistics` counters/traversal, Java-exact rounding) and 1b (`getNormalizedScore` + weights + `p7t7`).
