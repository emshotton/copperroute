...
Replace the Java-derived design rule checker in `copper-drc` with a native Rust
...
| Where the port lives | Inside `copper-drc`, behind the existing `DesignRulesChecker` façade. |
...
1. **`DrcConstraints` and `DrcSeverity`** are defined in `copper-board`
   (`rules/drc_constraints.rs`), alongside `merge`. The builders, `from_dsn`
   and `from_kicad_project`, and the constraint resolver live in `copper-drc`
...
3. The `drc` command, the `route` command's final report, and the MCP
   ...
   `--kicad-project` input.
4. `copper_settings::DesignRulesCheckerSettings` is not touched.
...
1. **Unit tests per check** in `copper-drc`, on small hand-written DSN fixtures
...
3. **Oracle fixtures.** A new script `scripts/gen-kicad-drc-reference.sh`
   ...
   `tests/reference/<stem>/`. A new `crates/copper-drc/tests/kicad_oracle.rs`
...
4. **Retired tests.** `reference_parity.rs`, `java_ports.rs`,
   ...
   router and CLI parity tests with `COPPERROUTE_REGOLDEN=<label>`; this branch's
   ...
   `COPPERROUTE_SLOW_PARITY=1`) are unaffected: no failure was observed against the
