...
2. Settings reach a run from dedicated flags, `--set section.field=value`,
   ...
   and the `COPPERROUTE_ROUTER_BUDGET` variable are dropped.
...
2. Parse the board (`copper_core::parse_board_if_needed`).
3. Resolve settings with `copper_settings::resolve_headless`, feeding
...
4. `copper_core::apply_router_settings_for_loaded_board` and
   `apply_immediate_post_load_processing`.
5. Re-read the rules file against the loaded board
   (`copper_dsn::rules_reader::read`), then the KiCad project
   (`copper_drc::apply_kicad_project`), then the session
...
| `drc <input> [-o <report>]` | `--rules`, `--ses`, `--kicad-project`, `--schema kicad\|copperroute` (default `kicad`) |
| `info <input>` | none |
| `mcp` | none |
...
`--router.x=y` spelling; `COPPERROUTE__ROUTER__*` environment variables;
`COPPERROUTE_ROUTER_BUDGET`.
...
`copper-settings` resolver is not changed by this work.
...
- The version written into DRC reports and manifests is
  ...
  `PARITY_JAR_REVISION` are removed from `copper-core`; `copper-drc`'s test alias
  follows the crate version.
- Routing output does not change. Every committed golden stays
  byte-identical; the only stems that may move are none, and `COPPERROUTE_REGOLDEN`
...
| item | action |
|---|---|
| `crates/copperroute/src/legacy.rs`, `tests/legacy_cli.rs` | deleted; `ExitCode` moves to `lib.rs` |
| `crates/copperroute/src/logging.rs` | `MESSAGE_MAP` and its test removed |
...
| `scripts/gen-cli-reference.sh` | deleted; goldens are cut with `COPPERROUTE_REGOLDEN=<label> cargo test -p copperroute --test cli_e2e` |
...
| `crates/copperroute/tests/cli_e2e.rs` | scenario tests translated to native argv; the log rung and the legacy-only rows removed; new rows for `--max-passes`, `--timeout`, `drc` exit code |
| `crates/copperroute/tests/mcp_stdio.rs` | message-text assertions updated where wording changes |
| `scripts/quality-ab.sh`, `scripts/gen-drc-reference.sh`, `scripts/gen-batch-reference.sh` | port invocations switched to the native form; `COPPERROUTE_ROUTER_BUDGET=disabled` becomes `--set router.opt_changed_area_ms=0`; jar invocations unchanged |
...
| `crates/copperroute/README.md`, `crates/copper-core/README.md` | updated to the new surface |
...
- Simplifying the two-merge precedence inside `copper_settings::resolve_headless`.
- Renaming or removing the `Java*`-named types in `copper-geometry` and
  `copper-router`.
