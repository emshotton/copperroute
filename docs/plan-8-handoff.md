...
## Errata — corrections landed after this plan merged

This document is the project's terminal report, so anything found after the merge is recorded here
rather than left to a commit message. Both rows below are **port defects, not Java quirks**: no new
register id, and quirk #44's own row carries the code-level history of the first.

| date | what was wrong, and what fixed it |
|---|---|
| 2026-09-02 | **The fanout target sort broke every distance tie the wrong way.** `RoutingBoard.fanout:1002-1021` sorts the pin's unconnected set by squared distance with a **stable** sort, so the seed order decides every tie, and Java seeds from a `TreeSet<Item>` — **descending** id, quirk #44. `board_ext::routing_board_ext::sorted_unconnected_targets` seeded a `BTreeSet` **ascending**, so on a tie the port escaped the pin toward the lower-id pad and Java toward the higher. Found by the post-merge benchmark outlier investigation (`.superpowers/sdd/2026-09-01-plan-8-core-cli-mcp/outlier-investigation.md` §5.2) on `pcbench-EncoderBoard_Enc_Pan_Led` (a symmetric `PCB_jumper_2`) and `pcbench-LVDS2TMDS_LVDS2TMDS`; both are byte-identical to the jar with the fix. The fix is one `.rev()`, plus the doc comment that had asserted the wrong premise and the `fanout_order.rs` test that pinned it. **§4's claim that the eight-stem `batch_parity` corpus is the port's strongest evidence stands, but it has a blind spot that was real, not theoretical: no corpus board contains a fanout distance tie, so all eight stayed byte-identical with the bug in place.** The blind spot is now covered by the directed, mutation-checked fixture `crates/fr-router/tests/fanout_tie_break.rs` + `tests/data/p8-fanout-tie.dsn` against the HEAD jar's own transcript |
| 2026-09-02 | **The job deadline was observed only at pass boundaries.** `RouterStop::poll_deadline` had one caller, `pipeline/batch_loop.rs` (`AutorouteBatchLoop:251`); the item loop at `pipeline/pass_runner.rs` (`AutoroutePassRunner:203`) called `poll_cancel` only, and `poll_cancel` carries no clock. So a `--router.job_timeout` run overshot by whatever remained of the pass that was running when the budget expired — measured on `zx-sizif-512-ext` at 300 s: 341 s, where the jar finishes at +8 s because its monitor thread flips the flag mid-pass. §6's Task-11 row says the residual latency is "one pass"; it was, and the deadline half of it is now closed. The second call site is one `RouterStop::poll_deadline`'s own doc already sanctioned — a **job**-level reader, so ruling AI's per-stage prohibition does not reach it — and it is byte-invisible on an untimed run because `deadline: None` makes the call a constant `false`. **Measured, artifact preserved:** `crates/fr-router/tests/data/deadline-overshoot.txt` — `zx-sizif-512-ext` at a 10 s budget, overshoot **+23.69 s → +0.94 s**, both binaries' sha256 checked to differ first (see `scripts/differential/README.md`, "A/B-ing release binaries under `sccache`"). Java's own gap is a **1000 ms monitor sleep**, not a pass, so the port now lands marginally tighter than Java rather than level with it |
