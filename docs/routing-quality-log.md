...
### Default corridor guidance: requested activation

The user requested corridor guidance on by default after reviewing its measured quality/performance trade. Implemented default-on with `COPPERROUTE_CORRIDOR_GUIDANCE=0` opt-out, keeping the existing final post-shove path check coupled to guidance. Protected-prefix routing is excluded. Two failing-first regressions cover environment behavior and default-path safety; the full workspace suite passed (2,608 checks including isolated child executions, 77 ignored, zero failed). Four-way full validation `quality-epyc-corridor-default-full-01` is running against main f9d4be5, including old opt-in and new opt-out controls. See `corridor-default-report.md` for final results.

Default activation validation completed: all 3,004 KiCad referees successful. New default vs main on 719 completed pairs is −36 unrouted / zero net copper; PCBench 709 pairs −39 U (29 better/12 worse), −1 copper (6 gainers), CPU ratio 1.0054; local 10 pairs +3 U (one worse), +1 copper (one gainer), CPU ratio 1.0285. Opt-in/new-default 723 completed pairs and old-default/explicit-opt-out 719 completed pairs have identical SES bytes, zero U/copper changes. Timing ratios are single-run observations. User-requested activation proceeds with the known per-board trade retained. Protected-prefix code is not included.

DRC parity follow-up: reproduced two missing unnetted-pad mask findings with independent KiCad 10.0.3 and 10.0.6 oracles; new test failed 0 vs 2 before removing the skip. Full copper-drc crate tests pass after the change. See `drc-parity-log.md`; no full parity or merge claimed. Structural full evaluation is running separately on root@185.8.106.150.

DRC mask accuracy validation: native survey attempts 751 boards, supports 549, and improves mask-count agreement on 119 of 529 uncapped boards with none farther from KiCad. Spatial pad-pair matches rise 0→4413 of 4509 KiCad findings; precision, report-coordinate and unsupported-feature limitations remain explicit. Fixed the six false Feather warnings by preserving logical pad identity. Final full DSN compatibility run has 751 successful KiCad referees: −274 U, −14 ordinary copper, +16 mask, CPU ratio 0.9599; all U/violation differences involve deadlines, and all 696 completed pairs have unchanged quality. Benchmark gate fails on six deadline-limited boards, with a no-optimizer one-pass control in progress. Workspace 2617 passed/0 failed/77 ignored; 52 web tests pass. See [full evaluation](drc-parity-evaluation.md) and [experiment log](drc-parity-log.md). This improves accuracy without claiming complete KiCad parity.

PR #34 deadline follow-up: all six one-pass/no-optimizer pairs completed without deadlines, with byte-identical SES and identical KiCad findings. The initial optimizer-enabled control was stopped deliberately and retained as incomplete. The full-run six-loss gate result remains in the report.

Isolated native pad-drill correction on fix/native-undrilled-pads after main f91d8496. TDD reproduced the invented drill radius (56.25 instead of zero), then passed after recording explicit zero drill metadata on single-layer undrilled pads. Existing drilled and multi-layer fallback handling stays intact. Full workspace test and isolated server build running; 656 source/build/test hashes match. Full KiCad compatibility run undrilled-full-01 is queued after the six routing variants. Native prototype control already completes Nano 2→0 without changing the four inherited pad-hole findings; isolated-source measurements remain pending. See undrilled-pad-evaluation.md.

Isolated undrilled-pad workspace suite completed: 2618 harness checks passed, zero failed, 77 ignored, terminal exit 0. Server build finished; isolated Nano result is zero unrouted with the same four inherited hole-clearance errors. Feather/full-corpus checks remain pending, so no commit or PR yet.

Isolated pad-fix Feather deadline control undrilled-native-02 completed both sides with byte-identical native board JSON: seven KiCad U and 35 routing errors, 322.28/319.85 CPU seconds. The first 300-second trial returned identical outputs but timed out; the completed 600-second control removes that uncertainty. Full undrilled-full-01 compatibility run remains queued.


### Isolated undrilled-pad full evaluation completed

751/751 valid KiCad scores, no failed/unjudged cells. Main→fix: 4853→4844 unrouted, 858→862 copper findings, 13747→13746 mask findings; 6 boards improve / 5 regress in unrouted, one copper gainer and one mask gainer, CPU ratio 1.0004. All changes involve deadlines; all 705 both-completed pairs have byte-identical SES outputs and unchanged quality. Seven automated quality-gate losses are retained in the generated report and receive a fixed-pass paired control before publication. Native evidence remains Nano 2→0 with unchanged KiCad findings, Feather byte-identical. See [undrilled-pad-evaluation.md](undrilled-pad-evaluation.md).
...
### Effective solder-mask expansion metadata

Independent KiCad 10.0.3 pair fixtures show one bridge versus zero when only the expansion of a copper pad without a declared mask opening changes 0.2→0 mm. Browser and Rust regression tests both failed before correction. Separate effective expansion metadata now survives import and pin copying without adding an aperture. All 17 mask tests and 53 web tests pass. Native-05 survey is building/running on workbench with exact input-difference and no-metadata behavior controls; no full-workspace or corpus-routing acceptance yet. See mask-effective-expansion-evaluation.md.

Effective expansion native-05 complete: all supported reimports differ only by the new field after design-name normalization, and the new binary on old inputs has identical counts to NPTH v1. Two boards improve and none regress by mask count: Baofeng +4 matched findings, Xmas Ornament +1. Pad-pair matches 4483/4509, candidate-unmatched 36 unchanged, missing KiCad pairs 26. Full workspace queued; fresh native Nano/Feather controls running; final cumulative 751-board routing evaluation queued behind the existing combined-mask run, gated on workspace success. No new PR or merge yet.

Native effective controls completed: Nano remains 0 U/4 inherited hole findings; Feather remains 7 U/35 routing findings plus 8 courtyard findings. All four jobs completed; each board's output JSON is byte-identical between main and candidate. Nano CPU10.64→10.68s/RSS14.86→15.15MiB; Feather245.51→246.36s/76.85→78.48MiB. No speed claim.

Rotated-pad audit isolated Sizif RN3 pads2/3 with exact original source geometry and project. KiCad10.0.4 reports no mask bridge. Native checker at10000units/mm reports one (0.3998mm vs0.4000mm); data-only100000 and1000000unit/mm probes report zero. Existing integer placement/rotated-corner rounding is a sufficient explanation on this pair. No global grid or epsilon change accepted. See mask-precision-investigation.md; full artifacts on workbench.

Capped-report follow-ups: Dropbot's one added NPTH/rear-pad finding is confirmed by an isolated KiCad10.0.4 check. Mechaduino's eight added layer findings were isolated with exactly the two HS copper pads and six affected U2 pads, all original tracks/vias and the original project: main13 mask, candidate21,KiCad21 (below cap). HS has overlapping opposite-copper pad objects declaring both mask sides, so the count includes KiCad's per-object multiplicity. Unrelated diagnostic dangling-track/padstack findings retained, not treated as routing results. No production changes. See mask-capped-report-audits.md.

Cumulative effective-mask full workspace passed:2622 passed,0 failed,77 ignored,exit0; 53 web tests already passed. Source/binaries remain frozen. Added a final routing report generator with all/PCBench/local splits, internal+external deadline handling, completed-state checks, separate copper/mask deltas and gainers, CPU/RSS, saved-SES hashes and regression details. Calibration against archived main/control/grid-end full runs reproduced every prior total and comparison field exactly (floating tolerance for sums). Final report is queued after the cumulative routing run and refuses incomplete/unjudged/mismatched board sets.

Referee diagnostic: both wxPython and pcbnew resolve the same wxWidgets3.2.9 shared libraries, so a mismatched-wx-library explanation for the GTK failures is unsupported. The importer calls wx.App(False) without retaining it; a real-process lifetime probe shows wx.GetApp() already None immediately afterward, while retaining the object keeps it alive through pcbnew import and forced collection. A copied importer retaining the app was compared against the unchanged importer on six previously failing saved routes, four repetitions each:24/24 imports succeeded on each side, and every wire/via/skip summary was identical. Thus the resource-lifetime issue is reproduced, but this small experiment does NOT demonstrate a reduction in intermittent GTK failures. No active benchmark source or routing binary changed. Artifacts:workbench provenance/wx-lifetime.

Referee follow-up on the active benchmark display:72 imports per variant reproduced2 original-importer failures (GTK) and1 retained-app failure (unable to access X display). Retention alone is insufficient. Separate fresh displays, one worker each, tested the unchanged importer96 times with default reset and96 with -noreset:both0 failures. This isolated trial did not demonstrate a reset-setting fix. No harness change accepted; all probes preserved under workbench provenance/wx-lifetime-shared-display and wx-display-{default,noreset}. The first reset-driver attempt had a Path/string TypeError before starting imports; corrected only after verifying it had exited.

All48 changed production/test files in the cumulative worktree have SHA-256 hashes matching the evaluated workbench snapshot; fingerprint manifest is committed-ready under routing-quality-artifacts/mask-effective/source-fingerprints.json. Private PR text is prepared, but no PR/commit is being submitted before full-routing validation.


### Layer-only routing validation and referee variability (2026-09-10)

The first full comparison is complete: 751 valid KiCad boards on each side. Main remains 7ecbeac250ff4b78877610dad3f71c4e96ba4c08 after a fresh fetch. The unmodified benchmark gate flags 48 routing-quality losses; this is not a passing routing evaluation.

Main/layer totals: unrouted 4585/5280 (+695); copper 874/886 (+12, three gaining boards); mask 13906/13832 (-74, seven gaining boards); fully connected 602/600; internal-or-external deadlines 29/62; CPU 33980.74/34392.29 seconds (1.0121); maximum RSS 295.2/295.0 MiB. Unrouted improves on one board and regresses on 23. All unrouted/copper changes occur on PCBench; the 11 local fixtures have unchanged quality. The generated split report and raw regression details are retained in routing-quality-artifacts/mask-effective/layers-full-routing-summary.{md,json}; the original gate report is layers-full-pr-summary.md.

All 689 both-completed pairs have byte-identical SES routes and identical unrouted/copper counts. All 676 both-completed pairs whose mask reports are below 199 have identical mask counts. The remaining capped mask differences cannot be advertised as quality improvements. The completed-pair CPU ratio is 1.1392, so this run also does not establish zero execution overhead.

Authoritative host inspection found an independent PR30 run (PID 1026389, /home/em/native-eval, pr30-native-vs-dsn-02, 12 workers) alongside the 12 mask workers. Many affected candidate jobs received only 170–200 CPU seconds before their 300-second wall deadline, versus roughly 300 CPU seconds on main. For example m2fc: main 1 U/11 Cu, 298.41 CPU seconds; layer 111 U/4 Cu, 190.69 CPU seconds; both wall times approximately 301–302 seconds. RoBoC: main 0 U/3 Cu completed at 190.64 CPU seconds; candidate 0 U/15 Cu timed out at 179.80 CPU seconds. This establishes differing CPU availability, not proof that every regression is solely contention. Do not stop or modify the independent run. No extra routing workers were launched for this investigation.

An exact-file referee audit ran KiCad 10.0.4 five times on the same saved badge2016 PCB/project without reimport or refill. Mask counts were 253, 254, 260, 254, 256; input hashes were unchanged. Thus high/capped KiCad counts can vary even on identical files. PCB_constant_current_ac_hv consistently returned seven shorting_items and one clearance on all five fixed-file checks. Its classification change across separate SES imports is not reproduced on one fixed file; UUID/order effects remain an unproven hypothesis. Full audit reports and script are preserved on workbench under copperroute-mask-results/provenance/referee-repeat-audit.

Known GTK SES-import failures are recovered automatically only after archiving the original cell and confirming its exact failure signature. Recovery uses the standard importer on a fresh X display, with unchanged SES hash and unchanged routing CPU/RSS/wall/timeout measurements. All first-run cells are now valid. Manual batch05 and subsequent automatic recovery evidence are preserved on workbench. Recovery is a measurement workaround, not an accepted fix for the underlying importer issue.

The combined and final cumulative full runs remain pending. No new production edits or merge were made from these results.


Follow-up import audit: PCB_constant_current_ac_hv main/candidate project files are byte-identical. Parsing both PCB files, removing only UUID/tstamp nodes recursively, and sorting top-level expressions yields identical canonical SHA-256 d02f6b023e8deed1ce7c5b95acb3655e0e187b6d46f46742c6cafb24bd5a96d3, with zero differing expressions. A fresh candidate DRC again gives one shorting_items/seven clearance versus main's seven shorting_items/one clearance. Thus the different classification is associated with object identity/top-level ordering, not different stored geometry or project rules. The audit does not distinguish identity from ordering as the causal variable. Evidence and normalization script: workbench provenance/referee-repeat-audit/import-comparison.

Queued mask-deadline-control-01 (Python PID1161377) after both the final routing report driver1091688 and independent PR30 evaluator1026389 finish. It selects the union of unrouted/copper regression boards against main from the complete report, then compares unchanged main, layer-only and cumulative effective binaries with one routing pass, optimization disabled, 1200-second cap, one thread and12 workers. This is a first-pass equivalence control, not a replacement for the full10-pass evaluation. No additional routing workers run while either dependency is live. Exports require inspection for valid KiCad scores and actual completed states; a successful script exit alone is insufficient. Scripts/config retained on workbench and under /tmp/quality-mask-effective locally.
...
Independent circular-clearance follow-up: isolated Sizif rounded-pad/via controls reproduce a false positive at normal10000units/mm (analytic0.205606mm gap, KiCad0 findings, old Rust1), while a closer0.158604mm case remains a real violation in both. Separate worktree fix/circular-copper-clearance reuses exact circular distance handling from hole checks. Three regression tests and the existing DRC package suite pass; workspace running. Its build/native-accuracy survey1181971 waits behind existing controls1161377; no added current workbench load and no change to frozen mask-effective-v1. No acceptance/merge or full routing result yet. See the separate worktree docs/circular-copper-clearance-evaluation.md.


Native accuracy coverage audit:549 imported boards and202 inherited initial-import failures. Largest exclusions:69 unclosed outlines,25 target objects on Edge.Cuts,15 footprint copper graphics,14 invalid/excessive coordinate reports. This is not an outlier classification. Later mask runs did not retry the unsupported subset; the report now states that explicitly. Full751-board KiCad routing compatibility coverage remains separate. See native-drc-coverage.md and its full board list.

### Combined mask routing validation (2026-09-10)

The intermediate layer-plus-overlap candidate completed all 751 boards with valid KiCad scores. The generated benchmark gate reports 54 routing-quality losses. Main remains the same measured 7ecbeac2 baseline. Full reports are retained as routing-quality-artifacts/mask-effective/both-full-{routing-summary.json,routing-summary.md,pr-summary.md}.

Against main: two boards improve unrouted count and 28 regress; total unrouted 4585→5254 (+669). Copper violations 874→884 (+10), with six gaining boards. Mask findings 13906→13864 (-42), with ten gaining boards; capped and variable reports prevent treating this aggregate as an accuracy improvement. CPU ratio 1.0255; maximum per-process RSS 295.2→295.0 MiB. Fully connected boards 602→600; partial boards 149→151. Actual completed states 722→683, deadlines 29→68.

PCBench: unrouted improves on two boards and worsens on 26, +653 total; copper +10 across six gainers; mask -42 across ten gainers; CPU ratio 1.0268. Local KiCad fixtures: zero improvements and two unrouted regressions, +16 total; unchanged copper and mask; CPU ratio 0.9679. These are observed routing results, not passing acceptance evidence.

All 682 pairs where both routers completed have byte-identical SES and equal unrouted/copper counts. All 669 such pairs below the mask-report cap have equal mask counts. Completed-pair CPU ratio is 1.1978, which does not establish zero checker overhead. Every one of the 30 distinct unrouted-or-copper regression boards involves a deadline. The largest unrouted regression, nonSNES, received 294.64 CPU seconds on main versus 197.40 on the candidate; m2fc received 298.41 versus 196.11. Contention therefore remains a material confound, not proof that the candidate is harmless.

The final cumulative mask evaluation has now started. The independent PR30 process 1026389 is absent, and a live process sample shows the final candidate's router workers near 99.5% CPU each. Keep the existing final evaluation and queued controls running; no extra routing pool was started. Production snapshots remain unchanged. No new commit or merge is justified yet.

Mask search-extent review: tested a potential missed-neighbor case with 0.2mm effective expansion, 0.1mm mask-to-copper clearance, and two 0.8mm circular pads 1.25mm apart; the source has no mask aperture. With minimum web0, native and KiCad10.0.4 both report zero mask findings. With web0.1, both report one. Thus this probe does not demonstrate a parity defect, and no search-radius patch is justified by it. Isolated KiCad inputs have no outline (both report invalid_outline); the wider-web control also has a silk finding. Native JSON was constructed from the existing regression fixture because the browser importer rejects missing outlines. Complete inputs/reports and summary are archived under workbench provenance/quality-mask-search-audit. Frozen production code is unchanged.

Final-candidate partial diagnostic at501 valid cells (not acceptance evidence): all488 both-completed pairs have identical SES. Two unrouted/copper regression boards were present: Blitz_.C68 main402U→candidate499U, copper0/0, CPU302.19/303.72s; Blitz_copy498U→499U, copper0/0, CPU303.00/302.43s. All four jobs report TIMED_OUT and one pass. Unlike earlier candidate runs, these pairs received similar CPU time, so contention alone does not explain the observed difference. The full report and queued deadline controls must assess them; neither is classified as an outlier. Partial raw metrics and script are archived on workbench as provenance/mask-effective-partial-diagnostic.json and inspect-effective-partial.py. Final totals remain pending.

Blitz_.C68 saved-route audit: main has wire/via entries on304 nets versus293 on the candidate. Geometry-item multisets differ on61 nets, including all28 /IA0–19 and /ID0–7 address/data nets. Many candidate nets retain a subset of main's wire/via items, but some nets also have candidate-only geometry; this is not proof of an identical route prefix stopped earlier. The difference is spread across a real bus, not merely one high-fanout net or a referee count anomaly. No outlier classification or algorithmic cause is inferred. Complete per-net wire/via differences and the script are archived under provenance/blitz-route-difference.json and compare-blitz-ses.mjs. The script changes only the root session tag in memory to use the existing S-expression parser; saved SES files are untouched. Fixed-pass completion controls remain required.

### Final cumulative mask routing evaluation complete (2026-09-10)

All751 boards have valid KiCad scores for all four arms; no unresolved referee failures remain. The final candidate's generated gate reports7 routing-quality losses, so this is not a passing gate. Main is still7ecbeac250ff4b78877610dad3f71c4e96ba4c08 after a fresh fetch. Complete split/per-board evidence and the unmodified generated PR report are retained as routing-quality-artifacts/mask-effective/full-{routing-summary.json,routing-summary.md,pr-summary.md}.

Main→effective: unrouted4585→4611 (+26), nine boards improve/four regress; copper874→873 (-1), one gaining board; mask13906→13914 (+8),14 gaining boards. Fully connected602/602; partial149/149. Completed722→724; internal/external deadlines29→27. CPU33980.74→32518.72s (ratio0.9570); maximum process RSS295.2→295.1MiB. Timing is a single-run observation, not a speed claim.

PCBench740: unrouted4355→4381 (+26,9 better/4 worse), copper816→815 (-1,1 gainer), mask13906→13914 (+8,14 gainers), CPU ratio0.9556. Local11: unrouted230/230, copper58/58, mask0/0, no quality gainers/regressions, CPU ratio1.0210. All722 both-completed pairs have byte-identical SES and equal unrouted/copper counts. All704 both-completed pairs below the mask cap have identical mask counts. Across all724 uncapped pairs, one mask gainer and net+9 findings remain.

The four unrouted regressions are Blitz_.C68 (+97), Blitz_copy (+1), karabas-nano-revB (+3), and karabas-nano-revG (+12). All are TIMED_OUT on both sides with similar CPU availability. nonSNES improves188→161 unrouted but gains one copper violation (0→1) and17 uncapped mask findings (11→28); both runs time out. These are actual outcome differences, not dismissed as outliers or explained away by contention. Saved-route comparisons establish completed-board compatibility, not harmlessness under deadlines.

The queued mask-deadline-control-01 has started33 boards per candidate (main, layer-only, effective), selected from the union of hard regressions across all three mask experiments. It uses one pass, optimization disabled and a1200-second cap. It is a completion/equivalence control, not a replacement for the full10-pass evaluation. Circular and rounded-track native surveys remain queued behind it. No new production edits, commit or merge yet.

nonSNES copper regression root-cause audit: KiCad's added clearance finding is /PA5 crossing a filled, unnetted F.Cu rectangle in U2's PQFP footprint (required0.125mm, actual0). The two1mm footprint rectangles, including the offending UUID cd681d3e-cb38-4696-bd6c-9f78d5bee1d0, are byte-identical in raw designer and candidate boards. The DSN image contains100 pins and these shapes only as outline paths, without their copper-layer/fill obstacle semantics. Do not interpret all component outlines as copper obstacles; that would also obstruct non-copper footprint outlines.

The native adapter already preserves fp_rect copper graphics as solid obstacles. Reimporting this exact candidate gives six obstacle polygons, but native DRC misses the /PA5/rectangle pair: its sole /PA5 finding is a different pad/track pair. Source inspection confirms the copper pair checker admits only pins/traces/vias; no general obstacle/copper-graphic clearance provider exists. This is a separate DRC coverage gap, not justification for ignoring the added KiCad violation. The native report contains88 clearance findings on this routed candidate; do not compare that with48 on the older native survey input as if it were a checker-only change. Exact import/report and scripts are archived under provenance/nonsnes-graphic-audit; original/candidate graphic records are in provenance/nonsnes-copper-graphic.json. A minimal isolated control and explicit physical-copper versus routing-keepout metadata are needed before implementing a correction. No outlier label or production patch applied.

Isolated nonSNES controls complete: one exact U2 copper rectangle and the /PA5 segment, plus an unrelated original U2 pad1 because native import requires a pad. Crossing case: KiCad10.0.4 reports1 clearance, native0. Translating only the track2mm right: both0 clearance. KiCad's unrelated library-footprint finding and native's dangling-track finding are retained separately. Original outline/project retained; no routing performed. The missing copper-graphic clearance check is now reproduced independently, with a clear control. Source scripts, native inputs, KiCad/native reports and isolated-summary.json are archived in provenance/nonsnes-graphic-audit. No production snapshot changed.


Mask completion controls finished99/99 valid, allCOMPLETED, no deadlines. Both candidates match main byte-for-byte on all33 SES files and unrouted/copper counts (66 comparisons). All23 uncapped mask pairs per candidate agree; remaining mask differences occur on capped identical routes. Main/effective control CPU4176.49/3729.66s (0.8930), RSS108.2/107.6MiB; not a speed claim. Includes bothBlitz, karabas-revB/revG andnonSNES. Full control reports archived locally and on workbench. All48 local production/test fingerprints still match evaluated sources; latest main remains7ecbeac2. Preparing the cumulative mask change as a separate accuracy PR, disclosing the failed7-loss full routing gate and +26U/-1Cu/+8mask deadline-dependent trade. First-pass equality does not claim full10-pass equality on timed-out boards. Circular native survey has started; rounded-track survey remains queued.

### Rounded copper corrections on merged mask main (2026-09-10)

PR36 merged as5c4a5c94 after full751-board comparison and99 completed controls. Independent circular native accuracy survey completed:35 supported boards closer to KiCad copper count,0 farther. Rounded-track survey:65 closer,0 farther. Both549 supported/202 inherited exclusions; neither result establishes individual-finding parity or full routing acceptance.

Created fix/rounded-copper-clearance on newmain5c4a5c94 and combined the two corrections. Fresh regression tests on newmain first fail on both exact KiCad controls (false clearance findings), then all five new tests and entire DRC suite pass with the combined change. Workbench baseline/candidate use separate source directories and targets. All22 production/test/fixture fingerprints are recorded. Driver1367443 runs workspace, builds, native accuracy, then gated full751-board routing with12workers. No algorithm experiment enabled; no full routing result or acceptance yet. See rounded-copper-clearance-evaluation.md.

## Combined survey completed; full routing started (2026-09-10)

The combined candidate passes 2,627 workspace tests, with zero failures and 77 ignored. Its own native survey completed all 751 entries: 549 supported, 202 inherited import exclusions, and all 549 supported copper reports uncapped. Copper counts are closer to the historical KiCad reference on 70 boards and farther on none; non-copper error counts are unchanged. This is count agreement, not item-level parity. The combined Sizif result removes all 70 native copper findings (KiCad has zero), leaving its 175 mask findings unchanged.

The validation gate passed and driver 1367443 started `rounded-copper-full-01` on workbench, comparing latest main 5c4a5c94 with the frozen combined candidate. Full routing results are pending; no routing-quality acceptance or commit yet.

Rounded-clearance baseline sanity: all 751 current-main cells are KiCad-judged, with 4,589 unrouted, 870 copper, 13,941 mask findings, 602 fully connected, 722 internally completed and 29 deadline cases. CPU 33,970.58 s; maximum peak RSS 295.2 MiB. The earlier equivalent mask-effective source produced 4,611/873/13,914, confirming the referee has not silently collapsed to zero. Across these equivalent-source runs, 729 SES files are identical and none of those have a copper-count difference; 17 identical SES pairs have mask differences, all at the reporting cap. Routing/deadline variability remains relevant to candidate interpretation. The full comparison is still pending the last candidate boards.

## Full routing evaluation complete (2026-09-10)

`rounded-copper-full-01` completed all 1,502 KiCad-judged cells and both driver 1367443 and referee monitor 1400151 exited. Main versus candidate on 751 boards: unrouted 4,589 → 4,553 (7 boards better, 5 worse, net −36); copper 870 → 872 (2 gainers, net +2); mask 13,941 → 13,942 (12 gainers, net +1). Among 724 uncapped mask pairs there is one gainer and net −1. CPU 33,970.58 → 32,567.98 seconds, ratio 0.9587; maximum process RSS 295.2 → 295.1 MiB. Fully connected 602 → 601; partially connected 149 → 150; completed 722 → 726; deadlines 29 → 25. This single comparison does not establish a speed improvement.

PCBench (740 boards) contains every quality delta: U 7 better/5 worse/−36, Cu 2 gainers/+2, mask 12 gainers/+1, CPU ratio 0.9596. The 11 local KiCad fixtures have identical SES and quality counts, CPU ratio 0.9186.

All 722 both-completed pairs have byte-identical SES, identical unrouted and copper counts. Their 704 uncapped mask pairs also have identical mask counts; completed mask differences occur only at the report cap. All connectivity/copper differences involve deadline cases. This supports a checker-accuracy interpretation, not a claim of improved routing algorithms, and does not erase the full-run trade-offs.

Hard regressions (both arms TIMED_OUT): Blitz_.C68 U425→432; bms-8s50-ic U55→56; decelerator4030 U475→492 and Cu10→11; karabas-nano-revC U89→93; dropbot-front-panel U0→2. The other copper gainer is real-time-chess U78→76/Cu70→72. The uncapped mask gainer is kitspace_T32_ref U0→0/mask179→181. These boards are not classified as outliers on that evidence.

Driver 1603676 now runs `rounded-copper-deadline-control-01`: all 13 boards with changed unrouted/copper or uncapped mask counts, both candidates, one pass, optimizer disabled, 1,200-second limit, one thread, 12 jobs. Controls require valid KiCad scores and internal COMPLETED state. Full reports (including the generated 75,347-byte benchmark PR report) are preserved in `docs/routing-quality-artifacts/rounded-copper`. No commit, PR or merge yet; investigate the individual regressions and matched-pass controls first.

Rounded matched-pass controls finished: all 26 cells completed and all 13 candidate/main SES pairs are identical. Both sides have 2,225 unrouted and 82 copper findings; all nine uncapped mask pairs match. CPU ratio 0.8689 is not claimed as a speed improvement. The full 10-pass trade-offs remain disclosed. Original chess geometry is clean and keeps about 0.85 mm around the inspected cutout, compared with about 0.21 mm in both autorouted deadline snapshots. Decelerator's extra finding is /A3 versus +5V (0.1279 mm versus 0.2 mm); its original has zero routing DRC findings. These are not dismissed as outliers. The standalone evaluation document now reflects the final evidence and the six-loss benchmark gate.

## After-optimization repair branch extraction — 2026-09-15

At user request, created `feat/after-optimization-repair` in isolated worktree `copperroute-after-optimization-repair`, from evaluated base `66612acf`. Extracted only terminal point-conflict repair: removed early/late hooks, topological alternatives, reservation modes, phase-selection machinery and temporary diagnostic output. The new flag is `COPPERROUTE_AFTER_OPTIMIZATION_REPAIR=1`; default behavior remains disabled. The final-phase timeout still propagates into PipelineResult. No main-checkout edits or default enablement.

The original full-run result is U4584→4506, copper869→871, reported mask13903→13936, CPU32672.23→32658.04 seconds. On723 both-completed pairs:7 fewer missing connections, no connectivity/copper regressions. Capped mask gains on normal pairs have byte-identical SES. Preserve these qualifications; do not claim the full deadline-sensitive78-connection change as the demonstrated mechanism gain.

Added `docs/after-optimization-repair-experiments.md`: all recent demand/target, combined/individual recovery, group/topology/conflict, reservation, preservation and terminal approaches, with headline and both-completed results, origin splits, original-board findings and negative decisions. Eleven supporting report/summary/CSV sets accompany it; older project history remains in this running log. No PR is opened or merge claimed.

Cleanup validation on workbench: `after-repair-cleanup-01`, all five normally completed full-run gain boards plus BMS, Sizif, Mouse, SRAMBO and FRM16. Workspace tests and release build precede ten KiCad scores, using300seconds/10passes/1thread/12jobs. Compare against frozen terminal prototype, not just main. Validation artifacts `/home/em/copperroute-after-repair-validation`. Commit and remote push follow successful validation.

Cleanup completed:2808workspace tests passed,0failed,77ignored;release build passed. All10KiCad checks valid and normally completed, all10SES byte-identical to frozen terminal prototype. Local/source validation hashes match. Verification and provenance saved in after-optimization-repair/cleanup-validation. Dedicated opt-in branch is ready for the user-requested push; no PR or merge performed.
