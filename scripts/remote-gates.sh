...
ssh "$HOST" "ln -sfn ~/freerouting gate-slots/freerouting; cd $DIR && git checkout -qf $SHA && ln -sfn ~/freerouting-bench/corpus/pcbench benchmark/corpus/pcbench 2>/dev/null; RUSTC_WRAPPER='' nix shell nixpkgs#gcc nixpkgs#jdk25 nixpkgs#cargo-nextest nixpkgs#python312 --command cargo build --release -q 2>&1 | tail -2"
for g in "$@"; do :; done
# run gates (single ssh per gate so output/exit are attributable)
run_gate() { echo "== gate: $* =="; ssh "$HOST" "cd $DIR && export JAVA25_HOME=\$(nix shell nixpkgs#jdk25 --command sh -c 'dirname \$(dirname \$(readlink -f \$(which java)))') && RUSTC_WRAPPER='' nix shell nixpkgs#gcc nixpkgs#jdk25 nixpkgs#cargo-nextest nixpkgs#python312 --command $1"  && echo "== exit: 0 ==" || { echo "== exit: FAILED ($?) =="; GATE_FAILED=1; }; }
i=1; args=("$@")
while [[ $i -le ${#args[@]} ]]; do
  g="${args[$((i-1))]}"
  case "$g" in
    ...
    quality-ab) tid="${args[$i]}"; i=$((i+1));
       ...
       extra="${args[@]:$i}"; i=$(( ${#args[@]} + 1 ))
       run_gate "bash -c 'set -o pipefail; bash scripts/quality-ab.sh $tid $extra 2>&1 | tail -10'"
       echo ">> pulling re-cut baselines back"
       rsync -az "$HOST:$DIR/benchmark/baselines/" benchmark/baselines/ ;;
