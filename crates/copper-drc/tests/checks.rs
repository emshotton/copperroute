...
use copper_board::prelude::FixedState;
use copper_board::{DrcConstraints, ItemId};
use copper_drc::checks::edge;
use copper_drc::checks::geometry::{
...
fn crossing_traces_are_tracks_crossing_and_nothing_else() {
    ...
    assert!(
        out[0]
            .position
            .distance(&copper_geometry::FloatPoint::new(0.0, 0.0))
            < 1.0
    );
}
...
fn a_trace_crossing_a_conduction_area_on_another_net_reports_nothing() {
    ...
    synthetic.board.insert_conduction_area(
        copper_geometry::Area::Shape(copper_geometry::Shape::Tile(TileShape::Box(
...
fn round_kicad_vias_do_not_have_square_corners_but_real_diagonal_gaps_fail() {
    ...
    for (intercept, expect_violation) in [(0.746, false), (0.67, true)] {
        ...
        let copper_dsn::BoardReadResult::Success {
            board: Some(mut board),
            ..
        } = copper_dsn::kicad::read_board(&json.to_string(), None)
        else {
            panic!("fixture import failed")
        };
        ...
        let ctx = board.ctx();
        let copper_board::Item::Via(via) = board.get_item(board.get_vias()[0]).unwrap() else {
            panic!("via")
        };
        assert!(matches!(
...
fn kicad_board(json: &str) -> copper_board::Board {
    match copper_dsn::kicad::read_board(json, None) {
