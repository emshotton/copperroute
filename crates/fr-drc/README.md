...
`fr_drc::prelude::*` to bring in every public type.

The crate sits on `fr-board` (the boards it checks) and on `fr-dsn` (for
...
not on `fr-settings`. `fr-drc → fr-dsn → fr-board → fr-geometry` is strict
...
`fr-drc` has no clock and no version constant, so everything the report
...
(`fr_router::score::BoardStatistics::normalized_score`) as a single-precision
...
(`DrcJsonFlavor::KiCad`, which the `freerouting drc` command uses by default)
or in a camelCase variant (`DrcJsonFlavor::FreeroutingHead`, selectable
with `--schema freerouting`). The two differ in exactly eight strings:

| field | `FreeroutingHead` | `KiCad` |
|---|---|---|
...
`crates/fr-drc/tests/report_json.rs::flavors_differ_only_in_the_key_tables_eight_strings`
proves it mechanically. Everything below the key is
`fr_dsn::format::json`'s: two-space indent, `": "` after every key, no
...
`fr_dsn::format::double::java_format_fixed`, so the report does not depend on
the process locale.

## Tests

`cargo test -p fr-drc` runs the unit tests plus the integration suites:
...
non-trivial quality scores) are the goldens `crates/freerouting`'s
...
- **The quality score is computed one crate up.** It needs the board
  ...
  it injected and `crates/freerouting/src/commands/drc.rs` computes it.
