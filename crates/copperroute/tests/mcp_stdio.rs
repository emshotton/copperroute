...
fn initialize_ping_and_list_over_pipes() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_copperroute"))
        .arg("mcp")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    assert_eq!(init["result"]["serverInfo"]["name"], "copperroute");
    writeln!(
        stdin,
...
use copperroute::mcp::server::{State, ToolDef};
use copperroute::mcp::stdio::run_with;
...
impl Pipes {
    fn start() -> Pipes {
        let mut command = Command::new(env!("CARGO_BIN_EXE_copperroute"));
        command.arg("mcp");
        let mut child = command
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("the copperroute binary starts");
...
fn all_schemas() -> Value {
    use copperroute::mcp::tools::schema;
    json!({
...
fn the_schemas_match_the_committed_golden() {
    ...
    if std::env::var_os("COPPERROUTE_UPDATE_GOLDEN").is_some() {
        std::fs::write(&path, format!("{rendered}\n")).expect("the golden is writable");
        return;
    }
    let golden = std::fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "{}: {e}\nrun with COPPERROUTE_UPDATE_GOLDEN=1",
            path.display()
        )
    });
    assert_eq!(
        rendered.trim_end(),
        golden.trim_end(),
        "the tool schemas changed; review the diff and re-run with COPPERROUTE_UPDATE_GOLDEN=1"
    );
}
...
fn the_tool_list_and_schemas_over_spawned_pipes() {
    ...
    assert_eq!(result["serverInfo"]["name"], "copperroute");
...
fn drc_through_the_binary(board: &str) -> Value {
    let output = Command::new(env!("CARGO_BIN_EXE_copperroute"))
        .args(["drc", board])
        .output()
        .expect("the copperroute binary starts");
