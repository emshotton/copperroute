...
fn the_via_net_number_fixture_routes_instead_of_hanging() {
    ...
    let options = copper_dsn::DsnReadOptions::default();
    ...
    let summary = copper_dsn::ses_reader::read(text.as_bytes(), &mut board, &coordinate_transform)
...
fn the_session_is_imported_after_the_rules() {
    ...
    let mut request = copperroute::ops::load::LoadRequest::for_board(
        copperroute::ops::load::BoardSource::Path(dsn.clone()),
    );
    request.rules = Some(typed.clone());
    request.session = Some(ses.clone());
    let mut loaded = copperroute::ops::load::load(&request).expect("the fixture loads");
    let expected = copper_drc::DesignRulesChecker::new(&mut loaded.board)
        .get_all_violations()
        .len();
