    ...
    state.register_tool(
        ToolDef {
            ...
            description: "Run the design-rule check on a board and return the KiCad DRC report \
                          — the same document `copperroute drc` writes. Coordinates are in \
                          millimetres."
                .into(),
...
pub fn file_payload_fields(details: &copper_core::BoardFileDetails) -> Value {
    json!({
        "size": details.size,
        "crc32": details.crc32,
