...
//! | `searchTreeManager` | mutated item by item (`:1254`, `:1273`) | replaced wholesale | the *contents* agree; the tree **shape** is the pre-attempt one rather than Java's remove-then-reinsert one. Quirk #74's lesson is that shape can matter, so this is measured rather than argued: `p7t8 item` drives every item of a stem and dumps the final board, and a shape divergence would show as different geometry on a later item |
