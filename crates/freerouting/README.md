...
| 11 | shutdown after the peer goes away | the bridge thread calls `System.exit(0)` the moment `readLine` answers `null`, killing the JVM out from under anything still running — including a response half written to stdout. A read failure on stdin is `System.exit(1)`, the same way | EOF (and a read or **write** failure) cancels every in-flight `tools/call`, then drains until the last tool thread has reported, then exits. The exit code is Java's on Java's two paths — 0 for EOF, 1 for a read failure — and **1** for a write failure, which Java cannot reach because `PrintStream.println` swallows its errors. Not visible in any transcript line; visible as process-exit latency after stdin closes | `Freerouting.java:777-782`, `:771` |
