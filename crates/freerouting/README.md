...
`--schema <kicad|freerouting>` (default `kicad`; the other spelling is the
...
`--set` names a field by its path under `router.`; `freerouting mcp`'s
...
| `check_drc` | `dsn_path` \| `dsn_text`, `ses_path?`, `rules_path?`, `kicad_project_path?` | the KiCad DRC report — the same document `freerouting drc` writes, coordinates in `mm` |
...
to `freerouting mcp` sits below it.
...
`cargo test -p freerouting` runs the unit tests plus six integration
suites:
...
cargo fmt --all --check
FR_SLOW_PARITY=1 cargo test --release            # the slow lanes

# re-cut the CLI goldens from the binary, after a change that is meant to move them
FR_REGOLDEN=<label> cargo test -p freerouting --test cli_e2e
```
...
for the whole-board `batch.*` files `fr-router`'s `batch_parity` reads,
