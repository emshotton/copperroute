...
|---|---|---|
| `route_board` | `dsn_path` \| `dsn_text`, `ses_path?`, `rules_path?`, `output_path?`, `settings?` | `ses_path` when `output_path` was given, else `ses_text` **and** `data` (Base64); plus `stats`, `incompletes`, `unrouted_report`, `drc_violation_count`, `timed_out`, and `job_id`/`size`/`crc32`/`format`/`filename`/`path` under `api/dto/BoardFilePayload`'s own names (ruling AO). **Always a Specctra session**, whatever went in — a KiCad design JSON routes to a `.ses` here, because spec §13's result has no member for anything else. (`route -de board.json -do out.json` used to reproduce quirk **#289** and write the board *as loaded*; Plan 9 Task 3 fixed it, so the CLI's `.json` now carries the routed board too.) |
...
| `do-out-dsn` | `-de <dsn> -do b.dsn -mp 1` | MATCH | quirk #268, **fixed in Plan 9 Task 3**: the jar writes a 0-byte file and exits 1; the port refuses the extension at the argument and exits 1 having written nothing. Still a MATCH — the exit codes agree and the port's refusal is port-only prose `MESSAGE_MAP` does not carry |
...
| **`p8t7`** | spec §1's acceptance: KiCad DSN → route → SES → re-read by `fr_dsn::ses_reader::read`; `-de board.json -do out.ses`; and quirk #289 (label T)'s measurement, an **XDIFF** since Plan 9 Task 3 — the jar writes the board as loaded, the port the routed one. Rungs (a)/(b) are port-golden since the M1 accept wave; **rung (c) keeps its live jar in both lanes and cannot convert** — two of its three checks are assertions about the *jar*, which a port-cut golden cannot carry | `run.sh p8t7` | **3 rungs: 2 MATCH, 1 XDIFF, 0 DIFF** (under `--against-jar`, rungs (a)/(b) DIFF by design — R1/R2; rung (c) is unchanged by the flag) |
