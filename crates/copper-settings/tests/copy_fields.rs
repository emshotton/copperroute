use copper_settings::copy_fields::{CopyFields, NamedEnum, enum_copy_by_name};
...
fn absent_source_fields_are_not_copied() {
    ...
    target.algorithm = Some("copperroute-router".to_string());

    let report = target.apply_new_values_from(&source);

    assert_eq!(target.max_passes, Some(42));
    assert_eq!(target.algorithm.as_deref(), Some("copperroute-router"));
...
fn scalars_wrappers_and_strings_copy() {
    ...
    source.algorithm = Some("copperroute-router-v19".to_string());
    ...
    assert_eq!(target.algorithm.as_deref(), Some("copperroute-router-v19"));
...
fn populated_optimizer() -> OptimizerSettings {
    OptimizerSettings {
        enabled: Some(true),
        algorithm: Some("copperroute-optimizer".to_string()),
        max_passes: Some(21),
        max_items: Some(22),
        max_threads: Some(23),
...
fn populated_router() -> RouterSettings {
    ...
    s.enabled = Some(true);
    s.algorithm = Some("copperroute-router-v19".to_string());
