...
use copper_settings::field_path::{FieldKind, set_field_value};
use copper_settings::{
...
fn separator_characters_in_the_value_survive() {
    ...
    set_field_value(&mut settings, "algorithm", "copperroute-router").expect("resolves");
    assert_eq!(settings.algorithm.as_deref(), Some("copperroute-router"));
}
...
fn field_tables_match_the_declaration_order_pins() {
    fn rust_names(fields: &[copper_settings::field_path::FieldSpec]) -> Vec<&'static str> {
...
fn field_kinds_match_the_java_field_types() {
    ...
    fn check(
        what: &str,
        fields: &[copper_settings::field_path::FieldSpec],
        want: &[(&str, FieldKind)],
    ) {
...
fn every_field_converts_according_to_its_kind() {
    ...
    let tables: [(&str, &[copper_settings::field_path::FieldSpec]); 5] = [
        ("", RouterSettings::FIELDS),
