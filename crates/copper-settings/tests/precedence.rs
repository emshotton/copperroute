...
fn rules_outrank_env_and_cli_for_autoroute_fields() {
    ...
    let env = env_settings(&[
        ("COPPERROUTE__ROUTER__SCORING__VIA_COSTS", "20"),
        ("COPPERROUTE__ROUTER__MAX_PASSES", "111"),
    ]);
...
fn scheduler_rules_path_follows_javas_else_if_chain() {
    let dir = std::env::temp_dir().join("copper-settings-plan4-task8");
...
fn adjacent_rules_discovery_strips_the_extension_javas_way() {
    let dir = std::env::temp_dir().join("copper-settings-plan4-task8-dotfile");
    let _ = std::fs::create_dir_all(&dir);
