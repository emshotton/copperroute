...
`copper_settings::prelude::*` to bring in every public type.

The crate sits on `copper-dsn` (which owns the `.dsn` and `.rules` readers) and
on `copper-board` (for the one pass that reads a board), and depends on `serde`,
...
| 10 | `JsonFileSettings` | a `copperroute.json` named on the command line (`--settings <file>`) |
...
| 55 | `EnvironmentVariablesSource` | `COPPERROUTE__ROUTER__<A>__<B>=v` |
...
- **`resolve_headless` models the CLI-started run.** An MCP `route_board`
  call has a sparse priority-70 payload instead, and `crates/copperroute`
...
in declaration order through `copper_dsn::format::json::to_gson_string_pretty`,
...
priority-60 `CliSettings` spelling. The `copperroute` binary no longer accepts
that form; nothing in the workspace calls this table any more.

## Tests

`cargo test -p copper-settings` runs the unit tests plus eleven integration
...
- **`corpus.rs`** runs `DsnFileSettings::new` over every `.dsn` in the
  ...
  `cargo test -p copper-settings --release --test corpus`.
