...
use copper_geometry::{Area, IntBox, PolylineShapeRef, Shape, TileShape};
...
const CRIT_INT: f64 = copper_geometry::CRIT_INT as f64;
...
pub fn read_snap_angle(
    scanner: &mut DsnScanner,
) -> Result<Option<copper_board::AngleRestriction>, DsnError> {
    use copper_board::AngleRestriction;
...
fn insert_missing_power_planes(layer_info: &[DsnLayer], p: &mut ReadScopeParameter<'_>) {
    ...
    let conduction_area_layers: Vec<i32> = board
        ...
        .filter_map(|id| match board.get_item(id) {
            Some(copper_board::Item::ConductionArea(area)) => {
                i32::try_from(area.area.get_layer()).ok()
            }
            _ => None,
        })
        .collect();
...
mod tests {
    ...
    fn read_snap_angle_reads_the_three_keywords() {
        assert_eq!(
            ...
            Some(copper_board::AngleRestriction::NinetyDegree)
        );
        assert_eq!(
            read_snap_angle(&mut scan("fortyfive_degree)")).expect("scan"),
            Some(copper_board::AngleRestriction::FortyFiveDegree)
        );
        assert_eq!(
            read_snap_angle(&mut scan("none)")).expect("scan"),
            Some(copper_board::AngleRestriction::None)
        );
