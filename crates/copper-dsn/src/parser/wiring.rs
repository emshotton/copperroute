...
use copper_board::datastructures::TimeLimit;
use copper_board::rules::ItemClass;
use copper_board::{Board, BoardError, FixedState, Item, ItemId, NetClassId, PadstackId};
use copper_geometry::{Area, FloatPoint, Line, Point, Polygon, Polyline, TileShape};
...
fn try_correct_net(board: &mut Board, id: ItemId) {
    let Some(copper_board::Item::Trace(trace)) = board.items.get(&id) else {
        return;
    };
...
fn via_exists(
    board: &Board,
    location: &copper_geometry::IntPoint,
    padstack: PadstackId,
    net_numbers: &[i32],
) -> bool {
    ...
    for id in board.pick_items(&point, Some(pick_layer)) {
        let Some(copper_board::Item::Via(via)) = board.items.get(&id) else {
            continue;
        };
