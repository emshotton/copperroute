use copper_board::{Keepout, PackagePin, Padstack, PadstackId, Padstacks, equals_ignore_case};
use copper_geometry::{Area, Circle, IntVector, Shape, ShapeOps, TileShape, Vector};
...
pub fn write_package_scope(p: &mut WriteScopeParameter<'_>, board_package: &copper_board::Package) {
...
fn are_package_pins_identical(pkg1: &copper_board::Package, p2: &[PackagePin]) -> bool {
...
pub fn read_library_scope(p: &mut ReadScopeParameter<'_>) -> Result<bool, DsnError> {
    ...
    let board = p.board.as_mut().expect("checked above");
    board.library.packages = copper_board::Packages::new();
