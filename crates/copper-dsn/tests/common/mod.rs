...
fn point(p: Option<copper_geometry::Point>) -> String {
    match p {
        Some(copper_geometry::Point::Int(p)) => format!("({},{})", p.x, p.y),
...
fn fixed_state(item: &Item) -> &'static str {
    match item.header().get_fixed_state() {
        copper_board::FixedState::Unfixed => "UNFIXED",
...
pub fn read_directed(path_name: &str) -> (Board, copper_dsn::CoordinateTransform) {
    let name = format!("p8t13-{path_name}.dsn");
    let text = test_data(&name);
    let options = copper_dsn::parser::scope_parameter::DsnReadOptions::default();
    match copper_dsn::read_board(text.as_bytes(), None, Some(&name), &options) {
...
pub fn write_directed_ses(
    board: &Board,
    ct: &copper_dsn::CoordinateTransform,
    path_name: &str,
) -> String {
    let mut out: Vec<u8> = Vec::new();
    copper_dsn::ses_writer::write(board, ct, &mut out, &format!("p8t13-{path_name}"))
