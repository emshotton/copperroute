use copper_board::{AngleRestriction, Item, Layer, LayerStructure};
use copper_dsn::format::{DSN_RESERVED, IdentifierType, IndentFileWriter};
use copper_dsn::keyword::{Keyword, ScopeKeyword};
use copper_dsn::lexer::{DsnScanner, Token};
use copper_dsn::parser::autoroute_settings::{DsnRouterSettings, write_autoroute_settings_scope};
...
fn the_corpus_sweep_counts_boards_whose_settings_were_read() {
    ...
    for path in &files {
        ...
        if let copper_dsn::BoardReadResult::Success {
            metadata: Some(metadata),
            ..
        } = copper_dsn::read_metadata(bytes.as_slice())
