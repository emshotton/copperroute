...
use copper_board::{Board, DefaultItemClearanceClasses, Item, ItemClass, NetClassId, Unit};
use copper_dsn::error::{BoardMetadata, BoardReadResult};
use copper_dsn::format::format_double;
use copper_dsn::kicad::{UnitJson, read_board};
use copper_geometry::PolylineShapeRef;
...
fn angle_restriction_name(angle: copper_board::AngleRestriction) -> &'static str {
    match angle {
...
fn emit_shape(shape: Option<&copper_geometry::Shape>) -> String {
    use copper_geometry::{Shape, ShapeOps, TileShape};
...
fn emit_corners(corners: &[copper_geometry::FloatPoint]) -> String {
...
fn fixed_state_name(state: copper_board::FixedState) -> &'static str {
    match state {
        copper_board::FixedState::Unfixed => "NOT_FIXED",
        copper_board::FixedState::ShoveFixed => "SHOVE_FIXED",
        copper_board::FixedState::UserFixed => "USER_FIXED",
        copper_board::FixedState::SystemFixed => "SYSTEM_FIXED",
    }
}

fn emit_b(result: &BoardReadResult) -> Vec<String> {
    use copper_geometry::{Area, ShapeOps};
...
fn an_unknown_unit_falls_through_to_the_documented_arm() {
    ...
    assert_eq!(
        serde_json::from_str::<copper_dsn::kicad::KiCadBoardJson>(r#"{"unit":"FOO"}"#)
            .expect("Gson does not throw on an unknown constant")
            .unit,
        None
    );
    assert_eq!(
        serde_json::from_str::<copper_dsn::kicad::KiCadBoardJson>("{}")
            .expect("parses")
            .unit,
        Some(UnitJson::MM)
    );
    ...
    let names: Vec<&str> = (1..=board.library.padstacks.count())
        .map(|no| {
            board
                .library
                .padstacks
                .get(copper_board::PadstackId(no))
                .expect("1..=count")
                .name
                .as_str()
        })
