...
use copper_dsn::parser::dsn_file::{read_float_scope, read_integer_scope, read_on_off_scope};
use copper_dsn::parser::geometry::{DsnLayer, DsnLayerStructure};
use copper_dsn::parser::scope_parameter::{
    DsnReadOptions, ReadScopeParameter, read_scope, skip_scope,
};
...
fn a_truncated_dsn_reads_as_partial_not_success() {
    ...
    let options = DsnReadOptions::default();
    let result = copper_dsn::read_board(text.as_bytes(), None, Some("truncated"), &options);
    let copper_dsn::BoardReadResult::Partial {
        board, diagnostic, ..
    } = result
    else {
    ...
    assert!(matches!(
        copper_dsn::read_board(closed.as_bytes(), None, Some("truncated"), &options),
