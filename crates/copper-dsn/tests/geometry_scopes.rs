...
use copper_dsn::parser::geometry::{
...
fn board_to_dsn_shape_maps_a_box_to_a_rectangle() {
    let transform = CoordinateTransform::new(10.0, 0.0, 0.0).expect("a valid scale");
    let shape = copper_geometry::Shape::Tile(copper_geometry::TileShape::Box(IntBox::new(
        IntPoint::new(0, 0),
...
fn a_circle_bounding_box_is_not_twice_too_wide() {
    ...
    let transform = CoordinateTransform::new(10.0, 0.0, 0.0).expect("a valid scale");
    let board_circle =
        copper_geometry::Shape::Circle(copper_geometry::Circle::new(IntPoint::new(100, 200), 25));
...
#[test]
fn write_parser_scope_reduced_drops_the_quote_and_the_router_marker() {
    let info = Communication {
        host_cad: Some("KiCad".to_string()),
        ..Communication::default()
    };
...
#[test]
fn read_parser_scope_reads_the_quote_char_host_and_router_marker() {
    let options = DsnReadOptions::default();
    let mut p = ReadScopeParameter::new(
        scan(
            "(string_quote ')(host_cad KiCad)(host_version 8.0)(constant a b)(write_resolution mil 10)(generated_by_copperroute))",
        ),
        &options,
    );
