...
//!
//! `DsnWriter.writePcbScope` (DsnWriter.java:60-70) reads the transform straight off the board:
...
//!
//! DsnWriter.java:76-80 passes the literal `false` for `reduced`, so `(string_quote …)` and
...
/// `DsnWriter.write(BasicBoard, OutputStream, String, boolean)` (DsnWriter.java:46-52).
///
...
/// `DsnWriter.writePcbScope` (DsnWriter.java:58-88).
///
/// `WriteScopeParameter` is built with `autorouteSettings = null` (DsnWriter.java:63), so the
