...
// Quirk #105's directed test — `read_via_scope_pads_a_multi_subnet_vias_net_numbers_with_zeros`
// — used to live here. Plan 9 Task 5 fixed the quirk, and the test moved with it to
// `tests/wiring.rs::a_multi_subnet_via_carries_every_net_number`, which is where the
// port-vs-jar divergence on that one row is now written down. `tests/plan_3_zero_coverage.rs`'s
// register points at the new home.
