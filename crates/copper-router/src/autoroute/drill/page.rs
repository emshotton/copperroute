...
use copper_board::{Board, ItemId, StopCheck, TreeObject};
use copper_geometry::{IntBox, Point, PolylineArea, TileShape};
        ...
        for current_entry in overlaps {
            ...
            let smd_skip = attach_smd && {
                ...
                board.get_item(item).is_some_and(|i| match i {
                    copper_board::Item::Pin(pin) => pin.drill_allowed(&ctx),
                    _ => false,
                })
            };
            if smd_skip {
...
fn calc_pin_center_in_drill(drill_shape: &TileShape, layer: usize, board: &Board) -> Option<Point> {
    let overlapping_items = board.overlapping_items(
        &copper_geometry::Area::Shape(copper_geometry::Shape::Tile(drill_shape.clone())),
        Some(layer),
    );
    let ctx = board.ctx();
    let mut result = None;
    for item in overlapping_items.into_iter().rev() {
        let Some(copper_board::Item::Pin(pin)) = board.get_item(item) else {
            continue;
        };
