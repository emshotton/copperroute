...
use copper_board::{BoardError, ItemId, TimeLimit};
use copper_geometry::{FloatPoint, IntOctagon, IntPoint, Line, Point, Polyline, Side, Signum};
...
/// `copper-board` uses for the same purpose.
...
impl<'a> TightenerBase<'a> {
    ...
    pub(crate) fn check(&self, board: &mut Board, shape: &copper_geometry::TileShape) -> bool {
    ...
    pub(crate) fn clip_is_outside(&self, point: &Point) -> bool {
        match &self.current_clip_shape {
            Some(clip) => copper_geometry::TileShape::Octagon(*clip).is_outside(point),
            None => false,
        }
    }
    ...
    pub(crate) fn line_through(corner: &FloatPoint, dir: &copper_geometry::Direction) -> Line {
