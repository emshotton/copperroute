use copper_board::BoardError;
use copper_board::ids::{ItemId, PadstackId};
...
use copper_router::PageId;
use copper_router::autoroute::maze::engine::AutorouteEngine;
use copper_router::board_ext::{DrillItemMover, RoutingBoardExt, TraceShover};
...
fn assert_rows_match_returning_stale_failing(mode: &str, actual: &[String]) -> usize {
    ...
    for i in 0..expected.len().max(actual.len()) {
        ...
        if differs_only_by_the_end_closing_line(want, got) {
            changed_closing_lines += 1;
            continue;
        }
        diffs.push(format!("row {i}\n  jvm:  {want}\n  rust: {got}"));
    }
    if MODES_WITH_TWO_POINT_POLYLINES.contains(&mode) {
        assert!(changed_closing_lines > 0);
    }
    assert!(
        diffs.is_empty(),
...
/// `P6T15bProbe.ln`.
fn t15b_line(line: &copper_geometry::Line) -> String {
