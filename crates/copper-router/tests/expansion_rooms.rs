use copper_board::datastructures::ShapeTree;
use copper_board::ids::{ItemId, RoomId, TreeObject};
use copper_board::prelude::*;
use copper_dsn::{BoardReadResult, DsnReadOptions};
use copper_geometry::{FloatLine, IntBox, ShapeBoundingDirections, TileShape};
...
use copper_router::autoroute::maze::{MazeAdjustment, MazeSearchElement};
...
fn every_expandable_id_is_stable_and_injective() {
    ...
    let mut engine = copper_router::autoroute::maze::AutorouteEngine::new(&mut board, 1, false);
...
fn a_target_door_id_folds_in_the_item_and_the_room() {
    assert_eq!(
        copper_router::autoroute::expansion::target_door_id(ItemId(3), 4),
        3 * 31 + 4
    );
    assert_eq!(
        copper_router::autoroute::expansion::target_door_id(ItemId(3), 0),
        3 * 31
    );
}
...
fn a_target_door_intersects_the_item_with_its_room_and_a_null_room_is_empty() {
    ...
    assert_eq!(
        ...
        Some(copper_router::autoroute::expansion::target_door_id(
            item, room_id_no
        ))
    );
    ...
    assert_eq!(
        ...
        Some(copper_router::autoroute::expansion::target_door_id(item, 0))
    );
