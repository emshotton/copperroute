...
fn a_user_fixed_contact_never_reaches_the_ripped_connections() {
    ...
    let connections = board.connection_items(trace, copper_board::StopConnectionOption::None);
    ...
    assert!(
        unfixed_board
            .connection_items(free_trace, copper_board::StopConnectionOption::None)
            .contains(&free_via)
    );
}
...
fn an_unimproved_item_restores_the_clone_byte_for_byte() {
    ...
    optimizer.min_cumulative_trace_length = f64::from(
        copper_router::score::BoardStatistics::new(&mut board)
            .traces
