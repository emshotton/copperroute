...
use copper_geometry::{IntBox, IntOctagon, IntPoint, Point, Polyline, Shape, TileShape};
use copper_router::autoroute::maze::ViaPricing;
use copper_router::board_ext::RoutingBoardExt;
use copper_router::pipeline::{BatchAutorouter, NamedAlgorithmType, RouterBudget};
use copper_router::{
    AutorouteAttemptState, AutorouteEngine, route_connection, route_connection_full,
};
...
fn the_constants_are_javas_literals() {
    ...
    assert_eq!(
        copper_router::BoardHistory::MAX_HISTORY_SIZE,
        BatchAutorouter::BOARD_RANK_LIMIT,
        "BOARD_RANK_LIMIT = BoardHistory.MAX_HISTORY_SIZE (:40)"
    );
    assert_eq!("copperroute-router", BatchAutorouter::ID);
    assert_eq!("Copperroute Auto-router", BatchAutorouter::NAME);
    assert_eq!("1.0", BatchAutorouter::VERSION);
    assert_eq!("Copperroute Auto-router v1.0", BatchAutorouter::DESCRIPTION);
...
fn retain_autoroute_database_has_no_setter() {
    ...
    assert!(
        offenders.is_empty(),
        "ruling AJ: `retainAutorouteDatabase` must have no setter anywhere in `copper-router`, but \
...
fn the_work_list_is_sorted_by_airline_distance() {
    ...
    for (item, expected) in [(c1, 2000.0), (c2, 3000.0), (c3, 8000.0)] {
        assert_eq!(
            copper_router::pipeline::calculate_item_distance(&board, item),
            expected
        );
    }
...
fn equal_airline_distances_keep_the_descending_id_tie_order() {
    ...
    let keys: Vec<f64> = [c1, c2, c3]
        .into_iter()
        .map(|id| copper_router::pipeline::calculate_item_distance(&board, id))
        .collect();
    assert_eq!(
        keys,
...
fn an_empty_pass_still_polls_the_job_deadline() {
    ...
    let settings = RouterSettings::new();
    let mut router = BatchAutorouter::for_routing_job(&board, &settings, RouterBudget::disabled());
    let stop = copper_router::pipeline::RouterStop::with_deadline(-1);
    let mut failure_log = copper_router::pipeline::RoutingFailureLog::new();
    let mut sink = copper_router::pipeline::NoopProgressSink;
...
impl copper_router::pipeline::ProgressSink for IncompleteCountSink {
    fn on_event(&mut self, event: &copper_router::pipeline::RoutingEvent) {
        if let copper_router::pipeline::RoutingEvent::BoardUpdated { counters } = event {
