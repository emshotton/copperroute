...
use copper_board::ids::{ConnectionId, ItemId, RoomId};
use copper_board::prelude::*;
use copper_geometry::{Area, IntBox, IntPoint, Point, Polyline, Shape, TileShape};
...
fn tree_rooms(board: &Board, engine: &AutorouteEngine) -> std::collections::BTreeSet<RoomId> {
    ...
    tree.overlapping_tree_entries_with_rooms(&probe, None, &[], &board.items, &engine.rooms, &ctx)
        ...
        .filter_map(|entry| match entry.object {
            copper_board::ids::TreeObject::Room(id) => Some(id),
            copper_board::ids::TreeObject::Item(_) => None,
        })
        .collect()
}
...
fn touching_sides_is_length_checked() {
    ...
    let room_ref = copper_router::autoroute::expansion::RoomRef::Complete(rooms[1]);
...
fn an_abandoned_rooms_incomplete_neighbours_are_not_deleted_with_it() {
    let mut store = copper_router::autoroute::expansion::ExpansionRoomStore::new();
...
fn a_committed_room_survives_the_catch() {
    ...
    let answer = engine.complete_expansion_room(&mut board, stale);
    assert!(
        matches!(answer, Err(copper_router::RouterError::Panicked(_))),
