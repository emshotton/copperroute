...
use copper_geometry::{
    FloatPoint, IntBox, IntOctagon, IntPoint, Point, Polyline, Shape, TileShape,
};
...
fn run_one_pass(
    ...
    stop: &RouterStop,
) -> Result<bool, copper_router::RouterError> {
...
fn a_panicking_item_ends_the_pass_and_returns_an_error() {
    ...
    let Err(copper_router::RouterError::Panicked(message)) = answer else {
...
fn the_failure_log_records_what_java_records() {
    ...
    log.record_failure(
        ...
        item,
        7,
        copper_router::AutorouteAttemptState::Failed,
    ...
    assert_eq!(
        ...
        Some(copper_router::AutorouteAttemptState::Failed),
        ":138"
    );
    ...
    log.record_failure(
        ...
        item,
        9,
        copper_router::AutorouteAttemptState::InsertError,
        None,
    );
...
fn calculate_airline_takes_the_closest_drill_item_pair() {
    ...
    let airline = copper_router::pipeline::calculate_airline(&board, &start, &dest)
    ...
    let reversed = copper_router::pipeline::calculate_airline(&board, &dest, &start)
...
fn calculate_airline_answers_none_when_either_side_has_no_drill_item() {
    ...
    assert!(
        copper_router::pipeline::calculate_airline(&board, &vias, &traces).is_none(),
        ":26-29 skips every destination, so `toCorner` stays null"
    );
    assert!(
        copper_router::pipeline::calculate_airline(&board, &traces, &vias).is_none(),
        ":20-23 skips every source, so `fromCorner` stays null"
    );
    assert!(copper_router::pipeline::calculate_airline(&board, &empty, &vias).is_none());
    assert!(copper_router::pipeline::calculate_airline(&board, &vias, &empty).is_none());
    assert!(copper_router::pipeline::calculate_airline(&board, &empty, &empty).is_none());
}
