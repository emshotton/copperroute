...
use copper_geometry::{
    FloatPoint, IntBox, IntOctagon, IntPoint, IntVector, Point, Shape, TileShape,
};
use copper_router::board_ext::{combined_fallback_via_rule, sorted_unconnected_targets};
use copper_router::pipeline::{BatchFanout, EscapeStatistics, FanoutComponent, FanoutPin};
use copper_router::score::BoardStatisticsFanout;
...
fn load_board_with_rules(rel_path: &str, rules: Option<&str>) -> (Board, bool) {
    ...
    let (mut board, transform) = match copper_dsn::read_board(
        ...
        | copper_dsn::BoardReadResult::OutlineMissing {
            board,
            coordinate_transform,
            ..
        } => (
    ...
    if let Some(rules) = rules {
        ...
        applied = copper_dsn::rules_reader::read(rules_file, stem, &mut board, &transform, None)
