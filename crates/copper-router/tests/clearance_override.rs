...
use copper_board::{Board, Item, ItemClass};
...
/// `copper-board` cannot depend on `copper-settings`, so
/// [`copper_board::DEFAULT_COPPER_TO_EDGE_CLEARANCE_UM`] is a second copy of
...
fn the_two_copies_of_the_default_copper_to_edge_clearance_agree() {
    assert_eq!(
        copper_board::DEFAULT_COPPER_TO_EDGE_CLEARANCE_UM,
...
fn prepare_board_skips_a_none_setting_and_orders_copper_before_hole() {
    ...
    let hole_edge = both
        .rules
        .clearance_matrix
        .get_no(copper_board::HOLE_EDGE_CLEARANCE_CLASS_NAME)
