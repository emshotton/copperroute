...
use copper_router::autoroute::maze::engine::describe_connection;
use copper_router::board_ext::RoutingBoardExt;
use copper_router::{AutorouteAttemptResult, AutorouteAttemptState, route_connection};
...
fn ln(line: &copper_geometry::Line) -> String {
...
fn pt(polyline: &Polyline, no: usize) -> String {
    match polyline.corner(no) {
        ...
        _ => {
            ...
            format!(
                "~({},{})",
                copper_dsn::format::double::format_double(f.x),
                copper_dsn::format::double::format_double(f.y)
            )
        }
    }
}
