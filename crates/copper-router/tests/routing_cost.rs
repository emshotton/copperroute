...
fn the_job_deadline_ends_the_optimizer_stage_on_a_routed_board() {
    ...
    assert_eq!(result.state, copper_router::pipeline::TaskState::Cancelled);
    assert!(
...
fn the_routing_decision_statistics_skip_the_fanout_census() {
    ...
    assert_eq!(
        slim.fanout,
        copper_router::score::BoardStatisticsFanout::default()
    );
}
...
fn the_optimizers_carried_incomplete_counts_match_a_full_count() {
    ...
    let mut cursor = copper_router::pipeline::ReadSortedRouteItems::new();
    let mut accepted = 0;
    let mut undone = 0;
