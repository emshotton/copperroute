...
too. Use `copper_router::prelude::*` to bring in every public type.

The crate depends on `copper-board`, `copper-geometry`, `copper-settings`, `copper-drc` (for
the unrouted report and the score's clearance block) and `thiserror`, and on
nothing else. `copper-dsn` and `parity` are dev-dependencies, for fixtures.
...
| `pipeline/optimizer.rs` | `BatchOptimizer::run_batch_loop`, `opt_route_pass`, `opt_route_item` — re-route one item at a time and keep the result only if the score improves, rolling back through `copper-board`'s undo journal otherwise |
...
* **No new workspace dependencies.** Not `rand` (see `SplitMix64` in
  `copper-geometry`), not `slotmap` (see `Arena` below), not `rayon`.
...
* **No GUI, no logging, no observers, no clock, and no static mutable state,
  with one recorded exception:** `copper_geometry::Line`'s identity token, which
...
`copper-drc`) and fanout. `normalized_score` folds it into one number from the
...
`cargo test -p copper-router` runs the unit tests plus fifty-odd integration
