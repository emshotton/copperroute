...
The crate sits on `fr-geometry` and depends on `num-bigint` and `thiserror`
...
applied once after a design is loaded, by `fr_router::pipeline::prepare_board`.
...
`cargo test -p fr-board` runs the unit tests plus twenty integration suites,
