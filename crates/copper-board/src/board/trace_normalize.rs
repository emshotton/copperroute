...
use copper_geometry::{IntOctagon, Line, Point, Polyline, TileShape};
...
impl Board {
    ...
    pub fn split_trace_checked(
        ...
        for i in 0..lines.lines().len().saturating_sub(2) {
            ...
            let Some(current_line_segment) =
                copper_geometry::LineSegment::from_polyline(&lines, i + 1)
            else {
                continue;
            };
            ...
            while cursor < entries.len() {
                ...
                match found_item {
                    Item::Trace(found_trace) => {
                        ...
                        let Some(found_line_segment) = copper_geometry::LineSegment::from_polyline(
                            &found_lines,
                            shape_index + 1,
                        ) else {
                            continue;
    ...
    fn split_overlapping_entries(
        ...
        for group in entries.chunk_by(|a, b| a.object == b.object) {
            if let TreeObject::Item(entry_id) = group[0].object
