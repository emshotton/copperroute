use copper_board::datastructures::{LeafId, Node, ShapeTree, TreeEntry};
...
fn dump(tree: &ShapeTree<TreeObject>) -> String {
    ...
    fn walk(
        ...
        id: copper_board::datastructures::NodeId,
        prefix: &str,
        out: &mut String,
    ) {
        match tree.node(id) {
...
fn assert_bounds_are_the_union_of_children(tree: &ShapeTree<TreeObject>) {
    fn walk(tree: &ShapeTree<TreeObject>, id: copper_board::datastructures::NodeId) {
        let Node::Inner {
            bounds,
            first_child,
            second_child,
