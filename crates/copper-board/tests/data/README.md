# `copper-board` JVM probes (Plan 5)

JUnit-free Java drivers whose output is the source of the expected values in
`crates/copper-board/tests/clearance_violations.rs` and of quirks row 153 in `docs/java-quirks.md`.
...
**Why there is no fixture-level Rust test here.** `copper-board` cannot depend on `copper-dsn` (the
