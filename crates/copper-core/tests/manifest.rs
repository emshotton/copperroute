...
fn transcript_len(recorded: usize) -> usize {
    let delta = copper_core::SERVER_VERSION.len() as isize - TRANSCRIPT_VERSION.len() as isize;
...
fn transcript() -> Transcript {
    ...
    .replace(TRANSCRIPT_VERSION, copper_core::SERVER_VERSION);
    ...
    for row in manifests.values_mut() {
        if row
            .lines
            .iter()
            .any(|line| line.contains(copper_core::SERVER_VERSION))
        {
...
fn scratch() -> PathBuf {
    let dir = std::env::temp_dir().join("copper-core-plan8-task4");
...
fn the_duration_narrowing_matches_the_transcript() {
    ...
    for (start, finish, expected) in transcript().durations {
        ...
        assert_eq!(
            copper_dsn::format_float(seconds),
            expected,
            "DUR {start}->{finish}"
        );
    }
...
fn an_unreadable_input_omits_the_sha256_key() {
    ...
    assert_eq!(none.fixture, Some(copper_core::FixtureInfo::default()));
...
fn resolve_git_sha_child() {
    if let Ok(path) = std::env::var("COPPERROUTE_GITSHA_OUT") {
