...
use copper_board::{Board, Item, ItemClass, TreeObject};
use copper_core::{apply_immediate_post_load_processing, apply_router_settings_for_loaded_board};
...
/// The headless ladder's priority-0 source, as `crates/copper-router/tests/clearance_override.rs` uses.
...
fn read_board(bytes: &[u8], design_name: &str) -> Board {
    match copper_dsn::read_board(bytes, None, Some(design_name), &DsnReadOptions::default()) {
...
fn the_override_runs_once_on_a_dsn_load() {
    ...
    assert_eq!(
        twice
            .get_outline()
            .and_then(|id| twice.get_item(id).map(copper_board::Item::clearance_class)),
        board
            .get_outline()
            .and_then(|id| board.get_item(id).map(copper_board::Item::clearance_class)),
...
fn a_defaulted_500_and_an_explicit_500_diverge() {
    ...
    assert_eq!(
        defaulted.get_outline().and_then(|id| defaulted
            .get_item(id)
            .map(copper_board::Item::clearance_class)),
        explicit.get_outline().and_then(|id| explicit
            .get_item(id)
            .map(copper_board::Item::clearance_class)),
    ...
    assert_ne!(
        unsupplied.get_outline().and_then(|id| unsupplied
            .get_item(id)
            .map(copper_board::Item::clearance_class)),
        defaulted.get_outline().and_then(|id| defaulted
            .get_item(id)
